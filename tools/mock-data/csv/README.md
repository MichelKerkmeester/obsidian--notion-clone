---
title: "tools/mock-data/csv: the AppFlowy import files"
...
# tools/mock-data/csv: the AppFlowy import files

Ten generated files, one per use case, for AppFlowy's CSV import. They are output, not source.
...
`true` or `false`, and a date as `YYYY-MM-DD`. Whether AppFlowy infers a select column from repeated
values or needs its options declared first is open, and is the reason `catalogue.json` exists beside
these files — it carries a neutral type per column, and a loader that has both should read the types
from there.
