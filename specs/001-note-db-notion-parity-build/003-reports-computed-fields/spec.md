...
- A new isolated `src/data/` module (e.g. a hypothetical `RemainingSaved.ts`): the `EuroFormat.ts` isolated-module pattern is inherited via existing CellRenderer/SummaryRenderer call sites, not cloned (see `plan.md` Architecture). **Deviation (recorded 2026-08-27):** this boundary was crossed during implementation. Three new `src/data/` modules shipped instead — `ReportsInspector.ts`, `ReportsComputedConfig.ts`, `ReportsDisplay.ts` — wired behind a `configure-reports-computed-fields` command (commits `6639789`/`0baacde`/`6cb5331`, wiring fix `c766117`). The operator reviewed the deviation and chose to keep the shipped code and fix its defects rather than revert to this config-only design; see `implementation-summary.md` Deviations from Plan for the full record. The Saved-field classification (REQ-004) remains deferred pending operator input.
