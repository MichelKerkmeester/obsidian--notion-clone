---
title: "Task Breakdown: Computed Output Format and the Formula Workbench"
description: "Tasks for 021-output-format-and-formula, one per requirement, each closed only with read evidence."
trigger_phrases:
  - "021 output format and formula tasks"
importance_tier: "high"
contextType: "planning"
---
# Task Breakdown: Computed Output Format and the Formula Workbench
...
- [ ] **T-001** Percent semantics are decided before migration: ratio or 0-100, plus rounding, negative, zero, missing and non-finite behaviour
- [ ] **T-002** Preview, cell, card and footer agree for plain, currency, percent, precision, empty, NaN and error results
- [ ] **T-003** The AI prompt action is removed. It exists today at `formula-modal.ts:446-458`, so this is a deletion, not an omission
- [ ] **T-004** Formula evaluation stays numeric, so comparisons, rollups, editing and validation are undamaged
- [ ] **T-005** Footer aggregates fit without shrinking type below the touch-legible baseline. The footer sets no font-size and inherits the row token; the likelier cause of bulk is the column flex stacking label above value
