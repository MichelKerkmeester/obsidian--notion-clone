---
title: "Task Breakdown: Modal Surface Taxonomy and Plugin-Owned Menus"
description: "Tasks for 018-surface-taxonomy-and-menus, one per requirement, each closed only with read evidence."
trigger_phrases:
  - "018 surface taxonomy and menus tasks"
importance_tier: "high"
contextType: "planning"
---
# Task Breakdown: Modal Surface Taxonomy and Plugin-Owned Menus
...
- [ ] **T-001** All twenty-four `new X(...)` call sites are unchanged by the base-class insertion
- [ ] **T-002** `formula-modal.ts` still opens its confirm-on-close dialog. It overrides `close()` and calls `super.close()` only after the confirm resolves, so a base class that wraps `close()` breaks it
- [ ] **T-003** `status-preset-manager-modal.ts` still works. It calls `this.onOpen()` as its re-render, so base-class setup placed there runs three times per interaction
- [ ] **T-004** Formula, Invalid Time Events and Property Type Conflict get full-screen rather than a sheet. They are 860-1240px workbenches with their own responsive ladders
- [ ] **T-005** Any new portal class is registered in `database-view.ts` `portalSelectors`, or interaction-scope ownership breaks
- [ ] **T-006** Nested sheets are a deliberate decision, not a discovery. Six modals host dropdown popovers that already self-convert to sheets
