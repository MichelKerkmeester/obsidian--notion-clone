...
|---|---|---|---|---|---|---|---|
| AC-001 (B1) | REQ-001, REQ-002 | For every family the census finds, create the checkbox through its production producer at its production mount point and read computed `appearance` | computed `appearance: none` for every family; 0 families fall back to the platform box | **1 of 12 families** | N1 | Unmet | - |
| AC-002 (B2) | REQ-003 | Read computed `border-radius` and box size for every family across board, gallery, list, table, modal and panel; collect the distinct values | radius and box size are identical within a role — the set of distinct values has cardinality 1 | families diverge; 11 of 12 fall back to the platform's box. The four selectors that do declare `appearance: none` disagree on size: `styles.css:5428` **16px**, `:6628` **16px**, `:8252` **18px**, `:11039` **20px** | N5 | Unmet | - |
| AC-003 (B3) | REQ-002 | Create the same family at three mount points — inside a board card, inside a modal, and on `document.body` — and compare computed appearance | appearance identical at all three mount points | appearance is decided by ancestry, so it differs by construction. All four `appearance: none` selectors are ancestor-scoped, and across **two different roots**: `styles.css:5428` and `:6628` under `.note-database-container`, `:8252` and `:11039` under `.note-database-modal` | N4 | Unmet | - |
