...
|---|---|---|---|---|---|---|---|
| AC-001 (A1) | REQ-002 | Render 20 list rows with wrapping off through the production renderer; read each row's `getBoundingClientRect().height`; compute the standard deviation | **Measured 2026-08-29 by `tools/live/view-census.mjs`: standard deviation of row height is 0 at every width and field count.** 20 synthesised rows at 1, 6 and 20 fields, wrapping off, at 320/402/768/1440 — mean 62px, sd 0, one distinct height in all 24 cells; and 16 sibling row groups in the real fixtures, all sd 0. **The rows are not ragged in height.** They were uniformly 26px too WIDE, at every width and every row, which is what a row whose right edge is off-screen looks like. Fixed by giving `.db-list-row` `box-sizing: border-box`; 0 list-row elements now pass their container anywhere in the census | 0 | N3 | Unmet | - |
