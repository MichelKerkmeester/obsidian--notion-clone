...
| AC-004 (L4) | REQ-004 | Run `npm run live:probe` against a good tree, against a seeded defect, and with Obsidian stopped | exit 0, exit 1, exit 2 respectively — each read without a pipe | no such script exists. The transport's own behaviour is already *machine-verified*: with the app not running, `obsidian --help` exits **1** with `The CLI is unable to find Obsidian. Please make sure Obsidian is running and try again.` — so the driver must translate that into its own exit 2 | N3 | Unmet | - |
...
| AC-006 (L6) | REQ-006 | After `npm run build`, search the built bundle for the probe API | `grep -c __ndProbe main.js` returns 0 | no probe exists. The check must read the built file, not the source: the plugin's manifest declares `isDesktopOnly: false`, so a shipped evaluation hook would reach phones too | N5 | Unmet | - |
...
Work has not started. All thirteen rows are `Unmet`. AC-001 to AC-003, AC-008, AC-009 and AC-011 are
marked *trace* because no live transport has been run yet; AC-010 is marked *census* because no
