...
description: "Tasks for key-scoped inverse resolution in RelationRollup.ts after a local relationField miss. Resolution and sourcePaths union are one atomic RelationRollup.ts diff."
trigger_phrases:
...
T003 resolution and T004 `sourcePaths` union are **one atomic `RelationRollup.ts` diff**. Do not ship the miss branch without the `targetPaths` union.
<!-- /ANCHOR:notation -->
...
- [ ] T002 Confirm live lines `RelationRollup.ts:36,58-88,62-66,92-129,21,76,159-160` and that `:36` still early-returns when the viewed DB has no rollup columns [10m]
<!-- /ANCHOR:phase-1 -->
...
- [ ] T004 **`sourcePaths` union** — same `RelationRollup.ts` diff as T003: union inverse `sourcePaths` into `targetPaths` (`:21,76`) so `DatabaseView.ts:3362` can see Expense paths (`src/data/RelationRollup.ts`) [S]
- [ ] T005 **Round-trip tests**: fixture a DB that *has* rollup columns so `:36` does not early-return; inverse `count === 2` / `list` contains both Expenses via `aggregateRollup`; local key still forward; empty → `emptyRollupValue` (`src/data/RelationInverse.test.ts`) [S]
<!-- /ANCHOR:phase-2 -->
...
- [ ] T006 Report `count`/`list` over two Expenses with only forward `Month` links, no Report frontmatter relation (SC-001) [S]
...
- [ ] T003–T004 shipped as one `RelationRollup.ts` diff
<!-- /ANCHOR:completion -->
