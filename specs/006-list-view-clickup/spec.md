---
title: "Feature Specification: List View as a ClickUp-Style Grid"
description: "The list view gains the table's whole feature surface by becoming a presentation mode of the grid, and takes on ClickUp's list chrome: per-group column headers, inline cell editing, and a per-group add-task affordance."
trigger_phrases:
  - "list view clickup"
  - "punch list view refactor"
  - "list needs table features"
  - "clickup list chrome"
  - "list grid presentation mode"
  - "006 list view"
...
_memory:
  continuity:
    ...
    next_safe_action: "Scaffold phase 000 and build the two view-semantic guard checks before any guard is touched"
    blockers: []
    key_files:
      - "spec.md"
      - "plan.md"
    ...
    completion_pct: 0
    open_questions:
      - "Does list view keep its stacked-title reading mode"
      - "Subtasks are in the operator's own reference; confirm they are dropped rather than deferred"
    answered_questions:
      - "Route B: the list becomes a presentation mode of the grid renderer. Operator decided."
      - "The ordinal sort indicator is ClickUp's own form, confirmed on five primary screens"
      - "The effort pill is a real ClickUp select cell, confirmed on four primary screens"
      - "The selection checkbox does not replace the row icon; it sits in a reserved leading gutter"
---
