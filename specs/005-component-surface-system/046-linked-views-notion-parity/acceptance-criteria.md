...
_memory:
  continuity:
    ...
    recent_action: "Re-verified P0 #1's row-pitch close after rebase and corrected the pitch figure"
...
| AC-002 | REQ-002, US-001 | **Given** an embed whose table is wider than the code-block host, **When** it renders, **Then** no column is clipped at the right edge and the embed spans the reading view's content width. **Threshold:** `|embedContentWidth - proseContentWidth| ≤ 1px`. **Red:** 874 against 900 on desktop and 376 against 402 on the phone — a 26px deficit, being the 1px border plus 12px padding on each side. **Green:** 900/900 and 402/402, delta 0px on both. The table overflows its host on both devices (1392 into 892, 659 into 394) and scrolls inside the embed rather than past it, with page `scrollWidth` equal to `clientWidth` (1440 and 402). **Row-pitch evidence (2026-09-05, `review-ui-2026-09-05.md` P0 #1):** the same `constructed-linked-view-host` scenario also carried an empty 34px band between every data row — a `db-row-insert-line` seam that `styles.css`'s `.note-database-embed.note-database-container table.db-table th, td { height: 34px }` (added in this AC's own green pass) ties in specificity with the seam's own `height: 0` rule and wins by source order, doubling the embed's height against the standalone table's. **Red:** `tools/live/unstyled-links.mjs`'s constructed pass, a new `insertLineRows` reading in `chrome-geometry-measure.mjs` (own 4-case unit test), 1999 findings at 34px each against a 1px ceiling. **Green:** 0 findings after adding a same-specificity `tr.db-row-insert-line > td { height: 0; border: 0 }` rule scoped to the embed; linked row pitch measured live in the same host drops 68px -> 34px, against the standalone table's 35px (`insertLineRows` seam heights 34 -> 0; all 4 of `constructed-linked-view-host-{desktop,mobile}-{dark,light}.png` opened and read, and the same pitch re-derived off the PNGs). The residual 1px is not this seam: the embed cell rule carries `box-sizing: border-box` (34px inclusive of its 1px bottom border) where the standalone cell is content-box (34px + 1px), a pre-existing difference this fix does not touch. Row 0 sitting under the sticky header is a separate, pre-existing observation in the same captures (Known Limitation 2, `implementation-summary.md`) that this fix does not touch — see that document for why it is a harness stand-in, not a shipped defect. | The width half is measured and green. The not-clipped half is NOT closed here: the red control also reports the columns reachable, because the constructed fixture gives the embed its own `overflow-x: auto` and so never reproduces the code-block-host clipping the operator reported. That half needs the device. The row-pitch half is closed: red then green in `tools/live/unstyled-links.mjs`, read without a pipe | Unmet | - |
