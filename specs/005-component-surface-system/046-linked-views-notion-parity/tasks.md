...
- [ ] T002 Answer the host-layout question against real Obsidian, not against a harness: can an embed span the reading view's content width without breaking the layout of prose around it, and at what cost. This is the critical path; a guess here is a rewrite (`src/views/embedded-database-renderer.ts`, `styles.css`). **Inferred from the constructed reading host, pending device confirmation:** Obsidian's `.markdown-preview-sizer` is the readable-line-length container governed by `--file-line-width`, and the embed is its block child. The constructed scenario mirrors that relationship with prose above and below and the run is now complete: the embed's content box matches the prose content box exactly at both device widths, and prose above and below stays in flow with no page-level horizontal overflow. That is an answer from a fixture, not from Obsidian — it remains inferred until the operator reads it on device, never confirmed
...
- [x] T004 Strip the block furniture: no card border, no nested database title duplicating the page heading, no expand/collapse chevron. `hideHeader: true` stays honoured for existing blocks and stops being the answer to an unwanted header (`src/views/embedded-database-renderer.ts`, `styles.css`) — the linked-only stylesheet rule removes the border, radius and horizontal padding; the embed still asks the toolbar for `hideDatabaseTitle`, which withdraws the duplicate title row and chevron. Existing headerless options remain unchanged
- [x] T005 Widen the embed to the reading view's content width so no table column is clipped, using T002's answer. A fixed pixel width is not an acceptable fix (`src/views/embedded-database-renderer.ts`, `styles.css`) — the old ancestor walk and inline width release are removed and the linked-only stylesheet rule owns `width: 100%`, never a measured pixel. Measured: the embed's content box goes from 26px narrower than prose to exactly equal on both devices, and the table's overflow scrolls inside the embed instead of past it. Whether that also ends the reported clipping is T002's, since the constructed host never reproduces it
...
- [x] T011 Capture pair proving SC-001 and SC-002: a constructed embed scenario beside the standalone view of the same data, both device widths, both themes. Read the PNGs by hand — a clipped column is not something an assertion notices (`tools/screenshots/constructed-scenarios.mjs`, `screenshots/`). Four captures committed and read, plus a red control with the linked rule neutralised so the same check shows the failure it claims to fix
- [x] T012 Prove SC-003 by re-reading both files after a move: same database, same view, same options, and exactly one block across the two pages — the move test writes two vault-shaped pages, re-reads both through the adapter, parses the destination fence and checks the preserved options and one-block total
...
- [ ] T014 Recapture, read the changed PNGs, release the `styles.css` lane, and update `../roadmap.md` §4 row 42 with the measured result (`screenshots/`, `../roadmap.md`) — recapture, PNG review and lane release are done: 550 entries current, 5 content-changed captures read, lane released at `b5d5122d5b5b`. Row 42 is the operator's report row and stays for them
- [x] T015 The stylesheet leg, once T002 has an answer and the lane is free: drop the card border, the corner radius and the horizontal padding for `.note-database-embed-linked` (`styles.css:15652`), and retire the rules left matching nothing — `.note-database-embed-codeblock-host` (`:15857`), whose class nothing adds any more, and the two header-toggle blocks (`:15879`, `:15908`) with the `toggleHeaderChrome` action and `renderHeaderChromeButton` that now have no reachable caller (`styles.css`, `src/views/toolbar-renderer.ts`, `src/views/embedded-database-renderer.ts`) — stylesheet ownership and the phase-held lane checker are green; release still waits on the required captures
- [x] T016 Decide whether the write capability ships behind a settings flag. `checklist.md` CHK-121 asks for one and ADR-001's rollback assumes it; a plugin ships as one bundle, so reverting the release may be the whole rollback. **Operator decision, 2026-09-05: no feature flag; reverting the release is the rollback** — recorded as ADR-003 (`decision-record.md`)
<!-- /ANCHOR:phase-3 -->
