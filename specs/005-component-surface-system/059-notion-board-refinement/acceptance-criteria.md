...
_memory:
  continuity:
    ...
    recent_action: "Authored the acceptance criteria against the rebased tree"
    next_safe_action: "Run T002 and pin each observed red into its Today cell"
    blockers:
      - "AC-002 through AC-005 and AC-008 are gated on the operator answering ADR-004, ADR-010 and ADR-011"
    key_files:
...
| AC-005 | REQ-005 | **Given** a default view config, **When** the board renders a group with no records, **Then** the column is still shown and its shared empty card still builds | **Observed red in the reference, not in our tree:** Notion ships "Hide empty groups" **on** in all three management captures (`30ba5533`, `e9698e1b`, `2ef31bd5`); adopting that default would delete a state eight of our own captures hold. Green is `boardHideEmptyGroups` defaulting **false**, **0** columns suppressed for emptiness under a default config, the empty card still built at `board-renderer.ts:324-327`, and the eight `board-empty-column-*` / `constructed-board-empty-column-*` capture hashes unchanged | Unmet | - |
...
| AC-010 | OPERATOR | **Given** a release carrying the Groups panel, **When** the operator opens it on iOS and on desktop, **Then** they report it as an improvement rather than a fourth place to hunt for a setting | The operator's own words, after they have answered ADR-004, ADR-010 and ADR-011. Nothing in this repository can close this row, and an agent never ticks it | Unmet | - |
...
touched. Five of them (AC-002 through AC-005, AC-009) cannot start before the operator answers
ADR-004, ADR-010 and ADR-011, and AC-010 is the operator's own device read. The closure sentence
