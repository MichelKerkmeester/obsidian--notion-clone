...
_memory:
  continuity:
    ...
    recent_action: "Closed AC-001/002/003/005/006/007/008 harness gaps and six recorded defects"
    ...
    answered_questions:
      - "The dark-theme frozen-column shadow lightens instead of darkens (rgba(255,255,255,0.35)), measured 3.21:1 dark / 1.83:1 light against --db-surface-canvas"
...
**What this second closure pass fixed.** All four defects named above are now fixed at the source rather than recorded: the shadow is themed per surface and measured at 1.83:1 light / 3.21:1 dark against `--db-surface-canvas`; `.is-phone` now turns the freeze off explicitly instead of relying on a scope that never matched; the grouped table's scroll listener binds to `.db-grouped-table` (the element that owns the overflow) instead of the outer container; and the 560px chip-measurer cap carries a two-sided unit assertion. Every harness or unit gap this document's own cells named as missing — AC-001's frozen-`left`/unfreeze read, AC-002's cap, AC-003's date-range unit coverage, AC-005's resize-handle hover read, AC-006's vertical-lines gate, AC-007's peek-and-cell empty-value pair, and AC-008's add-row button text — now has one, cited in the row it closes. Two defects outside this document's own rows were also closed while here: the dead `cell.auditReadonly` i18n key (three locales) was removed, and `created-time`/`last-edited-time` cells now read through `formatDateTimeValueDisplay` rather than the date-only formatter, since the vault's own `ctime`/`mtime` carry a time component the plain-date display was silently dropping.
