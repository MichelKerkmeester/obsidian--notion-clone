...
_memory:
  continuity:
    ...
    last_updated_at: "2026-09-04T19:15:00Z"
    last_updated_by: "milestone-label-landing"
    recent_action: "Rebased, reconciled and landed the milestone-label anti-collision fix (T052) onto main"
    next_safe_action: "Await the operator's device confirmation (roadmap.md row 39) and vault compare (row 38) on main"
    blockers:
    ...
    answered_questions:
      ...
      - "Milestone-label overpaint keep-vs-revert (roadmap.md row 39): operator decided 'Reinstate local fix' (T052)"
---
...
| **2026-09-04, operator decision (resolves `roadmap.md` §4 row 39, `tasks.md` T050/T052):** "Reinstate local fix" — a local anti-collision fix is reinstated on the reference gantt's default render path, raising a milestone label that would overpaint a month/year band label. This is REQ-007's one deliberate divergence from the reference gantt (`acceptance-criteria.md` AC-007 divergence list, REQ-007-DIV-1) | The row-39 open question named two options with no default; the operator chose local polish over strict reference-faithfulness for this one case. Implemented by porting the estimate-then-move shape `resolveTimelineMilestoneLabelPlacement` already used for the pre-port renderer's own (different) milestone-vs-next-bar collision, not by copying anything from the reference — the reference source itself has no anti-collision here to copy |
<!-- /ANCHOR:decisions -->
...
| P2-G resolution (2026-09-04, T052, operator decision "Reinstate local fix", `roadmap.md` §4 row 39) | Red first: `calendar-timeline-gantt.test.ts` new case — a milestone due on a visible month's first day (band label 16px from the milestone's own centre) — failed with `expected '14' not to be '14'` against the unmodified renderer (the milestone label's `y` attribute and class never varied). Green after `renderGanttMonthBands`/`renderGanttYearBands` (`calendar-timeline-renderer.ts:1250-1301`) return each band label's own `x`/text, threaded through the five per-scale header dispatchers (`:1092-1244`) to `renderGanttMilestoneLabels` (`:1519-1570`), which estimates both labels' spans by character count (mirroring `resolveTimelineMilestoneLabelPlacement`/`getTimelineMilestoneLabelWidthUnits`'s shape for a different collision pair) and moves a colliding label from `y=14` to `y=8`, adding `.pm-gantt-milestone-label--raised` (new `styles.css` rule via the CSS lane, a background-color text stroke). The guide line's start needed no change (always begins at the header's bottom edge, outside the 0-24 band strip). `temporal.mjs`'s hand fixture was checked against the same heuristic across all five scales/two devices — no collision in any, so unchanged by the recapture. 10 real content-changed captures (`constructed-timeline{,-day,-subtask}` × desktop/mobile × dark/light where applicable) opened and read: "row-1"'s milestone label (bench date on a September month boundary) now stacks cleanly above "SEP 26" instead of overpainting it, confirmed against the `HEAD`-committed PNG before/after. `pixelHash` read all ten as unchanged (misses the ~6px baseline move); `layoutHash` and the direct visual read both confirmed the change, so all ten are named in the CSS lane release regardless. 6 further files the recapture moved (`constructed-owned-menu-mobile-dark`, `field-icon-picker-desktop-{dark,light}`, `constructed-record-detail-desktop-dark`, `constructed-sort-panel-calendar-mobile-light`, `board-view-desktop-dark`) were pixelHash-identical to `HEAD` (documented drift) and restored to `HEAD` bytes. `npx tsc --noEmit` exit 0; `npx vitest run` 1038/1038 (101 files, was 1037); `npm run lint` 172 (unchanged); `npm run lint:tools` clean; `scan-comments` PASS. Editing `styles.css` staled 8 `tools/live/*.json` evidence artefacts (`cascade-audit`, `checkbox-appearance`, `checkbox-inventory`, `design-conformance`, `engine-parity`, `surface-census`, `token-census`, `view-census`); each re-measured by re-running its own tool, no number hand-edited. `npm run gate`: first run 24 green/1 red (`evidence`, the 8 stale artefacts above); second run 25/25 green |
| Landing reconciliation (`worktrees/038-milestone-labels` onto `origin/main` at `c6b5f113`, a docs-only commit) | `git rebase origin/main` replayed clean, no conflicts (main added no code). `npx tsc --noEmit` exit 0; `npx vitest run` 1038/1038 (101 files); `npm run lint` 172 (unchanged baseline); `npm run lint:tools` clean; `scan-comments` PASS. Full recapture (`npm run screenshots`, 528 entries) run twice: the first run overlapped `worktrees/036-sheet-freeze`'s own real `gate`/capture processes (a `pgrep -f` pattern check false-negatived on other agents' wait-loop command lines rather than the real process, per the orchestrator's mid-task correction); re-run uncontended after those processes cleared, same result both times. This branch's 10 timeline captures reproduced byte-identical to `HEAD` (0 diff) — the raised label was already correct in the committed captures; `constructed-timeline-desktop-dark.png` and `constructed-timeline-day-desktop-dark.png` opened and read directly, confirming "row-1"'s label stacked above "SEP 26" at both the week and day scale. 8 further captures (`constructed-owned-menu-mobile-dark`, `field-icon-picker-desktop-{dark,light}`, `constructed-record-detail-desktop-dark`, `constructed-sort-panel-calendar-mobile-light`, `board-mobile-desktop-dark`, `board-view-desktop-dark`, `board-view-mobile-light`) moved on both recapture runs regardless of contention, unrelated to any file this branch or the rebase touched; `pixelHash` (`tools/screenshots/pixel-hash.mjs`) confirmed 6 as encoder-only noise (identical decoded pixels) and reproduced the same non-matching hash for the two `field-icon-picker` captures on both runs — a small, deterministic-within-worktree glyph-edge difference (~0.8% of pixels, one icon row) traced by direct raw-pixel diff to an untouched file (`icon-picker-popover.ts`/`record-icon-renderer.ts`), consistent with this lane's already-documented drift for the same two captures; all 8 restored to `HEAD` bytes, manifest untouched. `npm run screenshots:verify`: 0 stale. 8 `tools/live/*.json` evidence artefacts (`capture-device-parity`, `list-window`, `renderer-coverage`, `replay`, `sheet-rebuild`, `sheet-teardown`, `touch-targets`, `unstyled-links`) recorded a `styles.css`/`calendar-timeline-renderer.ts` fingerprint that predated this branch's own CSS/renderer edits — not part of the original T052 leg's 8-artefact refresh (a disjoint set: `cascade-audit`, `checkbox-appearance`, etc.) — re-measured fresh via `npm run gate`, no claimed value changed, only the recorded input fingerprints. `npm run gate`: 25/25 green |
<!-- /ANCHOR:verification -->
