...
| AC-008 | REQ-008 | Given a database open on timeline (or calendar), When the user switches to a different view type, Then the outgoing view's DOM root and observers/listeners are gone from the container before the next view renders, on both the full-tab host and the embedded host | `tools/live/render-assertion-harness.ts`'s `runViewSwitchResidueCheck`, wired as a permanent `render-assertions.mjs` lane, reproducing the production teardown-then-render sequence on the shipped renderers in headless Chrome; `database-view.test.ts`'s three view-switch teardown assertions (`tasks.md` T054) | **Met.** Red first: **1 leftover `.pm-gantt-view` root** survived timeline -> table before the fix (`rendered-view-roots.ts`'s `VIEW_ROOT_CLASSES` named `obnotion-timeline`, the opt-in local-extensions root, but not `pm-gantt-view`, the default render's own root); calendar -> table already read **0**. Green after: **0** for both. The two `database-view.test.ts` positive assertions were confirmed failing (destroy called 0 times, not 1) against the pre-fix code. `npx tsc --noEmit`, `npx vitest run` (1644/1644), `npm run build`, `render-assertions.mjs`, `sheet-grammar.mjs`, `npm run gate` (26/26) all exit 0. Full evidence: `tasks.md` T054 | - |
...
**Amendment 2026-09-07 (AC-008 added, operator report, `../roadmap.md` §4 row 67):** switching
from timeline back to table left the gantt's root sitting over the table, because
`rendered-view-roots.ts`'s root-class list — and its inline copy in
`embedded-database-renderer.ts` — never named `pm-gantt-view`, the class REQ-007's own 1:1 default
render roots itself under. Fixed and verified in-repo (`tasks.md` T054); AC-008 is `Met`. This
does not change the packet's overall closure: AC-001 through AC-006 remain `Unmet` and AC-007's
operator half is still outstanding, per D3.
<!-- /ANCHOR:closure -->
