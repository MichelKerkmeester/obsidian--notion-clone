...
| REQ-008 | **Amendment 2026-09-07 (operator report, `../roadmap.md` §4 row 67).** Every host that mounts the timeline (`database-view.ts`, `embedded-database-renderer.ts`) removes the gantt's root (`.pm-gantt-view`, the default render's own class per REQ-007) and disconnects its observers/listeners before rendering a different view type into the same container — the reference's own class vocabulary this packet ported must be named wherever a host tears down the previous view, not only where it renders the current one. |
