...
| 53 | *"Also make it easier to set wrap or no wrapping table rows etc"* | `053-toolbar-and-view-controls` (view settings) | **Shipped, awaiting device read.** A `ViewConfig.wrapText` switch (table view settings, default off) plus a three-state `ColumnDef.wrap` override (Wrap / Clip / Follow view, the column menu's own submenu) resolve as `col.wrap ?? config.wrapText` in `cell-renderer.ts` — a column's own choice always wins. Recorded `053` ADR-004. Red-first proof: `render-assertions.mjs` gained a wrap-toggle pass on the same catalogue mount `005`'s row-rhythm check uses — clipped measures at the row floor (36px), forcing the view default on regrows the row past it (133px), and the negative control (wiring reverted) measured the clipped case flat at the floor instead, confirming the check can actually go red. `npm run gate` 26/26 green. No vault changes appearance on upgrade | Operator report, 2026-09-05 22:35. Names a per-view wrap/no-wrap toggle; the per-column half of the same ask belongs to `052`'s column menu, not this row |
