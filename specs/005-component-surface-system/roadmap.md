...
- `042-harness-fidelity-and-replay` — landed `7e9fd27`, not operator-relevant. Of the parent's three DONE-table rows this phase targeted: row 3 (chart and calendar week/day renderer coverage) is closed — all seven `DatabaseViewType` values now constructed, armed controls read 11 reds against their bounds. Row 5 (replay) gained 21 claims, three delegating to the sheet lanes, and stays open on six more open-row fix commits. Row 6 (harness-dependency audit) closed two of four dependencies — `runtime-vars.css`'s calendar formula and `theme.css`'s `.mod-cta` are now pinned to product values — and stays open on the remaining fixture reads and the capture manifest's compare. Exit signal: `goal.md`'s six completion criteria all ticked with their observed red-then-green pairs; rows 5 and 6 remain.
