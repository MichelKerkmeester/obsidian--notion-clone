...
- `042-harness-fidelity-and-replay` — landed `7e9fd27`, `5fa0b0c`, `bea1b1c`, `8759399`, not operator-relevant. Of the parent's three DONE-table rows this phase targeted, row 3 (chart and calendar week/day renderer coverage) closed — all seven `DatabaseViewType` values now constructed, armed controls read 11 reds against their bounds. Three lanes landed since the prior snapshot: (1) `replay.mjs` now carries 27 claims, up from 21 — `5fa0b0c` added the six open-row fix claims (`7e36671`, `535373a`, `3f143df`, `a251a43`, `fa58c7f`, `b29bf7f`), each pre-fix-audited against its landing commit's parent tree; (2) `check-lane.mjs`'s manifest compare moved from a raw byte diff to a jitter-tolerant `pixelHash`/`layoutHash` content compare (`bea1b1c`), so a non-deterministic re-encode no longer reads as a changed capture; (3) `touch-targets.mjs`/`unstyled-links.mjs` gained a constructed-renderer pass (`8759399`) that measures real `src/views` output alongside their existing fixture pass, finding four real touch-target classes no fixture could see (`touch-targets-constructed-baseline.json`). Row 5 (replay) narrows to one gap: `7ca6cc2` — `037`'s fourth open row, landed after the six claims above — carries no replay claim yet. Row 6 (harness-dependency audit) stays open on the fixture passes of five gate lanes (`touch-targets`, `unstyled-links`, `css-lane`, `screenshots-fresh`, `device-parity`); the constructed pass cross-validates two of the five but does not replace their fixture pass. Exit signal: `goal.md`'s six completion criteria all ticked with their observed red-then-green pairs; rows 5 and 6 remain, both narrowed to a bounded, named list.
- `043-constructed-capture` — opened today for `042`'s row-6 remainder, not operator-relevant. `capture.mjs` accepts only a hand-written `html()` scenario; this phase gives it a second, constructed scenario type reusing `042`'s `buildRenderAssertionBundle()`/`runRenderAssertions()` seam, a harness-owned readiness wait for the two views (`calendar-renderer.ts`, `calendar-timeline-renderer.ts`) that schedule a post-render `requestAnimationFrame` correction, and coverage for all 13 registered-view/scale combinations (list, table, board, gallery, calendar's three scales, timeline's five, chart) — two of which (chart, calendar's day scale) have no fixture at all today. A `declared-fixtures.mjs` registry names the eleven existing fixtures the constructed captures supersede and the thirteen that stay fixture-only; css-lane, screenshots-fresh and device-parity are rewired to read the constructed capture where one is declared. Nothing built yet. Exit signal: `goal.md`'s eleven completion criteria all ticked with their observed red-then-green pairs, and DONE row 6 ticked or left with the bounded fixture-only list this phase names.
