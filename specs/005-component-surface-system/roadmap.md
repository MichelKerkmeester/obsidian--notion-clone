...
- `042-harness-fidelity-and-replay` — opened 2026-09-03, not started. Closes the parent's three unticked/narrowed DONE-table rows from that day's audit: chart and calendar week/day renderer coverage (row 3), replay entries for report 29, reports 34-36 and phases `037`-`041` (row 5), and the row-6 harness-dependency audit plus the capture manifest's non-content-hash compare. Exit signal: `goal.md`'s six completion criteria all ticked with their observed red-then-green pairs.
