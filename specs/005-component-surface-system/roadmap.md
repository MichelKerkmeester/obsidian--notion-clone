...
|---|---|---|---|
| 1. Timeline and gantt | `037-timeline-gantt-port` | Their timeline and gantt surface, into our timeline renderer | Landed `0262386` + `55bff9b`, shipped **1.4.4**; open-row fixes closed three of four `fa58c7f`+`b29bf7f`, reconciled `65fb7dd`, shipped **1.4.9**; the day-scale row closed after that cut, leg d `7ca6cc2`, riding the next release (pending). Not operator-confirmed. **Reopened 2026-09-04 for a 1:1 gantt copy at the operator's request** — REQ-007 in `037`'s `spec.md`; open defect rows plus the new 1:1 criterion live in `037`'s `goal.md`. **In flight, unmerged:** the TypeScript leg landed on `worktrees/024-gantt-one-to-one` at `d30ea78` + `9bd044a`; the CSS leg is in progress. `git merge-base --is-ancestor` confirms the branch tip is not an ancestor of main's `dc67803`, so main is unaffected and nothing here is landed work |
| 2. Board | `038-board-kanban-port` | Their board surface, into our board renderer | Landed `b9e2321` + `a6fcd31`, shipped **1.4.5**; hover/drag/drop-target/empty-column row closed `7e36671`, riding **1.4.8** (pending). Not operator-confirmed. **Reopened 2026-09-04 for a 1:1 board copy at the operator's request** — REQ-007 in `038`'s `spec.md`; open defect rows plus the new 1:1 criterion live in `038`'s `goal.md`. **In flight, unmerged:** the TypeScript leg landed on `worktrees/023-board-one-to-one` at `1c5f465`, where a drag defect was found and fixed; the CSS leg is under verification. `git merge-base --is-ancestor` confirms the branch tip is not an ancestor of main's `dc67803`, so main is unaffected and nothing here is landed work |
...
`manifest.json`/`package.json`/`versions.json` on `main` read **0.0.16** today (`46a8525`; they read
0.0.15 at `6f81eb8`, when the renumbering landed), and the iCloud install still reads **0.0.15** with
a backup of the pre-rename 1.4.10 manifest beside it. **0.0.16 is cut** (`46a8525`), carrying `038`'s board one-to-one copy only — the board leg
was merged and reconciled onto main in `854c748`, while `037`'s gantt leg is still landing from
`worktrees/024-gantt-one-to-one` (`7617f85`) and will ship in a later cut. The cadence continues at
...
touch-floor fix `a3781ae`), and **0.0.16**, the current cut — `038`'s board one-to-one copy, kanban
structure and stylesheet, merged and reconciled in `854c748`, cut in `46a8525`; the `0.0.16` tag is
present on `origin` at that commit, verified with `git ls-remote --tags`. None of 0.0.7 through 0.0.16 is operator-confirmed yet.
