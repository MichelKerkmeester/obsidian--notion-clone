...
| `057-calendar-anytype-parity` | **0%** — 0/10 | **Opened 2026-09-05 ~22:45, not started. T001 owed, and one ruling owed to the operator** | The calendar half of the same ruling — *"Same for calendar etc."* Level 3 (`recommend-level.sh --loc 800 --files 10 --architectural` **63/100** confidence 92% — Level 2 on the script's thresholds, raised the same way; phase score 10/50, a standard child). Nine elements against **44** capture files: 20 set captures and 24 menu files (**6** menus, not the 4 the opening brief said). **Two findings the authoring pass produced, both recorded because they change what the packet can promise.** First, the calendar carries **0** `pm-*` classes — `039-calendar-parity-port` ported behaviour, not markup — so `056`'s headline threshold has no analogue here and writing one would have produced a criterion that reads green on an untouched tree (ADR-003 records the absence deliberately). Second, **iOS Anytype ships no calendar layout**: `screenshots/anytype/mobile/` holds no calendar capture of any kind, so the phone half has no reference and every phone value is labelled *"design inferred from desktop"* — AC-007 counts the unlabelled ones and requires zero. **ADR-002 is Proposed and operator-owned**: ours has three scales (`calendar-renderer.ts:82`) against a product shipping one calendar layout, and deleting two is not this packet's to infer |
