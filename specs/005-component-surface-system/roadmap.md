...
| Decision | What it settles | Where it binds |
|---|---|---|
| **Drop the confirm for a single delete and make it reversible instead.** Operator, verbatim: *"No confirm for single delete, Undo toast"* | Closes `051` ADR-007's **E4**, the one hold that ADR flagged rather than took. The confirm stays for **bulk delete and anything not undoable**; a single delete raises an undo toast and no dialog. This is neither reading E4 offered: it reaches Anytype's no-confirm pattern without routing deletions through a Bin, by making the single delete reversible at the interaction layer rather than at the storage layer. **Not implemented by the leg that recorded it** — `051` exported the confirm primitive and closed the componentization census; removing the single-delete confirm and building the toast is a separate leg, and the primitive is what the surviving bulk and not-undoable confirms keep using | `051-modal-and-sheet-componentization/decision-record.md` ADR-007 E4 (**Closed**); §6A here; the implementing leg, which is not `051` |
...
| Decision | What it settles | Where it binds |
|---|---|---|
| **No confirm for single delete; Undo toast instead. Confirm stays for bulk delete and anything not undoable.** Operator, verbatim: *"No confirm for single delete, Undo toast"* | Closes `051` ADR-007's flagged hold, E4 — Anytype raises no destructive confirm anywhere because deletion is reversible into a Bin, which we do not have, so the confirm's original ground (data loss, not accessibility) stood unresolved rather than authorised. The ruling is a third path, neither of ADR-007's two posed readings: not "keep the confirm" and not "build a `TrashManagerModal`". Read from source rather than assumed: `row-menu.ts:166-176`'s single-row delete already calls `confirmWithModal` **and** `deleteRow` (`database-view.ts:8354-8371`) already shows a `showToast` with an Undo action — both exist today, so the fix removes the confirm call at one site, it does not build the toast | `051-modal-and-sheet-componentization/decision-record.md` ADR-007 (E4 closed, amended in place); `055-states-feedback-and-motion/goal.md` (new completion criterion), `acceptance-criteria.md` AC-013, `tasks.md` T019 — the owner of the `deleteRow` call-site change. `database-view.ts:4962`'s bulk `deleteSelectedRows` is unaffected by name, per the ruling's own carve-out |

### Two more, taken 2026-09-06 (~08:10, ~08:15)

| Decision | What it settles | Where it binds |
|---|---|---|
| **Hide the table footer at zero rows, 44px otherwise.** Operator, verbatim: *"Hide the footer at zero rows, 44px otherwise"* | Answers the batch question an empty phone table's footer raised: 173 `+ Calculate` triggers at 26px, under the 44px touch floor, with no `rows.length === 0` branch in `table-footer-renderer.ts` today | `053-toolbar-and-view-controls/goal.md` (new completion criterion), `acceptance-criteria.md` AC-114, `tasks.md` T015 |
...
### Two more, taken 2026-09-06, on the board's residual colours

| Decision | What it settles | Where it binds |
|---|---|---|
| **R6 — Anytype tint fill.** Operator: *"Anytype tint fill"* | Reaffirms `056` ADR-004 E1 as written, closing a mismatch a fresh read of the landed board found (`tasks.md` T012): the shipped header chip is unfilled with only the text darkened, which is E1's *declined* light-theme treatment, not its stated replacement. Fill the chip with the option's own tint, keep the darkened text, and measure contrast **per colour** — E1's 6.11:1 amber sample was one hue, not a guarantee for all seven | `056-board-anytype-parity/decision-record.md` ADR-004 addendum; `tasks.md` T012's R6 row |
| **R7 — Neutral, match Anytype.** Operator: *"Neutral, match Anytype"*, grey pair `#888888`/`#A8A8A8` | Not an E1 variant — a defect ADR-004 never took a position on. The shipped grey option chip is red-tinted (`#7E5D5D` light / `#BAABAB` dark, hue 0) against Anytype's own neutral grey. Adopts Anytype's values directly: tint `#E3E3E3`, text `#888888` light / `#A8A8A8` dark — no accessibility ground needed since neither shipped nor adopted value is declined on contrast | `056-board-anytype-parity/decision-record.md` ADR-004 addendum; `tasks.md` T012's R7 row |
