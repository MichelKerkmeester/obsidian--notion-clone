...
| **The simulator route is how mobile screens get captured** | Anytype's mobile clients were never installed for the capture sweep, so the twenty mobile images in `screenshots/anytype/` are **App Store and Google Play marketing creative** — good evidence of intent, weak evidence of pixels, and `design-trueup.md` REQ-012 refuses to take a number from them. The route out is the iOS simulator, not a second scrape of the stores | `074-anytype-ios-sim`'s worktree; `050` item 13's phone design, which is code-derived until the simulator produces a screen; and `053`'s T24 phone rung |
