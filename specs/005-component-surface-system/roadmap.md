...
fully landed on main (`dcff742e`, `npm run gate` 26/26) — the settings and column-width sheet legs
shipped in 0.0.21 and its remaining legs (grammar contract, shared header, add-view sheet, record
peek, gantt's owned menu, suggest modals) reconciled after `worktrees/043-sheet-alignment-2`
rebased onto the board card properties landing; `045` shipped on main (`ff1dacec`); `046` has no
leg yet and is blocked on the operator's ADR-001 ruling.

**Nothing named above is in flight any longer.** `worktrees/043-sheet-alignment-2`
(`044`'s remaining legs) landed as `dcff742e`; `worktrees/044-list-hide-migrate`
(`006-list-view-deprecation`'s hide-and-migrate leg) and `worktrees/042-screenshots-folders`, the
capture-folder split, have both landed (`7d95a882`+`aa049b45`, reconciled `933308a5`) — see §1's
Seventh reconciliation note. `worktrees/045-board-card-properties` has shipped on main (`ff1dacec`).
`worktrees/046-board-line-height` (the board's line-height gaps from the fresh in-repo
side-by-side, §4A row 37/38 continuity) has also landed (`74a26419`, reconciled `4df2720c`,
`b7dc7cf5`).
...
the vault, four declared losses); child `006-hide-and-migrate` is implemented (`8152cf4f`, a devin
pass that ran out of daily quota mid-way, finished by Grok 4.6 via cli-cursor) and is under
verification on `worktrees/044-list-hide-migrate`; children `007` and `008` are next. Its roadmap
is `../006-list-view-deprecation/roadmap.md`.
...
it is a superseded pre-release build, not the shipped one. The cadence continues at **0.0.22**
onward — always `0.0.N`, never a second `.N.N`.
...
close and inset fix, and `043`'s constructed fixture families, cut in `ccc946c3`; and **0.0.21**,
the current cut — `031`'s WebKit sheet second-bug fix, `044`'s settings and column-width sheet
chrome fixes, `037`'s milestone-label reinstatement, and the Project Manager reference captures,
cut in `5af7eef7`. The `0.0.16` through `0.0.21` tags are all present on `origin`. None of 0.0.7
through 0.0.21 is operator-confirmed yet.
