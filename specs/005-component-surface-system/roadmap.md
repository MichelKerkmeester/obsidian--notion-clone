...
`worktrees/086-land-phases-051-055`. **None has run its T001.** Their state cell reads *open,
drafted 2026-09-05, awaiting T001 true-up* for exactly that reason: the documents are reviewed, the
designs are not yet checked against the captures each row names.
...
| **Every Anytype dropdown and mobile sheet is screenshotted** | The first capture pass reached no mouse-driven surface at all, and the second reached the panels but no hover state and no drag. The instruction closes the remaining vocabulary: every dropdown and every mobile sheet, photographed, so the menu and picker phases stop designing from source reads | `079-anytype-menus`' worktree; `052`'s `anytype-menu-grammar.md` §4's "what still has no capture" list, and `054`'s §5B rows the true-up did not reach |
