...
| 57 | *"btw this dropdown on desktop is horrible … should probably become a sheet, on desktop at least, and get dedicated button."* | `051-modal-and-sheet-componentization` (side-sheet shape) + `053-toolbar-and-view-controls` (gear button) | **NEW 2026-09-06 ~08:15, desktop screenshots supplied with the report (the operator's own captures, not committed to this repository).** Ruling: **"Right side sheet"** — full-height panel docked right, database stays visible and interactive to its left, independent scroll. `051` ADR-008 names the shape (new `side sheet` role, distinct from every existing `design-system.md` §3 role) and adds AC-013/T021-T023; `053` adds the toolbar-rail gear button before the `···` overflow trigger, AC-113/T014. No code changed yet | Operator report, desktop, 2026-09-06 ~08:15 |
...
### 5.A Every phase 000-057, and where each one stands
...
### One more, taken 2026-09-06 (~08:15), on the desktop Settings surface

| Decision | What it settles | Where it binds |
|---|---|---|
| **The desktop database Settings surface becomes a right side sheet; the toolbar gets a dedicated gear button.** Operator, verbatim: *"Right side sheet"*, and *"Gear icon in the toolbar rail, before ···"* | A full-height panel docked to the right edge, the database staying visible and interactive to its left, scrolling independently — a shape none of `design-system.md` §3's five existing roles produce, so it is a new role rather than a widened `panel`. The gear button opens it directly from the toolbar rail, before the `···` overflow trigger, rather than one level deep inside it | `051-modal-and-sheet-componentization/decision-record.md` ADR-008 (new role, `side sheet`); `051`'s AC-013, T021-T023 (the shape); `053-toolbar-and-view-controls/goal.md` (new completion criterion), `acceptance-criteria.md` AC-113, `tasks.md` T014 (the button) |
