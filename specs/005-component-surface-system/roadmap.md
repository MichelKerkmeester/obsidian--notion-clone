...
| `057-calendar-anytype-parity` | **40%** — 4/10 | **T004-T007 landed 2026-09-06 and were read back against the references; AC-002 reopened on two measured misses. T008-T016 open** | The calendar half of *"Same for calendar etc."* Level 3 (`recommend-level.sh --loc 800 --files 10 --architectural` **63/100** confidence 92%; phase score 10/50, a standard child). **T001** trued nine elements against 44 capture files and corrected the menu count to **5 distinct surfaces in 24 files** — the day and item captures are byte-identical. **ADR-002 is Accepted**, operator 2026-09-05 ~23:20: *"Keep week and day, styled to the month grid."* **T005-T007 landed the month retarget** and the landed captures measure to the recorded values at DPR 2: 137px row pitch, `#EBEBEB`/`#292929` rules on eight column positions, `#F7F7F7`/`#1E1E1E` weekend columns, a 52x48 device `#216DFA` today disc, flat 20px-pitch chips with no fill, bar or radius, a top-right 16px day number, month and year title selects, a today-scroll, a right-click day/chip menu and the `Show icon` toggle at its measured colour. **The gantt did not move**, by measurement and not by assertion: a zero-line diff on `calendar-timeline-renderer.ts` and both guard tests, `pm-gantt-*` still **119**, and all eight `reference-gantt-*` captures re-rendered MD5-identical. **AC-002 was claimed Met by the implementing pass and is reopened by the verification pass**: the 16px grid inset lands on the left edge only, so the month grid overruns its container by 16px on the right, and the weekday header row no longer shares the day grid's column pitch (377.3 device px against 386.2), drifting each label 15.5px to 43.5px out of its column. Five further residuals — the week and day scales never took the `#EBEBEB` rules or the `#216DFA` marker the ruling names, their headers keep the one-string title, the day number sits 6px low, the weekday labels are three-letter Sunday-start against a two-letter Monday-start reference, and the empty unscheduled drawer still occupies 85px above a surface the reference does not have — are carried as **T015**, with **T016** for the chip icon no capture currently renders. **One question is back with the operator** and is not decided in-repo: whether *"styled to the month grid"* also strips the week/day timed blocks' per-event colour, which ADR-004 does not authorise keeping and ADR-002's geometry argument does not reach. `npm run gate`: **26 green**; `npx tsc --noEmit`, `npm run build` and `npx vitest run` (132 files / **1418** tests) all exit 0 |
