...
| 58 | *"every dropdown on desktop should support search — you click on it, the dropdown opens and input becomes input active state allowing you to also search for the item."* Same report also shows the filter/sort Operator dropdown mis-anchored (popover x 123-489 under a trigger at x 290-480) | `052-dropdown-menu-and-picker-componentization` | **NEW 2026-09-06 ~08:15, desktop screenshots supplied with the report (the operator's own captures, not committed to this repository).** Combobox behavior for every desktop dropdown — the trigger itself becomes an active text input on open, not `dropdown-field.ts`'s existing gated (`>8` options) separate search field. `052` adds AC-011/T016 for the combobox and AC-012/T017 for the Operator-dropdown anchoring defect (P1, a left-edge miscalculation, not a width one). No code changed yet | Operator report, desktop, 2026-09-06 ~08:15 |

*2026-09-06 ~08:15: row 56 added from the operator's desktop filter-panel screenshot
(`.operator-dropdown-report.png`), owner `052-dropdown-menu-and-picker-componentization`. Not yet
phased into that packet's own task list — landed as a direct fix on this leg's branch, `tasks.md`
unchanged, deliberately — and **not ticked**: the row stays IN FLIGHT until the operator's own look
confirms it, per D3.*

| 56 | *"every dropdown on desktop should support search. So you click on it, the dropdown opens and input becomes input active state allowing you to also search for the item"*, with a screenshot of the filter panel's operator dropdown open and visibly mis-anchored | `052-dropdown-menu-and-picker-componentization` | **IN FLIGHT, this leg. Fixed and unit-tested; awaiting the operator's own look.** | The screenshot's own pixels name a different defect than the words: the operator dropdown (`equals`/`does not equal`/`contains`/`is empty`/`is not empty`, 5 rows) opened at x 123-489 under a trigger at x 290-480 — right-aligned to the trigger's right edge (`anchor.right - width`, `dropdown-field.ts`'s call into `positionToolbarPopover` carried no `align`, so it inherited the shared function's `"right"` default), not missing search. Read against Anytype's own captured condition menus (`decision-record.md` ADR-006): every condition/operator picture in the reference product ships with **no** search field, up to 6 items (`anytype-menu-set-filter-checkbox-condition-*` at 2, `-select-condition-*`/`-object-condition-*` at 5, `-email-condition-*` at 6), while every property/value picker ships one from single digits through 30+. `dropdown-field.ts`'s existing `> 8` search-count gate already sits between those two data points and is **kept, not changed** (ADR-006). **Fix:** `positionToolbarPopover(panel, anchor, { …, align: "left" })` in `dropdown-field.ts` — the shared primitive every desktop dropdown consumer already goes through (census: zero hand-built `<select>`/dropdown triggers anywhere in `src/views/*.ts`, confirmed by `grep`), so the fix and the left-alignment reach every consumer without a per-call-site change. Also landed in the same primitive: `role="combobox"`/`aria-expanded`/`aria-activedescendant` on the search input (tracking the highlighted row's id), and `Tab` committing the highlighted row in a `searchable` popover instead of leaving it open uncommitted. **Red first:** `src/views/dropdown-field.test.ts`, six cases against a mock DOM — reverting the source (stash, not edit) reproduces 3 of 6 red (anchoring measured `200px` against the fixed source's `290px`; the a11y attributes absent; `Tab` never calling `onChange`), restoring returns 6/6 green | Operator report, desktop, 2026-09-06 ~08:15, `.operator-dropdown-report.png` (the filter panel's operator dropdown, open, mis-anchored) |
