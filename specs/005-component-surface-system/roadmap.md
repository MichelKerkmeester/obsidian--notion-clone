...
### 5.A Every phase 000-047, and where each one stands
...
| `046-linked-views-notion-parity` | **0%** — 0/7 | Opened 2026-09-04, Level 3; ADR-001 Accepted 2026-09-05, implementation pass running | Linked views to Notion parity, from report 42. ADR-001 (may an embed write) was decided by the operator 2026-09-05 ~05:30 CEST, verbatim: "Allow db writing from linked views" — full parity, an embedded view writes to its source database exactly as the standalone view does (cells, rows, status/board moves, view config), undo through the same history stack, read-only only when the source is missing or unresolved. Implementation pass (T006) running on `worktrees/054-linked-views` (external lane: devin first, Grok fallback). 0 of 7. |
...
the line-height fix landed underneath it; `045` shipped on main (`56a34199`, shipped 0.0.22); `046`
has no leg yet and is blocked on the operator's ADR-001 ruling.
...
harness` is **in flight** on `worktrees/047-list-remove-renderer` (mid interactive rebase, conflicts
open — see `../005-component-surface-system/handover.md` §2) now that `006`'s "shipped, not merely
merged" precondition is met; child `008-docs-and-release` stays blocked on `007`. Its roadmap is
`../006-list-view-deprecation/roadmap.md`.
...
**0.0.22** too. The cadence continues at **0.0.23** onward — always `0.0.N`, never a second `.N.N`.
...
`7b976e28`. The `0.0.16` through `0.0.22` tags are all present on `origin`. None of 0.0.7
through 0.0.22 is operator-confirmed yet.
...
Four decisions were taken on 2026-08-30, and **seven more on 2026-09-05** — those are in their own
table further down, not folded into this one. Each closes a question, and each is recorded here
...
| **Landed worktrees 003-055 are removed; their branches are kept** | The `.worktrees/` tree had grown past fifty entries, most of them landed. Removing a worktree is not removing its branch, and the branches stay as history. Separately, the stale `cli-external-orchestration` hub is being re-minted in `Code_Environment/Public` | Workspace hygiene, not this program's code. Recorded here because a later agent reading `§5.A`'s worktree citations will find the directories gone and the branches present |
