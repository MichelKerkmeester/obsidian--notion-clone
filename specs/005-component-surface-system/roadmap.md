...
- `043-constructed-capture` — opened 2026-09-04 for `042`'s row-6 remainder, not operator-relevant. `capture.mjs` accepts only a hand-written `html()` scenario; this phase gives it a second, constructed scenario type reusing `042`'s `buildRenderAssertionBundle()`/`runRenderAssertions()` seam, a harness-owned readiness wait for the two views (`calendar-renderer.ts`, `calendar-timeline-renderer.ts`) that schedule a post-render `requestAnimationFrame` correction, and coverage for all 13 registered-view/scale combinations (list, table, board, gallery, calendar's three scales, timeline's five, chart) — two of which (chart, calendar's day scale) have no fixture at all today. A `declared-fixtures.mjs` registry names the eleven existing fixtures the constructed captures supersede and the thirteen that stay fixture-only; css-lane, screenshots-fresh and device-parity are rewired to read the constructed capture where one is declared. Devin's initial pass (leg a) is running in `.worktrees/017-constructed-capture`; nothing landed on main yet. Exit signal: `goal.md`'s eleven completion criteria all ticked with their observed red-then-green pairs, and DONE row 6 ticked or left with the bounded fixture-only list this phase names.
