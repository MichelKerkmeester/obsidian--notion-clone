...
**Thirty-nine reports** (1-16, 18-43), thirty-nine resolved to a named phase; rows 40-43 were added 2026-09-04 from the operator's evening pass — three phone-sheet asks owned by `044-phone-sheet-alignment` and the linked-views ask owned by `046-linked-views-notion-parity`; rows 34-36 shipped a
...
### 5.A Every phase 000-046, and where each one stands
...
| Phase | Derived | State | Where it stands, 2026-09-04 |
|---|---|---|---|
...
| `030-gallery-view-deprecation` | **67%** — 4/6 | Withdrawn, not removed | Migration and importer change carried. The gallery is still renderable by design — that withdraw-then-migrate pattern is `006-list-view-deprecation`'s precedent. |
| `031-sheet-lifecycle-ownership` | **70%** — 7/10 | Reopened by reports 34-36 | The `getPanel()` resolver shipped 0.0.8 onward. The entrance fix landed on main (`c96467c9`, reconciled `4c6b2c78`, recorded `ec24f9a`). **A second bug is in flight on `branches/001-sheet-webkit`: a toolbar rebuild behind an open sheet drops the sheet.** Ships in 0.0.21. |
...
| `037-timeline-gantt-port` | **50%** — 10/20 | Landed and shipped; reopened, then closed in-repo | Gantt 1:1 copy through 0.0.17/0.0.19/0.0.20; AC-007's in-repo half MET at `30c4b746` (60/60 `pm-gantt-*` classes, zero divergence). **Open: AC-007's operator half (row 38) and the overpaint ruling (row 39, decided and landed `1358927`, ships 0.0.21).** Its reference captures are the `037` worktree leg. |
| `038-board-kanban-port` | **30%** — 3/10 | Landed and shipped; reopened, then closed in-repo | Board 1:1 copy through 0.0.16/0.0.18/0.0.19/0.0.20; T12's in-repo half MET at `c563f08` (fourteen carried-forward elements matched to the pixel). **Open: T12's operator half (row 37).** |
...
| `043-constructed-capture` | **55%** — 6/11 | Landed (partial) | Constructed capture through T030; parent DONE row 6 ticked at `2242fa0`. **T031, the Project Manager reference captures, is in flight on `worktrees/037-reference-captures` — done there, not on main.** Five rows open here. |
| `044-phone-sheet-alignment` | **0%** — 0/7 | Opened 2026-09-04, Level 2, nothing landed | Phone sheet grammar for reports 40, 41 and 43. Three legs: `worktrees/039-column-width-sheet`, `worktrees/040-settings-sheet`, and the Add view sheet not started. 0 of 7. |
| `045-board-card-properties` | **0%** — 0/6 | Opened 2026-09-04, Level 2, not started | Per-view ordered property list for board cards, behind the existing `boardExtensionsEnabled` flag. 0 of 6. |
| `046-linked-views-notion-parity` | **0%** — 0/7 | Opened 2026-09-04, Level 3, not started | Linked views to Notion parity, from report 42. ADR-001 (may an embed write) is a precondition, not a discovery. 0 of 7. |
...
several new rows have landed since.
...
gathers all 103 of them in one place.

**Three phases are new tonight and have landed nothing**: `044`, `045`, `046`, opened 2026-09-04
from the operator's evening pass (reports 40-43). Their first legs are `worktrees/039-column-width-sheet`
and `worktrees/040-settings-sheet`, both `044`'s; `045` and `046` have no leg yet.

**Two other legs are in flight and are not on main**: `worktrees/037-reference-captures` (`043` T031,
the Project Manager reference captures under `screenshots/project-manager/`) and
`branches/001-sheet-webkit` (`031`'s second bug, the toolbar rebuild that drops the sheet). A third,
`worktrees/042-screenshots-folders`, carries the capture-folder split.
...
removal. Its roadmap is `../006-list-view-deprecation/roadmap.md`.
