...
### 5.A Every phase 000-048, and where each one stands
...
| `048-stacked-sheets` | **0%** — 0/7 | Opened 2026-09-05, Level 2, standard child | From the 0.0.23 device check, §4 rows 44-46. A sheet opened from inside another sheet must read as one stack: the child overlays the parent in place, the parent dims and scales back with its bounding box unchanged, one scrim sits **between** them, and the child carries `044`'s header-everywhere grammar — title, 44px close, 16px row inset, 16px title. The mechanism is an absence, not a bug: one `z-index` for every sheet (`styles.css:194`), one scrim behind all of them (`mobile-bottom-sheet.ts:478`), no parent read on child mount (`:274`), a keyboard inset each sheet computes for itself (`popover-position.ts:406`), and an `overlay-stack.ts:47` `parentId` that nothing reads. T001 is done — `048-stacked-sheets/stacked-surface-inventory.md` groups every stacked pair parent → child → opener kind → current → target with a `file:line` per row, and records that **all 8 `sheet-grammar` registry rows are first sheets, so every stacked child is unregistered**. `recommend-level.sh --loc 600 --files 13 --architectural` → 64/100 confidence 82%, phase score 10/50 below the 25 threshold. **D1 is operator-owned and open**: do Obsidian modals opened from a sheet present as sheets, or does the phone flow use a sheet instead? 0 of 7. |
