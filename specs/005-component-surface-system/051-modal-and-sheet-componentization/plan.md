...
1. **The shell** — `src/views/surface-shell.ts` (new), with `mobile-bottom-sheet.ts` kept as the
   engine underneath it and `db-modal.ts` delegating to it.
...
| Header | `createSheetHeader` (`:160`) at twelve independent sites | One call, from the shell |
...
- **The sub-page replaces in place; it does not stack.** This is `design-trueup.md` REQ-002's
  captured pattern and `048` REQ-002's stated preference, and the two agree: a parent that does not
  move is cheaper than a parent that dims and scales back.
...
|-------|-----|-------|----------------------|
| 1 | Evidence | `modal-surface-inventory.md`, `checklist.md` | Every Today cell filled from the tree |
...
- **Regression**: the `sheet-grammar` lane's twelve surfaces and thirty-one pairs after every leg —
  the pairs are the check that extraction did not move behaviour.
