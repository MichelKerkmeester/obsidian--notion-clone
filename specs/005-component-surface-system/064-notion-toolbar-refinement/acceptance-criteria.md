...
from `059`, `060`, `061`, `062`, `065` and `066`): `npx tsc --noEmit`, `npm run build` and
`npx vitest run` (149 files / **1600** tests) all exit **0**, and `npm run gate` exits **0** at
**26 green, 0 red for a declared reason** — including `toolbar-collapse` (observed red in T001,
green after T007) and `sheet-rebuild` (three scoped fixes recorded in `tasks.md` T011, the third
one made at the landing after the leg's own re-basing was proved unable to catch the defect it was
written for). Every criterion was mutation-tested one surface at a time (`tasks.md` T015); one
mutation survived, and it is named in T016(a) rather than absorbed. Full evidence per row lives in
`tasks.md`'s Phase 1-3 entries. AC-011 is the operator's device sitting
