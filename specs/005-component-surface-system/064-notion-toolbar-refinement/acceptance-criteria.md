...
_memory:
  continuity:
    ...
    recent_action: "Re-derived and mutation-tested every Met row at the landing"
...
|-------|-----|---------------------|--------------|--------|--------|
| AC-001 | REQ-001 | Given a database with more than one view, and the persistence-layer read (`database-view.ts:3445-3456`) taken first for whether a deleted view is recoverable by any existing undo path, When the view is deleted from the all-views hub row (`toolbar-renderer.ts:1180`) or from the tab context menu (`:1330`), Then **Branch A (unrecoverable)**: `051`'s confirm (`buildConfirmSheetBody`, `confirm-sheet.ts:46`), presented as the `061`/`067` centred card at one danger weight, is raised, its copy names the view, declining is a no-op and accepting deletes exactly one view; **Branch B (an undo path covers it)**: no confirm is raised, the view is deleted, and an Undo toast presents whose Undo action restores it — the same shape `051` ADR-007's E4 ruled for row deletion | **The read found Branch B applies**: `deleteView` already saves through `saveCurrentViewConfigInBackground` → `saveViewEntryConfig` → `recordConfigHistory`, which pushes an undoable `"config"` history entry the toolbar's own Undo action and Ctrl+Z already read — no existing-undo-path assumption to build, it is already wired. `src/views/database-view.test.ts`'s "DatabaseView deleteView" block: deleting from a 2-view database raises no confirm, records the entry labeled `undo.deleteViewConfig`, and `undoLastEdit()` restores both views; `src/views/toolbar-renderer.test.ts` pins that neither call site gained `buildConfirmSheetBody`/`confirmWithModal`. `npx vitest run` exit **0**. Driven through the real methods at the landing (`tasks.md` T015): the toast carries `notice.deletedView` with `{name}` interpolated and an action labelled `toolbar.undo`, and pressing that action restores both views and the deleted view's whole config. Measured and recorded there, and since fixed: the **selected tab was not restored** — `currentViewIndex` stayed on the neighbour the delete moved it to, because `recordConfigHistory` was handed a `viewId` resolved after the splice. `deleteView` (`database-view.ts:3468`) now captures the deleted view's own id (`removed.id`, already held by the splice's own destructure) and passes it through an explicit mutation override, so the history entry names the view undo is about to bring back rather than whichever view the delete happened to leave selected. `src/views/database-view.test.ts`'s new "restores the selected tab" case: deletes the active, last view (index 1 of 2), asserts `currentViewIndex` moves to its neighbour (0) on delete, then asserts it moves back to 1 — the restored view's original index — on undo; red-proved by reverting to the un-captured `viewId`, which left it on 0 after undo too. `npx vitest run` exit **0**. This row asks that the views come back, and they do, on the tab the operator was looking at | Met | - |
...
| AC-004 | REQ-002 | Given the filter panel with zero rules, When it opens, Then a searchable flat property list renders, built from the `toPropertyDropdownOption` vocabulary (`filter-panel-renderer.ts:497`), picking a property creates the first leaf through `createDefaultFilterRule` (`:90`) and `appendLeaf` (`:223`), and a `+ Add advanced filter` footer switches to the tree | `src/views/filter-panel-renderer.test.ts` asserts the entry tier's structure, the leaf-creation path and the distinct footer wording; the live `sheet-rebuild` lane mounts the real `FilterPanelRenderer` and exercises this exact branch end to end. `npx vitest run` and `node tools/live/sheet-rebuild.mjs` both exit **0**. The lane's "holds still" check was re-based twice — see `tasks.md` T011: the leg's viewport-floor formulation was proven not to catch the replayed entrance it was written for (836 measured on an 844px screen, because the sheet floats 8px off the bottom), and the landed check measures against the deeper of the two resting positions instead, red-proved by reintroducing the replay. The suite's own source-grep coverage was later proven not to see a real defect — mutating `addFirstLeaf(col.key)` to `addFirstLeaf(columns[0].key)` (every row filing its rule on the first property) left every assertion green, recorded as a gap in `tasks.md` T016(a). Since closed: the suite now mounts the real `FilterPanelRenderer` on a hand-built tree and drives a click on a non-first row, asserting the resulting rule's field matches that row's own property; red-proved against the exact named mutant (`state.filters[0].field` read `"file.name"` instead of the clicked `"colB"`), green on the real source | Met | - |
...
| AC-007 | REQ-004 | Given the toolbar swept 250→900px in 10px steps (`run-toolbar-collapse-sweep.mjs:39`), When the width narrows, Then the New label reads absent before the first width at which any cluster is hidden, zero-overflow holds at every width, the accessible name is unchanged, and the landed `:2571` drop order is not reordered | `tools/live/toolbar-collapse-sweep.ts`'s lane, extended with `newLabelVisible`/`newButtonAriaLabel` readings; `run-toolbar-collapse-sweep.mjs` now asserts all three directly (label-ahead-of-cluster, non-vacuous, aria-stable) rather than only printing switch points. `node tools/live/run-toolbar-collapse-sweep.mjs` exit **0**, run through the `toolbar-collapse` gate row. Measured at the landing on a 2px sweep: the label is absent while every cluster is still drawn across **456-486px**, and the rung buys the New cluster 32px (it drops at 456px where the pre-rung tree dropped it at 488px). Both assertions red-proved — disabling the rung fails on "still visible at 212 width(s) where a cluster is already hidden", restoring main's renderer fails on the vacuity control. The fixture's New cluster also hides again from 716px up; that tail is pre-existing, measured present with the rung disabled | Met | - |
| AC-008 | REQ-005 | Given at least one chip in the rail, When the rail renders, Then exactly one `db-active-control-add` control per rule group is present, wired to the existing `toggleFilterPanel` / `toggleSortPanel`, at the landed 28px chip pitch (`styles.css:1821`) and carrying its own accessible name; with zero chips, none | `grep -rn "db-active-control-add" src/ styles.css` now returns the definition and its three call sites, from **0**. `src/views/active-view-controls-renderer.test.ts` now mounts the real `ActiveViewControlsRenderer` on a hand-built tree (was a source grep, recorded as a gap in `tasks.md` T016(a)) and drives the add control's own click through to `actions.addFilter`/`actions.addSort`, asserting the anchor element passed, the per-group gating (a group with no `add*` action renders none), and one control per present group. `tools/live/render-assertion-harness.ts`'s `constructed-active-view-controls` scenario (was unphotographed, recorded as a gap in T016(b)) now supplies both actions in its bag, so the control finally draws in the one capture that mounts the real rail renderer; `constructed-active-view-controls-desktop-{dark,light}` recaptured and opened, `tools/lane/css-lane.json` released naming both. `npx vitest run` and `node tools/live/render-assertions.mjs` both exit **0** | Met | - |
...
its own is closed, **re-derived at the landing on the rebased tree** (twenty-seven further commits
...
re-basing was proved unable to catch the defect it was written for). Every criterion was
mutation-tested one surface at a time (`tasks.md` T015); the one surviving mutation is closed in
`tasks.md` T016. Full evidence per row lives in `tasks.md`'s Phase 1-3 entries and T016. AC-011 is
the operator's device sitting and rides `053` AC-111; nothing in this repository closes it.
<!-- /ANCHOR:closure -->
