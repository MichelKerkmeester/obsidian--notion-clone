...
_memory:
  continuity:
    ...
    recent_action: "Restated ac-002's verification cell to the adr-005 rejected form"
...
|-------|-----|---------------------|--------------|--------|--------|
| AC-001 | REQ-001 | **Given** the record sheet, the peek and the properties panel, **Then** each header is built by the P1 primitive, and the census reads **1** distinct header builder in the family (down from 4); the record sheet's desktop DOM matches its pre-phase geometry | Census lane counting header builders across the three surfaces. Negative control: bypass P1 in one consumer, require the count to go 2. **Today, re-measured 2026-09-05:** **4 builders** — `rg -n -e 'cls: "db-record-detail-header"' -e 'db-record-peek-header"' -e 'cls: "db-panel-header"' -e 'protected getSheetTitle' src/views/record-detail-panel.ts src/views/table-record-peek.ts src/views/column-manager-renderer.ts src/views/modals/db-modal.ts` → `record-detail-panel.ts:360`, `table-record-peek.ts:233`, `column-manager-renderer.ts:186`'s phone/desktop branch, `db-modal.ts:82`'s scrape (line numbers drifted a few lines from authoring; count and shape unchanged) | Unmet | - |
| AC-002 | REQ-002 | **Given** one column rendered through the record sheet, a board card, the peek and the properties panel, **Then** all four consumers build the row through P2, and the census reads **1** property-row vocabulary (down from 3); the row's anatomy is **label, then value, value left-aligned**, with **no format icon on the row** | SC-001's four-consumer lane row; badge colour, rating/progress/ring and conditional-format rendering asserted identical across consumers, and the desktop value's left edge asserted common across rows. **Proof stays the by-hand screenshot read** — labels at x 59, values right-aligned at 273, 311, 319, 343, 349 and 362, as measured on `constructed-record-detail-desktop-dark.png` in the true-up — performed by an image-capable in-runtime leaf at the leg's close (`decision-record.md` ADR-005, **Rejected 2026-09-05 ~18:20**; operator: "Keep the pixel reading"). Negative control: reintroduce the peek's private `renderProperty` body, require red. **Today, re-measured 2026-09-05:** **3 vocabularies** — `card-field-renderer.ts:102`, `table-record-peek.ts:334`, `column-manager-renderer.ts:267`+`board-card-properties-panel.ts:50` (both citations corrected from `:265`/`:48`, drifted since authoring); and the desktop record sheet **still right-aligns** its values (`styles.css:10161`, `text-align: right`, inherited from the board card — confirmed unchanged, `styles.css:10477` still carries only the mobile-bottom-sheet override, no desktop override added). **Pixel read owed:** this measurement-only leg could not open the PNG capture under its no-image-read constraint, so the labels-at-x59/values-at-273-362 figures were not re-observed here. As corroboration only (not as the proof), the leg re-measured through `getBoundingClientRect()`/`getComputedStyle()` on the mounted `panel-record-detail/file-view` scenario (a throwaway harness reading `record-detail-panel.ts`'s label/value elements): every row's `.db-record-detail-field-label` left edge at x 42 and `.db-board-card-value` box spans x 122-364 with `text-align: right` computed, at a 1100×900 desktop viewport — the box position is uniform across rows because it is the field's DOM box, not the glyph run, so it does not reproduce the PNG's per-row glyph-start figures. Same conclusion (right-aligned, unmet), different observable. **Anatomy restated by ADR-004** — the draft's "type icon + label + value on the right" is contradicted on both halves by `design-trueup.md` C1 | Unmet | - |
...
`property-row.ts`, `add-property-row.ts`, `hidden-properties.ts`, `cell-editor-contract.ts` — is
built beside its consumers this same pass, with `card-field-renderer.ts` already a re-export shim
over `property-row.ts`'s display value renderer. No consumer has switched yet, so AC-001 through
AC-007 stay Unmet exactly as measured; nothing in this pass ticks a threshold whose own count did
not move.
