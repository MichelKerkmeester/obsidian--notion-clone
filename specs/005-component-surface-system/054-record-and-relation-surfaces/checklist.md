...
| C15 | Source census (ADR-007): shared-builder calls vs hand-built headers/rows in `record-detail-panel.ts`, `table-record-peek.ts`, `board-card-properties-panel.ts` | **Red observed 2026-09-06**: a hand-built `.db-column-manager-row` `createDiv` temporarily added to `board-card-properties-panel.ts` (bypassing `buildCheckboxPropertyRow`) reads `node tools/live/surface-census.mjs` → "hand-built headers/rows across the three surfaces: 1", exit 1; reverted, the same command reads header builder calls 1/1/0 (no header on the board-card panel, by design), row builder calls 1/1/1, hand-built 0, exit 0 | **0** hand-built headers/rows across the three named surfaces | [x] |

**C1, C2 and C14 are the componentization ask, measured against the DOM (a family-wide, 4-consumer
hand count; `column-manager-renderer.ts` is the fourth site C15 does not cover). C15 is the
source-level census ADR-007 amends D3's observable to, scoped to the three surfaces T011/T023
named. C4 and C5 are the Anytype object-page ask. C6 is the edit-surface ask. C11-C13 are the
checks that the rest are not theatre.**
