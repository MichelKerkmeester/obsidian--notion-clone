...
| C3 | Distinct property-type lists | **1 list + 1 filtered subset + 1 submenu**, not 3 independent lists — `create-property-modal.ts:69-74` is the list, `property-type-conflict-modal.ts:364-369` renders a **filtered subset** of it (`:377-380` returns 9-of-13 or 5-of-13), and `column-menu.ts`'s type submenu is the third rendering of the same set. *Corrected 2026-09-05 from "**3**" per T001's restatement of AC-005 (`design-trueup.md`): three code sites, one vocabulary, and the conflict modal's shortening becomes a **gate** (disable with a reason) rather than a filter* | **1** (P7) across all five consumer sites | [ ] |
