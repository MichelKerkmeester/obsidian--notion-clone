...
_memory:
  continuity:
    ...
    recent_action: "Added AC-008/009 for the phone sheets' horizontal overflow sweep"
...
| AC-008 | REQ-005 | **Given** every surface the sheet and stacked-surface inventories enumerate, mounted at 390x844 with `is-mobile is-phone is-ios` on both Chrome and WebKit, **When** each one is measured as the fixtures build it, **Then** its `scrollWidth` is inside its `clientWidth`, nothing it drew reaches past its own right edge, and neither the document nor the body scrolls sideways. **Failing value at `3407dab0`: the mobile inline cell editor** — `.db-cell-edit-mobile-actions` 36.0px past its surface and the surface at 402/366 in Chrome, 410/374 in WebKit, on the fixtures' own short content with no stress at all. | `tools/live/sheet-grammar.mjs` overflow sweep, `as built` pass: red on the current tree at 6 rows across both engines, green after the gutter variable landed. Its own negative control injects a 600px child into a surface the sweep just called clean and requires the same measurement to go red — observed `390/390 -> 632/390 -> 390/390` on both engines, so the sweep is measured rather than assumed | Met | - |
| AC-009 | REQ-005 | **Given** those same surfaces, **When** every vault-derived string on them is replaced by one unbreakable word — a property name, an option value and the field a picker was opened for all belong to the user, so no length can be assumed — **Then** the same three measurements still hold. **Failing value at `3407dab0`: ten of eleven sheet headers**, scroll widths 615 to 687 against a 390px client width, `span.db-panel-title` named in 146 of the 156 offending elements. The eleventh was the record sheet, whose title has carried `min-width: 0` since it was written. | `sheet-grammar.mjs` overflow sweep, `long name` pass: 316 failures on the current tree, 0 after `.db-mobile-bottom-sheet .db-panel-title` gained `min-width: 0` and `overflow-wrap: anywhere`. Shipped copy is deliberately outside the stress and ADR-003 records why: a caption from the translation table is bounded by a translator, not by a vault, and stressing it reported the column-width preset row red against a string it can never receive | Met | - |
...
Eight of nine criteria are `Met`, each with observed evidence rather than a reading of the source.
