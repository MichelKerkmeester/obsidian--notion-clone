...
| report-driven | `044-phone-sheet-alignment` (Level 2) | Bring every sheet and dropdown instance on the phone onto one shared bottom-sheet grammar — surface, handle with working drag-to-close, header and close, padded rows, segmented choices, keyboard avoidance, safe area — driven by `003`'s sheet-and-dropdown inventory. `003` and `016` stay the contract owners; this phase owns conformance | **Opened 2026-09-04, not started.** Three operator reports in eight minutes (`roadmap.md` §4 rows 40, 41, 43): the column-width adjuster is a bare strip that never calls `applySheetChrome` and is covered by the keyboard while typing; the settings sheet has the chrome but a desktop two-column body and a grab handle that does not close it; the Add view sheet has the chrome and no row grammar. Legs running on `worktrees/039-column-width-sheet` and `worktrees/040-settings-sheet` |
...
| 44 | 044-phone-sheet-alignment/ | Every phone sheet and dropdown instance on one shared bottom-sheet grammar; reports 40, 41 and 43 first, then one task per non-conforming instance the `003` inventory ranks | **Opened 2026-09-04, not started.** Level 2 (`recommend-level.sh` 67/100, phase score 20/50 so it stays a standard child). Two legs already running |
