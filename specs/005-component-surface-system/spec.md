...
| report-driven | `044-phone-sheet-alignment` (Level 2) | Bring every sheet and dropdown instance on the phone onto one shared bottom-sheet grammar — surface, handle with working drag-to-close, header and close, padded rows, segmented choices, keyboard avoidance, safe area — driven by `003`'s sheet-and-dropdown inventory. `003` and `016` stay the contract owners; this phase owns conformance | **Landed on main `dcff742e`.** All three operator reports fixed (`roadmap.md` §4 rows 40, 41, 43) plus the ranked non-conforming instances (table record peek, gantt's owned menu, three suggest modals, `group-order-modal.ts` removed). `npm run gate` 26/26 including the new `sheet-grammar` lane (6 surfaces x 7 elements, negative control red-then-green); `verify-placement.mjs` 402/403 (1 declared red, unchanged). AC-001 through AC-005 and AC-007 `Met`; AC-006 (operator) `Unmet`. Not operator-confirmed |
