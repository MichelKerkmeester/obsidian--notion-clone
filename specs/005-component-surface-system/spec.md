...
| operator-directive | `045-board-card-properties` (Level 2) | A Notion-style Properties control for the board: per view, which fields render on a card, in what order, and whether each shows — persisted on `ViewConfig`, honoured by the local extension card renderer, and with zero effect on `038`'s one-to-one reference path | **Opened 2026-09-04, not started.** The ask reads as a feature and is mostly a decoupling: `board-renderer.ts:1439` reads the *table's* visible-column set, then removes the title, the grouped field and every `select`/`status` column by rule, so a card has no field model of its own. The migration must change nothing on upgrade, proved by a capture pair |
...
| 45 | 045-board-card-properties/ | Per-view ordered card field list with visibility, a Properties control on desktop and phone, and a board renderer that reads it instead of the table's hidden columns | **Opened 2026-09-04, not started.** Level 2 (`recommend-level.sh` 46/100, phase score 0/50). Confined behind `boardExtensionsEnabled` so `038`'s reference parity cannot move |
