...
| seam-driven | `043-constructed-capture` (Level 3) | `042` left DONE-row 6 open on five fixture-only gate lanes (css-lane, screenshots-fresh, device-parity, plus the fixture pass of two constructed-pass-supplemented checks). This phase gives `capture.mjs` a constructed scenario type that reuses `042`'s bundle/mount seam, photographs at least one scenario per registered view (list, table, board, gallery, calendar's three scales, timeline's five scales, chart), declares which existing fixtures it supersedes, and rewires css-lane/screenshots-fresh/device-parity to read the constructed capture where one exists | **Landed (partial)** `2ab4942` + `0af4ca6` + `bf67475`, not operator-relevant. Nine constructed scenarios, 36 captures, declared alongside 7 of 11 planned fixtures via `fixtureOf`; `captureData` (T004-T006) now gives list, board, gallery, calendar and timeline real typed cells and 21 real icon names, closing the typed-state/icon gap for those 7 pairs. css-lane and device-parity now read the constructed captures with zero code change, screenshots-fresh's DECLARED-staleness wiring stays open. AC-002 (the readiness negative control) is unmet as written — screenshot rasterisation flushes the frame the wait proves — and needs an operator ruling. Constructed entries share `screenshots/manifest.json` rather than AC-006's separate file, and the timeline is captured at week scale only. Parent DONE row 6 stays open (done-audit-7), narrowed to table's permanently untyped cells, chart's absent per-row typing, and 13 fixture-only scenarios |
...
| 43 | 043-constructed-capture/ | Photograph the constructed production renderers (reusing `042`'s bundle/mount seam) for at least one scenario per registered view, declare the fixtures they supersede, and wire css-lane, screenshots-fresh and device-parity to read the constructed capture where one exists | **Landed (partial)** `2ab4942` + `0af4ca6` + `bf67475`, Level 3. Typed data and real icons landed for 7 of 9 constructed views (T004-T006). AC-002 unmet as written and needs an operator ruling; shared manifest kept over the separate file AC-006 specified; timeline captured at week scale only. Parent DONE row 6 stays open (done-audit-7), narrowed to table's/chart's untyped rendering and 13 fixture-only scenarios |
### Phase Transition Rules
