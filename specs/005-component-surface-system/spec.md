...
| 43 | 043-constructed-capture/ | Photograph the constructed production renderers (reusing `042`'s bundle/mount seam) for at least one scenario per registered view, declare the fixtures they supersede, and wire css-lane, screenshots-fresh and device-parity to read the constructed capture where one exists | **Landed (partial)** `2ab4942`, Level 3. AC-002 unmet as written and needs an operator ruling; shared manifest kept over the separate file AC-006 specified; timeline captured at week scale only. Parent DONE row 6 stays open (done-audit-6) |
### Phase Transition Rules
