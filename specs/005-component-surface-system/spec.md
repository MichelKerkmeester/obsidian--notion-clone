...
| 47 | 047-competitor-references-and-pm-alignment/ | Anytype and AppFlowy reference captures under `screenshots/anytype/` and `screenshots/appflowy/` — official product images and locally installed apps both — plus a measured board and gantt fidelity pass against Project Manager for the operator's *"align closer"* ruling on §4 rows 37/38 | **Opened 2026-09-05, not started.** Level 2 (`recommend-level.sh` 53/100 confidence 92%, phase score 10/50 — below the 25 threshold, so a standard child). Blocked first on `manifest-schema.mjs:118`, which rejects any reference group but `project-manager` |
...
| 048-stacked-sheets | 049-test-environments-and-mock-data | None — `049` is ask-driven (the operator's 2026-09-05 *"set similar test environment… add a lot of mock data… do same for AppFlowy"*) and not blocked on `048`. It reuses the CDP session `047`'s capture leg established with Anytype, which is an instrument rather than an outcome | `049`'s own `validate.sh --strict` first `RESULT: PASSED`, and `npm run gate` exit 0 |
