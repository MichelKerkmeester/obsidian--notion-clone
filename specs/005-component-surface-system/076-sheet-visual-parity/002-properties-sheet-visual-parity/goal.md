...
_memory:
  continuity:
    ...
    next_safe_action: "Run tasks.md T014-T018 (frame-ruling remediation), then dispatch the image judge"
    blockers:
      ...
      - "The shipped two-container tree does not match the divider target; remediation (T014-T018) must land before re-judging (D7)"
    key_files:
      - "spec.md"
      - "plan.md"
      - "tasks.md"
    ...
    open_questions:
      ...
      - "Whether Notion offers a direct (non-Settings-drill-in) entry to Property visibility is unobserved; Frame's leading-vs-trailing header control stays deferred to 076/001's Proposed ADR-I, now also informed by ClickUp's circular close (D9)"
    answered_questions:
      ...
      - "Sheets group with hairline dividers on the plain background, never a card container (D7, operator, 2026-09-11) — supersedes the settings-card treatment this child shipped"
---
...
- [ ] Frame-ruling remediation (D7): L4/L6 rewritten RED→GREEN against the divider target, row height widened to the 44-48pt provisional range (`tasks.md` T014-T018)
- [ ] Image judge **≥ 14/16, no row at 0** — pass #1 scored **11/16** against the shipped two-container tree, before D7; superseded by the remediation above rather than counted toward the two consecutive passes
...
### 2026-09-11 — Frame ruling remediation queued (D7-D9)

The image judge ran after this log's previous entry and scored **11/16** against the shipped
two-container tree. Before a second pass ran, the operator ruled the card container out directly
(D7, same ruling `076/001` received): *"Never use bg container like here for values, notion /
anytype use dividers on plain sheet bg thats better"*. Recorded alongside **D9** (reference
composition: Anytype, Notion and ClickUp, best-of-three per row) in `../decision-record.md`. `spec.md`
§13 is rewritten to the divider target — L4 and L6 now assert "0 containers; dividers present"
instead of a card background — and the row height target widens to `076/001`'s own 44-48pt
provisional range. `tasks.md` carries the remediation as `### Frame-ruling remediation (2026-09-11)`
(T014-T018): RED against the shipped tree, the producer change, GREEN, recapture, and a fresh judge
pass — which is the pass this child's "twice consecutively" now counts from, not the 11/16 pass
above. 0.0.40, cut after this child's CREATE landed and before either judge pass reached the floor,
is the release D8 now prevents recurring.
<!-- /ANCHOR:log -->
