---
title: "Tasks: Phase 8: Record Sheet Visual Parity"
...
# Tasks: Phase 8: Record Sheet Visual Parity
...
- [ ] T009 Register the two title-currency states as constructed scenarios mounting the production record renderer, with accurate `sources` — they are the only sheets in this packet photographed by fixture alone. L1 RED → GREEN (`tools/screenshots/constructed-scenarios.mjs`)
- [ ] T010 Bring the record sheet's property row onto `002`'s settled row exactly — same primitive, same order, same trailing control — and assert the two surfaces structurally identical rather than merely both passing. Run L2 RED first and record the divergence (`src/views/record-surface/property-row.ts`, `src/views/record-detail-panel.ts`)
- [ ] T011 Apply `001`'s card vocabulary to the property and body regions so they read as separated blocks. Run L4 RED first and record it (`src/views/record-detail-panel.ts`, `styles.css`)
- [ ] T012 Re-run `071/011`'s and `071/006`'s clauses unchanged; the header centring and the 4px icon gap are landed values `008` regression-checks rather than redesigns (`tools/live/sheet-grammar.mjs`)
<!-- /ANCHOR:phase-3 -->
