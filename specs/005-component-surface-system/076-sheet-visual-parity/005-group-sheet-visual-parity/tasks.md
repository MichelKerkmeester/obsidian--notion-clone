...
- [ ] T009 If `071/015`'s missing group-sheet screenshot scenario is still open, register it first: a constructed scenario mounting the production group builder through `runRenderAssertions`, with accurate `sources`. Nothing else in this child may proceed until the sheet is photographed from production (D2b). L1 RED → GREEN (`tools/screenshots/constructed-scenarios.mjs`, `tools/live/render-assertion-harness.ts`)
- [ ] T010 Split the sheet into a short entry sheet (`Group by ›` plus a help row) and a separate property picker, matching `001`'s navigation-row vocabulary. Run L2 RED first and record the row count (`src/views/toolbar-renderer.ts`, `styles.css`)
- [ ] T011 Make the property picker terminal: a search field above one flat borderless card, a checkmark on the active row, no chevrons and no per-row controls. Run L3 RED first and record the chevron count (`src/views/toolbar-renderer.ts`, `styles.css`)
- [ ] T012 Decide the Shown/Hidden partition's fate on **our own** consistency argument and write the reasoning down — the reference cannot answer it. Whichever way it falls, `071/012`'s clauses either re-run green or are amended by a Proposed ADR, never silently dropped (`decision-record.md`, `tools/live/sheet-grammar.mjs`)
<!-- /ANCHOR:phase-3 -->
