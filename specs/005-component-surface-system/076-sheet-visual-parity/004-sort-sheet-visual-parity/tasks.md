...
- [ ] T009 Bring the active-rule sort popover onto the sheet's rule grammar — the same second-surface gap `003` closes for filter, and the reason `004` owns two producers. Run L1 RED at 1 and record it (`src/views/active-rule-popover-renderer.ts`, `src/views/sort-panel-renderer.ts`)
- [ ] T010 Merge the property and direction rows into one card with a shared outline and hairline divider, keeping `071/012`'s two-row reading and its 42px indent. Run L2 RED at 2 and record it (`src/views/sort-panel-renderer.ts`, `styles.css`)
- [ ] T011 Move direction choice into a drill-in sub-sheet with two flat rows and a `Done` control, replacing the inline picker. Run L3 RED at 1 and record it (`src/views/sort-panel-renderer.ts`, `styles.css`)
- [ ] T012 Lift the per-rule delete out of the rule block into its own card below it, and add the terminal add/delete-sort card. L4 and L5 RED → GREEN (`src/views/sort-panel-renderer.ts`, `src/i18n.ts`, `styles.css`)
<!-- /ANCHOR:phase-3 -->
