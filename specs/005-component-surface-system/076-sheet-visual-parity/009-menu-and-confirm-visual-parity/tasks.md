---
title: "Tasks: Phase 9: Record Menu, Cell Menu and Destructive Confirm Visual Parity"
...
# Tasks: Phase 9: Record Menu, Cell Menu and Destructive Confirm Visual Parity
...
- [ ] T009 Re-read the seven `delete-confirm` frames and either confirm `071/013`'s action-order ruling at the larger sample or record the disagreement with frame-by-frame counts. Nothing is implemented until this is written down (`screenshots/notion/ios/sheets/`, `spec.md` §13)
- [ ] T010 Bring the confirm card onto `001`'s card grammar, with the destructive styling `007` and `009` agree on — one convention across both children, not two. Run L1 and L2 RED first and record both (`src/views/confirm-sheet.ts`, `src/views/modals/confirm-modal.ts`, `styles.css`)
- [ ] T011 Give the row and column menus the leading-icon · label · trailing-state anatomy, and bring the owned menu's sheet presentation onto `001`'s handle and header vocabulary. Run L3 RED first and record it (`src/views/owned-menu.ts`, `src/views/row-menu.ts`, `src/views/column-menu.ts`, `styles.css`)
- [ ] T012 Re-run `071/013`'s clauses unchanged; if T009 recorded a disagreement, it becomes a Proposed ADR rather than an amendment to `013` (`tools/live/sheet-grammar.mjs`, `../roadmap.md`)
<!-- /ANCHOR:phase-3 -->
