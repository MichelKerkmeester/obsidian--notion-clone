---
title: "Tasks: Phase 6: Add View Sheet Visual Parity"
...
# Tasks: Phase 6: Add View Sheet Visual Parity
...
- [ ] T009 Convert layout choice to a tile grid with a single visible selected state, replacing whatever the T002 baseline records. Run L1 RED first and record the failing shape (`src/views/toolbar-renderer.ts`, `styles.css`)
- [ ] T010 Put the view's settings into one card with hairline dividers and no section heading, matching `001`'s card vocabulary. Run L2 RED first and record it (`src/views/toolbar-renderer.ts`, `styles.css`)
- [ ] T011 Split the settings rows by kind: booleans get a trailing toggle, value settings get a trailing grey value and a chevron that opens a picker. Run L3 RED first and record the counts (`src/views/toolbar-renderer.ts`, `styles.css`)
- [ ] T012 Re-run `add-view-popover-layout.test.ts` unchanged; any regression closes here or the change is reverted (`src/views/add-view-popover-layout.test.ts`)
<!-- /ANCHOR:phase-3 -->
