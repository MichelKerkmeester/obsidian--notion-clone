---
title: "Tasks: Phase 10: Date, Icon, Colour and Property-Type Pickers Visual Parity"
...
# Tasks: Phase 10: Date, Icon, Colour and Property-Type Pickers Visual Parity
...
- [ ] T009 Raise every under-floor picker control to ≥ 44px, starting with the calendar mini-nav at 24px that `071/015` opened and that this child inherits. Run L1 RED at 24 and record it (`src/views/calendar-mini-calendar-renderer.ts`, `src/views/date-value-picker.ts`, `styles.css`)
- [ ] T010 Rebuild the icon picker's search row so no placeholder clips and every control clears the floor, closing the `071/014` P3 finding. Run L2 RED first and record the clipped width (`src/views/icon-picker-popover.ts`, `styles.css`)
- [ ] T011 Close the date picker's CSS-specificity leak at its source rather than out-specifying it, and assert the closure. Run L3 RED first and record the winning selector (`styles.css`, `src/views/date-value-picker.ts`)
- [ ] T012 Bring the property-type picker onto `007`'s landed inline type list; `type-picker.ts` is shared, so a divergence is a bug in one of the two children (`src/views/record-surface/type-picker.ts`)
<!-- /ANCHOR:phase-3 -->
