---
title: "Implementation Plan: Phase 12: sort-and-group-sheet-rows"
description: "The sort sheet gives a rule one row carrying five controls where Notion gives it two rows and a labelled Delete, and the group sheet has no Shown/Hidden partition and no bulk action on its section header. This phase stacks the sort rule, labels its delete, settles the two competing reorder affordances, and gives the group sheet the same shown/hidden vocabulary the properties and record sheets use."
trigger_phrases:
  - "implementation plan"
  - "technical approach"
...
# Implementation Plan: Phase 12: sort-and-group-sheet-rows
...
### Overview
The sort sheet gives a rule one row carrying five controls where Notion gives it two rows and a labelled Delete, and the group sheet has no Shown/Hidden partition and no bulk action on its section header. This phase stacks the sort rule, labels its delete, settles the two competing reorder affordances, and gives the group sheet the same shown/hidden vocabulary the properties and record sheets use.
