...
_memory:
  continuity:
    ...
    last_updated_at: "2026-09-10T02:50:00Z"
    last_updated_by: "278-sort-group-rows-implementation"
    recent_action: "Landed the stacked sort rule, the warning delete and the shown/hidden partition"
    ...
    completion_pct: 90
    open_questions:
      - "ADR-001's Notion half: did Notion use a grip for sort-rule reorder? No repository capture shows one (audit C-4) — the ruling held the question provisional and our own evidence (the arrows carry the keyboard) decided ours"
    answered_questions:
      - "The ↑↓ arrow pair survives, the ⋮⋮ grip goes (ADR-001)"
      - "The group sheet's shown/hidden partition reuses panel.shownSection/panel.hiddenSection/panel.hideAllProperties/panel.showAllProperties — no new words"
...
| D3 | The `×` glyph's expanded `::before` hit area (`styles.css:13820`) was assumed to clear the 44px floor. Measured 2026-09-10: the glyph box is 28.0×28.0 and the painted hit box 40.0×40.0 — 4px short. The ruling survives, strengthened: replacing it with a labelled row is a **legibility** change whose labelled row owns the 44px box, not a touch-target fix, and must not be justified as one (ADR-002) |
...
- [x] A sort rule renders its property and direction on two rows with a labelled destructive Delete
- [x] The sort sheet carries one reorder affordance, not two, with the choice recorded and evidenced
- [x] The group sheet partitions its groups into Shown and Hidden with a bulk action on each header
- [x] No group row carries more than 4 interactive controls
- [x] No regression on `005`'s sort and group clauses, both engines
...
| Implementation | Done | 2026-09-10, worktree `worktrees/278-sort-group-rows`. RED: 7 clause failures — sort 5 controls/row, 2 rows carrying both pickers, 2 affordances, 2 × glyphs / 0 warning rows, group 1 heading / 0 bulk actions, prose 126 characters. GREEN: 0 — 6/6 sort rows 44–52px, heaviest rule row 3 controls, 1 affordance, 0 × / 2 warning rows, 3 group headings with 2 bulk actions, heaviest group row 1, prose 74; the 005 clauses unchanged and green on both engines; the 5-rule stack reads 759.6px = the 90svH ceiling with the body scrolling. Unit 3/3, red-then-green by producer revert (1F\|2P → 3P). Full battery: tsc 0, vitest 1601/1601 (158 files), build 0, render-assertions 0, placement 418/420 (2 declared), four screenshots runs 480/480 exit 0, 16 content movers judged by decoded pixels and kept / 2 jitters restored, evidence 16/16, gate 28/0, scans 0/0, css-lane acquire/edit/release at fcaf3fec28cf → 6e10b42f6324. The ruling: ADR-001 (the arrows survive; the Notion half PROVISIONAL on C-4) |
...
| The glyph's hit box measures 40×40, not the ≥44 the scaffold assumed | The lane's INFO line measured the glyph box 28.0×28.0 and the painted box 40.0×40.0 (the `::before` inset is −6/0/−6/−12, so 28+6+6 = 40). Recorded in ADR-002 as a correction by measurement; the ruling's practical effect: the labelled delete row (min-height 44px) is what owns the 44px box now, and the audit's “legibility, not touch-target” framing was if anything understated |
