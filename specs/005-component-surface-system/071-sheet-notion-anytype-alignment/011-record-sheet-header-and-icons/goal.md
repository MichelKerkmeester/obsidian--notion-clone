...
| Design-review follow-up (F-3) | Done | 2026-09-10, this worktree (`287-dr-record-icons`), tasks.md T010-T012. The review's P2 — the per-row type icon rendered glued to its label, computed text-to-icon gap 0px — closed at the row builder's stylesheet: `margin-right: 4px` → `margin-left: 4px` on `.obnotion-record-detail-field-type-icon` (the icon trails the label's text, so the leading edge is the one that owes the 4px rhythm gap; the 96px label box, the value's left edge and the 44px pitch untouched). RED first: the suite's 4th clause 1 failed | 3 passed → GREEN 4/4. `006`'s clauses unchanged (sheet-grammar exit 0, render-assertions exit 0); vitest 1614/1614, tsc 0, build 0; recapture ×2 480/480 both themes, judged by decoded pixel delta — 20 two-run movers kept at identical counts and deltas, 1 jitter restored; the lane triplet signed (holder this packet, baselineHash `1e320e716905`); evidence 16/16 fresh after 12 stale writers re-ran; naming scans 0; gate 28/0. Goal fraction unchanged: 6/7 — the operator's device read (D3) stays the standing open criterion |
