...
| AC-003 | REQ-002 | Given the phone profile, When the toolbar mini calendar or the date-edit popover paints a day cell, Then every `.db-calendar-mini-day` hit target reads **>= 44 CSS px** | Computed-style read per profile, pinned with a negative control. **Observed red on `3e1c3c65`: 34px** (`styles.css:15938`) and **28px** in the date-edit variant (`styles.css:6942`, inside a `(hover: hover)` block a touch device never enters). No `mini-*` selector carries a touch floor anywhere in the stylesheet - swept across all eight `(pointer: coarse)` / `(hover: none)` blocks and every `.is-phone` / `body.is-mobile` calendar rule. The calendar's own touch floors sit at `styles.css:20733-20856`, `:18492-18511` and the `.is-phone` rule at `:18628-18630`, and none of the three reaches a `mini-*` selector. **Met** by one shared `.is-phone` rule over both variants, plus a second rule widening both 252px hosts to 332px so the seven 44px columns fit: without it the toolbar popover spilled its last column past its own border and the date-edit popover clipped Sunday against `overflow: hidden`, read off the recaptured 402px phone images | Met | - |
