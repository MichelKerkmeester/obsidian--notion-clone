...
- [ ] A view can carry per-view new-row presets, applied at creation. Today: no preset can be stored — `createEntry` already accepts a `defaults` record (`toolbar-renderer.ts:157-159`) but every caller passes `undefined`, and `ViewConfig` carries no preset map (`types.ts:415-432`). `050` item 10's threshold, kept.
