...
- [ ] T022 Import the ten CSVs into the AppFlowy demo workspace, in an operator window. **Superseded, 2026-09-05** (`decision-record.md` ADR-001, superseded the same day by ADR-002) — first skipped because AppFlowy is Flutter with no DOM or accessibility tree and the import needs real mouse clicks the operator chose not to spend, then the operator removed AppFlowy from the reference set entirely, so this environment leg no longer applies at all. The ten CSVs stay in `tools/mock-data/csv/` as product-neutral CSV-export fixtures; `screenshots/appflowy/` (including its import-steps README) was deleted
...
renderer. T022 is not blocked — it is superseded, 2026-09-05 (`decision-record.md` ADR-001,
superseded the same day by ADR-002): the operator removed AppFlowy from the reference set entirely,
so the CSV-import environment leg no longer applies. The CSVs stay in `tools/mock-data/csv/` as
product-neutral fixtures rather than being deleted.
<!-- /ANCHOR:completion -->
...
- `decision-record.md` ADR-001 (superseded) and ADR-002 for why AC-008/T022 is superseded rather than blocked.
...
- [x] The two competitor legs named as blocked, done, or superseded rather than quietly dropped: Anytype (T021) is **done**, loaded over the app's local HTTP API rather than the CDP session it was blocked on; AppFlowy (T022) is superseded — the environment itself was removed from scope (`decision-record.md` ADR-001, superseded by ADR-002).
<!-- /ANCHOR:fix-completeness -->
...
import closes a sixth way — superseded, 2026-09-05, after the operator removed AppFlowy from the
reference set entirely (`decision-record.md` ADR-001, superseded by ADR-002). **The Anytype load
(T021) also closed, 2026-09-05**, over the app's local HTTP API rather than the CDP session it was
written against. One stays open and is not closeable from this session: the operator's own device
read.
<!-- /ANCHOR:summary -->
