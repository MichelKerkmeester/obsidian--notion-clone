...
|---|---|---|---|---|---|---|---|
| AC-001 (A1) | REQ-002 | Render 20 list rows with wrapping off through the production renderer; read each row's `getBoundingClientRect().height`; compute the standard deviation | **Measured 2026-08-29 by `tools/live/view-census.mjs`: standard deviation of row height is 0 at every width and field count.** 20 synthesised rows at 1, 6 and 20 fields, wrapping off, at 320/402/768/1440 — sd 0 in all 24 cells — **the "62px mean" first recorded here was a pre-fix number pasted in as a post-fix one; the measured value is 54.4px**, and 62 is what the row computes with the box-sizing fix removed; and 16 sibling row groups in the real fixtures, all sd 0. **Ragged in height is not refuted, only unmeasured.** The synthesised matrix emits field classes
the renderer never builds, so its 24 cells are unstyled spans; and both list fixtures hardcode a
fully-populated field set on every row, while the shipped default drops empty fields. Rebuilt with
faithful markup and a realistic empty distribution, heights are **76.3px x7 and 52.9px x1, sd 7.7**.
The uniformity recorded here is a property of the fixtures, not of the product.

**And the raggedness the operator reported is horizontal.** Field values start at up to 13 distinct
x positions in one list, because empty fields are skipped and each field is `width: max-content`. They were uniformly 26px too WIDE, at every width and every row, which is what a row whose right edge is off-screen looks like. Fixed by giving `.db-list-row` `box-sizing: border-box`; 0 list-row elements now pass their container anywhere in the census | 0 | N3 | Unmet | - |
| AC-002 (A2) | REQ-002 | With wrapping on, read each row height and divide by the declared line box; record the residual | **Measured 2026-08-29: with wrapping on, every row height is 62px — identical to wrapping off, at every width and field count.** No residual, because no row ever reaches a second line: the row height does not respond to content at all. This criterion is satisfied trivially and therefore weakly; it cannot distinguish a correct multiple-of-line-box from a row that ignores its content | residual 0 | N3 | Unmet | - |
...
| AC-004 (A4) | REQ-003 | Overfill the rail; read `scrollWidth`, `clientWidth` and the parent's width before and after | **Measured 2026-08-29: the rail scrolls and does not grow, at all four widths.** `overflow-x: auto`; scrollWidth 908 against clientWidth 199/281/623/850 at 320/402/768/1440; the rail's own width stays inside the container in every case (0 of 4 growing). The three-act fade is asserted for the first time: the mask is `none` in a static fixture and becomes `linear-gradient(to right, ...)` when `.is-overflowing` is applied — the class the renderer computes from `scrollWidth > clientWidth + 1`, which it would set here. The mechanism works; nothing had ever checked it | true, parent width delta 0 | N6 | Unmet | - |
...
| AC-014 (A14) | REQ-002 | **The table's own rhythm, on data that varies.** Mount a `tools/mock-data` use case through the shipped `TableRenderer` at phone width with the token sheets attached, read every data row's `getBoundingClientRect().height`, and count the distinct values | **Measured 2026-09-05 by `tools/live/render-assertions.mjs`: 1 distinct height, 36px, over 34 Home Inventory rows and 36 Project Tracker rows.** Pre-fix the same population read **7 distinct heights — 36/45/51/69/93/117/141** — and the tallest was attributed by the lane to `<db-multi-select-values>` in a 114px cell. The fixture scenarios cannot produce this reading at all: they give every row the same field count and the same value lengths, so no fixture row can grow past its neighbours | 1 distinct height, and it is <= 49px | N15 | **Met** | - |
| AC-015 (A15) | REQ-002 | **The cause is named, not just the number.** When a row exceeds the rhythm, the lane must report which cell asked for the height and how wide that cell was — measured as a range over the cell's contents, so a cell holding only text is reachable and a utility cell that merely stretches to the row cannot be blamed | **Both failure modes were hit and fixed while building this.** Attributing by tallest element child named the select column's inner box, which stretches to whatever height the row already has; excluding the utility columns then named the chips correctly. Attributing by element child alone could not see the wrapping `notes` cell, which holds a bare text node — a range over the cell's contents reaches it | the reported cell is the cell whose own content is tallest; 0 attributions to a utility column | N16 | **Met** | - |
...
| N15 | Restoring `flex-wrap: wrap` on the four value containers inside a table cell reproduces the recorded 7-height spread | AC-014 measures the row, not the fixture |
| N16 | Attributing the tallest row by element child alone re-blames the select column's stretched inner box | AC-015 measures what a cell asked for, not what it was handed |
