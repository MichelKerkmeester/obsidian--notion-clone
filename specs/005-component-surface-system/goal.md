...
_memory:
  continuity:
    ...
    recent_action: "0.0.37/0.0.38 cut; six 0.0.36 reports + 071/008-014 landed"
    next_safe_action: "Operator device rows (0.0.38); 007 T001 Notion capture"
    blockers:
      ...
      - "061's device row and 067's depth-cap/anchoring rows stay open"
      - "071/007's four card metrics stay provisional pending T001"
    ...
    answered_questions:
      ...
      - "Every phase 000-068 carries its own goal.md; the DONE table cites each open one"
      ...
      - "All eight Notion-refinement children (059-066) are open with a first implementation leg each"
...
| `056-board-anytype-parity` | [`056-board-anytype-parity/goal.md`](056-board-anytype-parity/goal.md) | The board rebuilt to read as Anytype's kanban rather than Project Manager's, each of thirteen elements trued against a named capture before it is written — column header with title, count and menu; card shape and padding; cover; property rows; the `+ New` affordance; column add; drag affordances; group-by and the ungrouped column; option colours; the sticky horizontal scrollbar at the captured 10px/8px/full-width geometry with our own theme colours; the empty column and the deleted-group-relation state; the card menu and its three sub-menus; and the kanban layout's captured page limit of 10 — with the seven affordances gated behind `boardExtensions = false` each retired or folded and none left default-off, `045`'s card-property mechanism kept and retargeted rather than rebuilt, `044`'s grammar and `048`'s stacking held green throughout, and the gantt provably unmoved — then the operator reading the rebuilt board on iOS and on desktop. **Opened 2026-09-05 ~22:45 from the operator's ruling, *"Board UI/UX should almost be 1:1 Anytype"*. It supersedes the board half of the 2026-09-04 Project Manager 1:1 instruction (`roadmap.md` §7.12), not the gantt half. Three reds are already measured on `3407dab0` — 39 constructed `pm-*` classes, 23 `pm-kanban-*` stylesheet rules, 7 default-off affordances — so its first leg has a figure to be checked against rather than one asserted afterwards. T001, the capture true-up over 62 files, is owed to an image-capable leaf and gates everything else.** **2026-09-06: the operator ruled the board scrollbar edge-only, and it landed** — `a70dd113` confined the desktop reveal to its own edge and `dfb416ab` hid the desktop vertical scrollbar at rest. The page-scroll finding the Notion loop ranked first landed separately at `dc1d54a9` and shipped in 0.0.30 (`e016e75c`). | 0/8 |
| `057-calendar-anytype-parity` | [`057-calendar-anytype-parity/goal.md`](057-calendar-anytype-parity/goal.md) | The calendar rebuilt to Anytype's calendar layout, each of nine elements trued against a named capture — month grid, day cells, event chips, the unscheduled area, navigation as the captures show it (month and year selects, arrows, a Today button, the today-scroll), the scale switch as Anytype has it or does not, the today marker, the date-property picker and the day menu — with `044`'s grammar and `048`'s stacking held green, the gantt provably unmoved, and both keyboard-navigation and search-placement tests green at zero lines changed; then the operator reading it on iOS and on desktop, knowing the phone half was inferred. **Opened 2026-09-05 ~22:45 from the same ruling, *"Same for calendar etc."* Two findings shape what it can promise. The calendar carries zero `pm-*` classes — `039` ported behaviour, not markup — so `056`'s headline class-count threshold has no analogue here and would have read green on an untouched tree; ADR-003 records the absence deliberately. And iOS Anytype ships no calendar layout at all, so the phone half has no reference and every phone value carries the label "design inferred from desktop", counted by AC-007 rather than left as a caveat. One row is operator-owned and open: ADR-002, whether the week and day scales survive parity with a product that ships one calendar layout.** **2026-09-06: the phone month chip's ellipsis was ruled and landed at `3e1c3c65`** (moved inside the cell); the calendar rebuild shipped in **0.0.30** (`e016e75c`), with the live evidence re-derived at `c6fde2a2`. | 7/10 (**corrected 2026-09-06**, the 058/refresh leg: `057/goal.md`'s own completion criteria carry 7 of 10 ticked today; the 0/10 figure here was not carried forward) |
| `058-card-title-and-title-formats` | [`058-card-title-and-title-formats/goal.md`](058-card-title-and-title-formats/goal.md) | A number or currency column set as a view's title field renders through that column's own formatter — not a raw stringified value — on the board card, the record sheet header and the phone record sheet; the board's Title fixed slot opens the existing `titleField` picker instead of only reporting it; and the choice already reaching every surface but calendar/timeline is locked by a regression test. **Opened 2026-09-06 from the operator's phone-board report. The per-view title picker is not new work — `ViewConfig.titleField` already ships; the gap is the format routing and the picker's reach from the board's own Properties sheet. Four ADRs record the findings (default title and precedence; where the picker lives; format applied to the title vs. the chosen property; reach to the record surface). No code changed yet.** **2026-09-06 20:10: still 0/5 — no code has landed and its lander is in flight, first in the order of work.** | 7/8 |
| `059-notion-board-refinement` | [`059-notion-board-refinement/goal.md`](059-notion-board-refinement/goal.md) | The board that `056` rebuilt, refined against the Notion kanban harvest and only additively: a Groups `panel` so a group's visibility stops being a dead axis — `hideGroup` and `deleteGroup` are declared on `BoardRendererActions` and implemented by neither host, so the board can neither hide a column nor restore one and `boardHiddenGroups` has no board-mounted writer; per-group toggles with hide-all and show-all; drag reorder from the same rows through the shared row builder rather than a second drag vocabulary; "Hide empty groups" defaulted off against Notion's own default, because an empty column renders a state eight of our captures hold; the eight Notion patterns a landed Anytype ruling already settles declined with an ADR each; four errata where the record contradicts the tree; and four device-only checks named on `056` AC-010. **Opened 2026-09-06 by the Opus synthesis of the board's five-iteration GLM loop, the first of D15's eight reserved children to land. Level 2 (`recommend-level.sh --loc 470 --files 8 --api` 46/100 confidence 82%, phase score 0/50, a standard child). Eleven ADRs: seven Accepted on landed rulings, four Proposed, three of which gate every code leg. What the loop ranked as its own P0 — the page scroll — landed at `dc1d54a9` before this packet opened, so it is a verification row here rather than a task.** **Landed as a packet at `9d9515ae`; ADR-004, ADR-010 and ADR-011 were ruled the same evening. Its implementation leg landed 2026-09-07** — the Groups panel, `hideGroup`/`showGroup` wired in both hosts, `deleteGroup` deleted with its guard and its keys, and `boardHideEmptyGroups` defaulting **on** per ADR-010, which reversed the packet's own "default off" proposal above. One row open, the operator's device read. **Re-verified 2026-09-07 at `f2a7ec34`.** | 9/10 |
| `060-notion-calendar-refinement` | [`060-notion-calendar-refinement/goal.md`](060-notion-calendar-refinement/goal.md) | The calendar that `057` rebuilt, refined against the Notion screen harvest and only additively. The harvest changes **one** decision on this surface and the rebuild landed most of its consequences first: four of the loop's six ranked rows closed on `main` between the loop's baseline and this packet — the `+N more` band (`057` G5/G8), the toolbar's segmented control (G13), the unscheduled chip's 44px floor and the Monday week start (G7) — and are recorded here as **verification**, not work. Two rows survive. The **week and day all-day strip** still prints an inline `start-end` string inside a multi-day bar (`calendar-renderer.ts:862-864`), an element Notion never draws across eleven weeks of multi-day bar in three frames of one entry and one the month grid itself stopped drawing when P0-3 rebuilt it — so one event reads two ways at two scales. And the **date picker's day cells** are the one interactive thing the calendar draws whose touch floor nobody pinned: 34px (`styles.css:15938`) and 28px inside a `(hover: hover)` block a touch device never enters (`:6942`), against `044`'s 44px. **Opened 2026-09-06** by the Opus synthesis of `057`'s five-iteration `/deep:research:auto` loop on GLM 5.3 flash max, lineage `glm-devpass-calendar`, 5/5 under `--stop-policy=max-iterations`. Level 2 (`recommend-level.sh --loc 220 --files 6` returns 28/100 at 80% confidence, recommended level 1, phase score 0/50 — raised on the go-higher rule). Seven ADRs: one adoption (the strip's range string), four Accepted declines that leave a landed ruling standing (chip pill, week start, drop target, today hue), and two **Proposed** rows routed by name to owners this packet does not hold (picker range shading; the picker's today marker). **Landed 2026-09-07 at `5fec918d`.** | 5/5 |
| `061-notion-sheet-refinement` | [`061-notion-sheet-refinement/goal.md`](061-notion-sheet-refinement/goal.md) | The two shapes Notion supplies where Anytype is silent, plus the operator's own report: a phone tap on an editable cell opening **that column's value editor and nothing else** rather than painting a selection and an eight-control bar; selection promoted to an **explicit mode** behind a compact single-row bar of at most six children that never wraps and clears Obsidian's phone navigation pill as well as the safe area; every cell editor claiming the bottom dock so none is drawn over a bar that stayed; and the destructive confirm re-presented as a **card** — inset ≥ 16px on every edge, `--db-radius-xl` on all four corners, actions stacked full width at ≥ 44px — on the phone and on desktop. Then the operator reads both on iOS. **Opened 2026-09-06** as the first of the eight D15 Notion children to land. Additive only: eight Notion-versus-Anytype conflicts are recorded and **zero landed rulings are overridden**; five of its six ADRs are **Proposed** and gate both legs. It deliberately restates none of `067`'s eleven rows — where the two loops found the same scrim, motion band, pill, chip, row pitch or device pass, `061` cites the `067` row instead. Four of its own research loop's citations were re-checked against the tree and corrected rather than carried. **Landed as a packet at `c49ca7f6`, and `870d87a2` recorded the cell interaction model decided from four reference products: tap edits, long-press selects, a three-control anchored pill, a `···` sheet, the editor at the cell claiming the dock, and a 30px desktop bar.** Its implementation leg (cell model plus the confirm card) **landed 2026-09-07 at `abb6827f`, with the tap/dock follow-up at `6ca4a5c3` closing the operator's cell-menu complaint in code.** | 6/7 |
| `062-notion-table-refinement` | [`062-notion-table-refinement/goal.md`](062-notion-table-refinement/goal.md) | The table view refined against Notion item by item: a column that can be frozen and holds its place while the rest scrolls; permanent assertions under the five parity behaviours that currently have none; a date that can carry an end; four type registries that agree; a resize handle that paints something; vertical lines as a view choice; an empty property reading as empty in the peek; an add-row button that names what it adds; and the operator reading all of it on iOS in both themes. **Opened 2026-09-06 by the Opus synthesis of the surface's 5-iteration research loop against the 98-screen Notion digest — the first of wave 1's four to land.** The read is honest about the size of the gap: **one** structural adoption (per-column freeze, absent from `column-menu.ts:38-63` and from `styles.css` entirely), five small additions, and **thirteen** behaviours already at or ahead of Notion, four of them ahead. **The research's second-ranked item is not carried** — the wrap-off row-height defect closed on `main` at `41513bd3` and `1a2c7e00` while the loop ran, and ADR-002 records that the resolution rule the research quotes is stale. **Zero conflicts with a landed Anytype ruling**, because §6A kept the table under item-wise adoption; four tensions dispositioned and three operator decisions Proposed. **Landed as a packet at `f52109c1`; ADR-003, ADR-005, ADR-006 and ADR-007 were ruled the same evening.** **Freeze plus shadow, the per-view noun and all Notion types landed 2026-09-07 at `2c8974fb`, with the freeze-defects follow-up at `3a94e58b`.** | 8/9 |
| `063-notion-dropdown-refinement` | [`063-notion-dropdown-refinement/goal.md`](063-notion-dropdown-refinement/goal.md) | The dropdown, menu and picker family refined against Notion's screen digest: the selection check flipped from leading to trailing in `dropdown-field.ts` — a landed ruling (Anytype **G14**, Notion **N2**) that two of the three row families already honour and that no pending `052` task opens the file to fix; the operator's desktop **sheet escalation** for an anchored dropdown that is cramped by a measured condition rather than a judged one; trailing current values on the **4 of 6** submenu parent rows that lack them, through the `menu-row.ts` slot that already exists; resolved dates under **0 of 3** relative date presets; and search kept unconditional on desktop through the new presentation. **Opened 2026-09-06** by the Opus synthesis of `052`'s five-iteration `/deep:research:auto` loop on GLM 5.3 flash max — the first wave-2 child of the D15 pipeline to land. Level 2, standard child. **Eight ADRs**: six name a Notion-versus-Anytype conflict and leave the landed ruling standing, ADR-007 records the operator's escalation ruling, and ADR-004 and ADR-005 are **Proposed and the operator's** (visible colour labels; a structure-removal carve-out to `051` ADR-007 E3). No code changed yet. **Landed as a packet at `7e44e487`, and `21e6366a` ruled the option colour picker into a labelled list (ADR-005).** **The labelled colour list and the sheet escalation landed 2026-09-07 at `a88894e5`, with the evidence-gaps follow-up at `755f2eac`; the packet's own tally now reads 6 of 8.** | 6/8 |
| `064-notion-toolbar-refinement` | [`064-notion-toolbar-refinement/goal.md`](064-notion-toolbar-refinement/goal.md) | The toolbar `053` rebuilt, refined against the Notion toolbar digest and only additively: a confirm in front of the two unconfirmed delete-view paths — the all-views hub row and the tab context menu, through `051`'s confirm primitive, the scope radio not adopted because our views own no data sources; a zero-rule filter entry tier that takes the commonest filter from three clicks to one while the nested builder goes untouched; the searchable flag on the condition dropdowns, meeting the count gate that already lives inside the dropdown primitive; one text→icon rung that collapses the New button's label before any control is dropped — ahead of the landed order, not instead of it; an add control per rule group in the chip rail, wired to panel toggles that already exist; and per-group visibility settled by ADR-007, whose correction is the packet's own headline — the hidden-group set already exists, is persisted and is read, so the question is whose writer it gets, not whether to add the field. **Opened 2026-09-06** by the Opus synthesis of the toolbar's five-iteration `/deep:research:auto` loop on GLM 5.3 flash max (`053/research/notion-toolbar/`, lineage `glm-openrouter-toolbar`, 34 findings, 6 of 6 questions answered, zero image reads — the second wave-2 child of the D15 pipeline to land). Level 2 (`recommend-level.sh --loc 600 --files 10 --db`: 48/100, confidence 82%; phase score **0/50**, below the 25 threshold, so a standard child). **Ten ADRs**: six record a landed ruling or the tree's own state, three are **Proposed and the operator's** — gating REQ-001, REQ-004 and REQ-006 under goal D6, while their red-first proofs run regardless — and ADR-010 is inherited rather than proposed: `062` ADR-003 carried a pointer here because `064` did not exist on `main` when the operator ruled the conditional-colour view-settings row at 18:32, and an agent may carry a ruling it may not make. Seven of the record's readings did not survive the tree and are corrected in `064/goal.md` §4 and `decision-record.md` rather than carried — one substantive (the hidden-group set already exists), five of citation, and one inherited digest error (conditional colour is ours, not a Notion-only feature). **The confirm-before-destroy row landed 2026-09-07 at `b46f4ef2`, routed through an undo branch rather than a blocking dialog; a follow-up is landing in `worktrees/212`.** | 7/8 |
| `065-notion-record-refinement` | [`065-notion-record-refinement/goal.md`](065-notion-record-refinement/goal.md) | The record and property surfaces refined against the Notion screen digest, additively: the word "Empty" gone from the board card and from every format that has an editor, not just the three ruled ones; the desktop record-sheet label at the same computed size as its value; the built-and-never-consumed `renderOptionValue` split wired on both surfaces; a property nameable and typeable in one pass, which the picker makes impossible today by calling `createProperty(type)` with no label; and a trailing add-property row on a sheet that carries no add affordance at all. **Opened 2026-09-06** from the surface's five-iteration Notion research loop. Its headline is that three of its five ranked candidates are this program's own half-landed Anytype rulings rather than new Notion features. Four ADRs are Accepted on landed rulings — Notion loses each — and four are Proposed and the operator's: the hidden group's per-row eye, the hidden group's population semantics (one label names two different sets today), record-level cover ownership, and where the add entry sits. **Landed as a packet at `cf15a636`; ADR-005 through ADR-008 were ruled the same evening.** **Landed 2026-09-07 at `a0d64df0`, with a touch-target fix at `8fb3c87e`; the packet's own tally now reads 9 of 10.** | 9/10 |
| `066-notion-states-refinement` | [`066-notion-states-refinement/goal.md`](066-notion-states-refinement/goal.md) | The Notion refinement of `055`'s states: an action-carrying toast that stays long enough to act on against the **2200ms** plain-success budget it inherits today (`toast.ts:62`, `:137`), the owned operation failures routed through the toast against **242** bare `new Notice(` sites (`database-view.ts:3681`, `:8378`, `:8468`), a permanent inline chip for a stale reference against a chip census of **0**, `055`'s five stale `goal.md` §3 rows and two lagging `tasks.md` checkboxes taken back to the tree, and the fast band's **4** untokenized `120ms ease-out` declarations resolved behind a recorded curve choice. **Opened 2026-09-06** by the Opus synthesis of the 5-iteration GLM loop; Level 3, standard child. Two Notion findings contradict a landed Anytype ruling — the second destructive red and the toast placement — and both are held as **Proposed** ADRs under D15 rather than applied. **Landed as a packet at `1b3aecf7`; ADR-001 and ADR-002 were ruled the same evening, phone-centred toasts among them.** **Landed 2026-09-07 at `cbb854c4`, with a lane-row fix at `af0e8796`.** **AC-010's dwell-budget criterion ticked 2026-09-08 at landing-verification** (the browser-measured lane rows the criterion itself demanded proved the threshold red-then-green on the rebased tree, `6134f29d`); **the 3500ms operator-report cut and the close-target fix landed 2026-09-09 at `8bc8f050`, land-verified at `1624041e`** (dwell 5000→3500ms, close hit ≥56×67, touch-targets ratchet 171→169). AC-008 (device recheck) stays the operator's, unticked. | 5/6 |
| `067-sheet-family-remediation` | [`067-sheet-family-remediation/goal.md`](067-sheet-family-remediation/goal.md) | The sheet family closed against the eleven rows its own 10-iteration research loop produced: no third stacked sheet and a body producer for the replace move; a `menu`-role surface presenting on the phone as a handle-less anchored card; the page under a first sheet dimmed to the measured level with a lane row for it; zero surfaces bypassing the shell and every shipping phone sheet registered; one source of truth per measured value with a check that fails when the stylesheet drifts; `npm run gate` at 0 from `$?` with one permanent lane row per remaining deliverable; and the operator opening a sheet, a stacked pair, a menu and a destructive confirm on one iOS pass. **Opened 2026-09-06 and landed at `e9cb2417`** from the sheet-family synthesis, with the live evidence re-derived at `dc6df4b4` and the comment-scan pointers at `52598819`. It is **not** a Notion-refinement child and consumes no `059`-`066` slot; `061` cites its rows rather than restating them. **Folded 2026-09-07 at `44101b47`, with a first implementation leg at `173f7d3a`; a follow-up builder (menu card, scale 0.96, AC-003 light, lane pairs, 17 stale captures) is in flight in `worktrees/215`.** A fourth residual leg (2026-09-09) closed the header block at 74px inside the 66-74px band (AC-007 Met) and the divider grammar on the filter, sort, group and Properties sheets (AC-010 Met), with T015/T020/T021 ticked and the gate/lane-rows criterion ticked. A fifth residual leg (2026-09-10) closed the depth-cap row on its own census: the count of stacked sheets at depth 3 the criterion's Done clause names is now a lane-census reading **0 governed of 32 pairs mounted**, with both menu-stack pairs' exemption asserted structurally (their registered parents never offered the cap a replace) and the count mutation-proven to bite — 5 of 7, the declined-anchoring row and the operator iOS pass remaining.** | 5/7 |

| `068-rename-to-obnotion` | [`068-rename-to-obnotion/goal.md`](068-rename-to-obnotion/goal.md) | The plugin called **Obnotion** everywhere a user or a machine reads it — the community plugin panel, the plugin id (so the vault folder, the `obsidian://show-plugin?id=` link and the iCloud install path all move), the ribbon, the settings tab in three locales, and the internal prefixes `note-database-container` -> `obnotion-container` and `db-` -> `obn-` — with an existing install keeping its settings, its open tabs and its notes through a **copy-never-move** `data.json` migration and a **permanent** alias for each of the five strings a user typed or a vault stored. **Opened 2026-09-06 ~18:07** from the operator's instruction, *"our plugin is still named note database but rename to obnotion"*, scoped by their own two answers, *"In community plugin"* and *"Everything, including CSS class prefixes"*. Level 3 at 90/100; the script's four-phase recommendation is **declined on the record** (ADR-006) because the risk is the concurrency, not the edit. Census on `dc1d54a9`: **3,375** name references in `styles.css`+`src`, **17,099** `db-` occurrences in **1,724** distinct tokens, **3,181** `note-database-container`, **4,314** changed lines over **134** files, **1,467** captures re-deriving. Two rows are the operator's and open: ADR-003's `obn-` prefix, and `manifest.json`'s attribution, which still credits pangy9 from the fork point. **Runs last**, after 0.0.30 and after every in-flight leg, in one rebase window. No code changed yet **2026-09-06 20:10: the plan landed at `b4c78e98` and the operator ruled its shape** — the plugin id plus an `obnotion-` prefix everywhere, a `data.json` migration, and `manifest.json`'s author set to **MichelKerkmeester**, which closes the attribution row that was open above. It runs as **one leg** after every in-flight landing, and **0.0.31 is the rename release**. **The GitHub repo itself was renamed to `obsidian_notion-clone` on 2026-09-07, with `origin` repointed; this packet folds that rename in rather than repeating it.** **2026-09-07: the rewrite leg landed** on its own worktree — identity, the compatibility shim, the mechanical sweep (`tools/naming/rename-prefixes.mjs`), the copy-never-move migration, a byte-identical recapture across all 608 tracked captures, and a 26/26 gate run twice. Not pushed; a fresh Opus verifier lands it and cuts 0.0.31. **The landing verification re-derived the leg's claims from the final tree: the decode-level pixel delta across two fresh recaptures found 0 REAL moves (608/608 pixelHash-stable, 3 known jitter files byte-only, max channel delta 1), and the 26-lane gate passed from the final tree after the failing-values ratchet caught 5 goal.md rows whose moved-from numbers were missing (now recorded) and the lane demanded the 3 jitter captures be named in the release entry (now named).** **Landed 2026-09-07 at `5e7f1426`, shipped 0.0.31.** | 5/8 |
| `069-board-cross-group-drag` | [`069-board-cross-group-drag/goal.md`](069-board-cross-group-drag/goal.md) | The board's cross-group drag on a phone — a long-press lift, a ghost that tracks the finger, a highlighted target column, and a drop through the exact write path (`moveRowWithGroupUpdatesAndPosition`/`updateGroup`) the desktop mouse drag already used — plus the Undo toast the embedded host's own cross-group fallback lacked. **Opened 2026-09-07 ~21:40** from the operator's report, *"board view needs to support dragging to other groups and thus updating that property to match grouped field. Like clickup for example. You have task on status 'open' and drag board card to 'in progress'"*, read against the tree: the desktop write path already existed; `board-renderer.ts:445`'s `card.draggable` gate excluded every coarse pointer, so touch alone could not reach it. Level 2 at 40/70 (`recommend-level.sh --loc 600 --files 10 --api`: 49/100, confidence 82%, phase score 0/50 — a standard child). It is **not** a Notion-refinement child and consumes no `059`-`066` slot; `059`/`056` carry a cross-reference row only, since neither owns the drag input layer this packet does. **Implemented and verified same-session**: touch drag wired directly into `BoardRenderer` reusing `resolveBoardColumnByPoint` (pre-existing, previously unwired) and the existing cross-group write path; the embedded host's Undo toast added; real headless-Chrome proof of both input devices (`tools/live/board-cross-group-drag.mjs`, `RESULT: PASSED` — desktop `DataTransfer` drag, phone `PointerEvent` drag at 402x874 with a measured 0.0px ghost delta, the reverse drag, a same-column no-op, and a read-only no-lift); mutation-proven unit tests across three files, two real bugs (a click-swallow torn down before the browser's own compatibility click could reach it, and a hit-test that silently skipped the first column highlight) caught and fixed by the tests themselves; the `styles.css` edit's css-lane acquired, recaptured (0 captures carrying a real content change across two full recapture runs judged by decoded pixel delta), and released; and the 26-lane gate green (`PASS — 26 green, 0 red for a declared reason`) after the `styles.css` hash move surfaced and resolved five pre-existing-tree lane failures unrelated to any pixel changing. **Landed 2026-09-08 at `d32d185d`** (rebased onto the 221/225-merged main), verified and recorded in the 005 handover at `87ec4c8f`, shipped in 0.0.32. **2026-09-09, a 0.0.36 report reopened it** — *"You still cant drag and drop board cards to different columns on mobile"* — root-caused to dispatched `PointerEvent`s bypassing the browser's own scroll-takeover decision; fixed and re-verified with a real-touch CDP harness (`tools/live/board-touch-drag.mjs`, `RESULT: PASSED`) added as the gate's 28th lane, landed `63fdcc5e`. One row is the operator's and open: dragging a card on their own phone. | 8/9 |
| `070-ios-view-data-regression` | [`070-ios-view-data-regression/goal.md`](070-ios-view-data-regression/goal.md) | Every property on every database view renders its frontmatter value on iOS exactly as it does on desktop, proven by a red-before-green harness reproduction with the metadata cache cold as an explicit case, rather than a defensive patch against an unconfirmed suspect. **Opened 2026-09-08 08:07-08:44** from the operator's report — Finance Reports table and Database Testbed board both showing empty properties on 0.0.32 — and its clarification that every property is empty, not just Income. Level 2 (`recommend-level.sh --loc 300 --files 6` → 32/100, raised by judgment). Nothing started; three named suspects (`data-source.ts`, `title-field-display.ts`, `legacy-plugin-data-migration.ts`), none excluded. **Landed 2026-09-08 (`694d7390` + this landing):** reproduced red-first in a permanent 27th gate lane (`database-cold-cache-property-read.mjs` — 0/18 property cells, 0/8 board cells before the fix, 18/18 and 8/8 after, mutation-proven); root cause confirmed in `data-source.ts` — a cold-cache `getViewDefFiles()` scan seeds the whole-vault record cache with `{}` frontmatter that nothing refreshed, so `startListening()` now also recovers every cached record on `metadataCache`'s `"resolved"` event; the other two suspects excluded with evidence. AC-001–AC-004 Met, AC-005 partial (the fixture's own captures, not the operator's vault), AC-006 the operator's. | 4/5 |
| `071-sheet-notion-anytype-alignment` | [`071-sheet-notion-anytype-alignment/goal.md`](071-sheet-notion-anytype-alignment/goal.md) | Every sheet, panel and popover the app can open, inventoried once (answering the operator's R7) and then brought as close as possible to Notion x Anytype's own presentation, one sheet family per child phase, starting with the settings sheet (R5) and the add-property sheet (R6). **Opened 2026-09-08 08:07** as a Level 3 phase parent (`recommend-level.sh --loc 1200 --files 25 --architectural` → 73/100, phase score 30/50, both thresholds met independently). Six children opened, `001-sheet-story-coverage-audit` active and gating the rest; nothing else may start redesigning before it closes. **ALL SIX CHILDREN LANDED 2026-09-09** (001 `527e8455`; 002 `70ee0b95`+`8b213929`; 003 `81b6e328`; 004 `b8b76435`; 005 `79500b89`; 006 `ae89043f`), **parent 4/4**: criterion 4's own no-regression check re-run from this refresh's worktree confirms `tools/live/sheet-grammar.mjs` PASS exit 0 and `npx vitest run` PASS exit 0 (157 files / 1581 tests). Only the operator's own device recheck (071 D3) stays open, and no agent ticks it. **2026-09-09 ~22:30, the operator widened the ruling to every sheet** (§4 row 87) and all seven scaffolded children (`008`-`014`) landed 2026-09-10, awaiting device — see `071/goal.md`'s own Progress table. The parent's 4/4 figure is unaffected, for the same reason `007` did not move it: these children are D3, the operator's own device recheck, held outside the tally. | 4/4 |
| `072-linked-view-blocks-ux` | [`072-linked-view-blocks-ux/goal.md`](072-linked-view-blocks-ux/goal.md) | The operator's ambiguous R2 ("separate views… bugged", "dragging doesnt work on mobile like it would on notion") resolved into a named, confirmed surface, with mobile drag on that surface matching Notion's own feel. **Opened 2026-09-08 08:07.** Level 2 (`recommend-level.sh --loc 200 --files 5` → 27/100, raised by judgment). Board cross-group drag is explicitly out of scope — already shipped in `069`. **Landed 2026-09-08 (`91501ed5` + `6365dfb1`):** the surface traced to the linked-view fence's drag handle, HTML5-only wired and touch-dead (`embedded-database-renderer.ts:3946-3952`); a 6-row UI/UX census recorded one fixed, four enumerated, one measured to parity; mobile drag rebuilt on the 069 gesture grammar (4 gesture tests red→green, two-bag lane parity); gate 27 green, 0 red. AC-001–AC-003 Met, AC-004 the operator's. | 4/4 |
| `073-checkbox-controls` | [`073-checkbox-controls/goal.md`](073-checkbox-controls/goal.md) | Phone checkboxes sized to match the Notion/Anytype reference, and every radio-style input in the app inventoried and converted to a checkbox or the correct non-radio equivalent. **Opened 2026-09-08 08:07** from the operator's R3, evidenced by the Database Testbed board's "Pinned" circle control. Level 2 (`recommend-level.sh --loc 300 --files 8` → 34/100, raised by judgment). AC-004 (the board's own control) depends on `070` restoring property reads. **Landed 2026-09-08 (`f846e605` + `8750c3c2`):** radio inventory found 3 producers (toolbar new-record placement, column-width presets, computed-sync cards), independent grep-confirmed, all 3 converted to checkboxes; 94 phone glyphs re-measured 28×28px → 14–18px glyph inside a ≥44px `::before` hit; the board card's bare-`0` regression (blocked on `070`) confirmed closed now that `070` landed — 36 checkbox fields, 18 checked, 0 bare-`0`. AC-001–AC-003 Met, criterion 4 (the landing-verification assertion) still open, AC-005 the operator's. | 4/5 |
| `074-test-data-consolidation` | [`074-test-data-consolidation/goal.md`](074-test-data-consolidation/goal.md) | One consolidated testbed database replaces the project's scattered fixture datasets, and the operator's Finance databases remain the second dataset, visible again once `070` lands. **Opened 2026-09-08 08:30** from the operator's R10. Level 2 (`recommend-level.sh --loc 250 --files 10` → 33/100, raised by judgment). The operator's own `Database Testbed/` folder stays operator-owned; this packet only proposes its consolidated shape. **Landed 2026-09-08 (`f2df348d` + `00cb6686`):** ten fixture databases consolidated into one 36-record, 28-column Testbed covering every surviving view type; capture/story/phone-smoke harnesses repointed and re-verified (vitest 1672→1678); the Finance fixture kept as the second dataset, code-side proven visible once `070` landed, the operator's own on-device read still open. AC-001–AC-003 Met. **2026-09-09, a 0.0.36 report added a fifth criterion and closed it the same session** — *"Also clean testbed only 1 database with table and board views"* — the testbed's view set narrowed to exactly one table and one board view (RED 2 failed/6 passed → GREEN 8/8, gate 27/0), landed `5cd44328` as AC-005. AC-004 (the operator's Finance read) is the only row left. | 4/5 |
| `075-toolbar-labelled-buttons` | [`075-toolbar-labelled-buttons/goal.md`](075-toolbar-labelled-buttons/goal.md) | The phone toolbar's controls carry icon+label buttons matching the operator's Notion/Anytype/Bases reference, meet the 44px touch-target minimum, and the row scrolls horizontally rather than wrapping or collapsing when it exceeds the viewport. **Opened 2026-09-08 08:50-08:52** from the operator's R14 and its horizontal-overflow follow-up. Level 2 (`recommend-level.sh --loc 300 --files 8` → 34/100, raised by judgment). Updates the existing `009`/`044` toolbar-collapse lane red-first. **Landed 2026-09-08 (`9bcbff0e3`, rebased onto the 070/008-001 landing):** labels via `appendToolbarControlLabel` (toolbar-primitives.ts) on the filter/sort/columns cluster and the settings/more/group buttons, shown only under `.is-phone` and widened to 44px there — desktop and the embedded/codeblock rows stay 28px icon-only (ADR-001); the red-then-green proof is the new 402px `run-phone-toolbar-scroll.mjs` lane: reverting the label rule reads every control bare at 398/398 with no overflow, the shipped rule reads 532/398 with the last control reachable and every control ≥44px; gate 27 green, 0 red. AC-001–AC-005 Met. **2026-09-09, a 0.0.36 report added a sixth criterion (the strip's own vertical-scroll lock) and closed it the same session** — the strip was a two-axis gesture scroller (the 8px touch halo's overhang plus `overflow-y: visible` computing to `auto`); fixed with `overflow-y: hidden` + `touch-action: pan-x` + `overscroll-behavior-x: contain`, landed `d7673e22`, gate 28/0. AC-007 Met, AC-006 the operator's. | 5/6 |
| `076-sheet-visual-parity` | [`076-sheet-visual-parity/goal.md`](076-sheet-visual-parity/goal.md) | Nineteen children, one per surface family, taken through the same six-step loop — define, plan, create, screenshot, verify, remediate — until a reviewer opening our capture beside its reference scores it **≥ 14/16 with no rubric row at 0, twice consecutively on an unchanged tree**. The DOM lane is the floor beneath that judge, never the gate: `071/009` passed its lane at 3 interactive controls per row while its own capture still showed two arrows and a filled checkbox, which is the failure this packet exists to stop. The eleven phone sheets run one at a time in the order the operator meets them, `001` settings first; `012`-`017` (board CSS, then five coverage-audit gaps) and `018`-`019` (the operator's ClickUp board rulings) run independently of that sequence. **Opened 2026-09-10 from the operator's 0.0.38 ruling, *"still has same old badish ui in most sheets nothing like notion"* and *"go step by step multiple ohases per sheet to define, plan, create, screenshot & verify and remediate… untill perfect"*; extended 2026-09-11 by the coverage audit (`013`-`017`) and the operator's ClickUp board rulings (`018`-`019`, `076/coverage-audit.md`). `012`'s two no-regression checks are the only items ticked; everything else scaffolded, nothing started.** | 2/22 |


**Reserved 2026-09-06 ~16:10: eight Notion-refinement children, `059`-`066`.** **All eight are now
open and each has shipped a first implementation leg on 2026-09-07** — `059` through `066` landed
as packets on 2026-09-06 and each landed code the next day (SHAs in the State column below); `064`
and `067` carry a follow-up leg in flight (`worktrees/212` and `worktrees/215`), and `061`'s device
row stays open. Each one is opened by its own Opus synthesis at the end of the pipeline D15 defines —
...
|---|---|---|---|
| `059-notion-board-refinement` | the board — owner `056-board-anytype-parity` | **Opened 2026-09-06** at `9d9515ae`, **implementation landed 2026-09-07**, re-verified at `f2a7ec34`, 9/10, from `worktrees/174`. The first of the eight to land and the first to ship code; ADR-004/010/011 ruled the same evening | the Notion research synthesis |
| `060-notion-calendar-refinement` | the calendar — owner `057-calendar-anytype-parity` | **OPENED 2026-09-06**, Level 2, standard child, landed 2026-09-07 at `5fec918d`, 5/5. The loop ran 5/5 in `worktrees/175` and its synthesis opened the packet | the Notion research synthesis |
| `061-notion-sheet-refinement` | the sheet family — owners `051`, `044`, `048` | **OPENED 2026-09-06** at `c49ca7f6`, Level 3, standard child, **6/7**. `870d87a2` added the cell interaction model; its implementation landed 2026-09-07 at `abb6827f`, with the tap/dock follow-up at `6ca4a5c3` closing the operator's cell-menu complaint in code — one row open, the operator's device read | its own Opus synthesis, from the five-iteration Notion loop plus the operator's 17:07 cell-menu report |
| `062-notion-table-refinement` | the table view — `053` and `052` own its files; the packet now exists | **Opened 2026-09-06 ~16:17** at `f52109c1`, **8/9**, Level 3 — wave 1's first landing. ADR-003/005/006/007 ruled the same evening; implementation landed 2026-09-07 at `2c8974fb`, with the freeze-defects follow-up at `3a94e58b` | the Notion research synthesis, 5/5 iterations in `worktrees/177` |
| `063-notion-dropdown-refinement` | dropdowns, menus and pickers — owner `052-dropdown-menu-and-picker-componentization` | **Opened 2026-09-06 in `worktrees/179`** at `7e44e487` — the first wave-2 child to land. Research 5/5, Level 2, **6/8**. `21e6366a` ruled the colour picker into a labelled list (ADR-005); implementation landed 2026-09-07 at `a88894e5`, with the evidence-gaps follow-up at `755f2eac` | the Notion research synthesis |
| `064-notion-toolbar-refinement` | the toolbar and view controls — owner `053-toolbar-and-view-controls` | **Opened 2026-09-06 in `worktrees/180-notion-toolbar`** — the second wave-2 child to land. Research 5/5, Level 2, **7/8**. It carries `062` ADR-003's conditional-colour view-settings row (operator, 18:32) as its own criterion. Implementation landed 2026-09-07 at `b46f4ef2` (delete-view routed through an undo branch); a follow-up is landing in `worktrees/212`. The loop's artefacts sit at `053/research/notion-toolbar/`, beside the `-toolbar`-suffixed digest (ADR-009 there) | the Notion research synthesis |
| `065-notion-record-refinement` | the record and relation surfaces — owners `054`, `058` | **OPENED 2026-09-06** at `cf15a636`, Level 2, standard child, **9/10**. ADR-005 through ADR-008 ruled the same evening; implementation landed 2026-09-07 at `a0d64df0`, with a touch-target fix at `8fb3c87e` | the Notion research synthesis, `054/research/research.md` |
| `066-notion-states-refinement` | states, feedback and motion — owner `055-states-feedback-and-motion` | **OPENED 2026-09-06** at `1b3aecf7`, **5/6** (AC-008 device recheck stays the operator's). ADR-001/002 ruled the same evening, phone-centred toasts among them; implementation landed 2026-09-07 at `cbb854c4`, with a lane-row fix at `af0e8796`; the 3500ms dwell cut and close-target fix landed 2026-09-09 at `8bc8f050`, land-verified `1624041e` | the Notion research synthesis |

**Why the table view had a row and no owner packet, and what the synthesis decided.** Every other
surface in this program has a phase that holds its files; the table did not, and `053` and `052`
between them hold the toolbar, the wrap switch, the column menu and the cell editors it is made of.
`062` was reserved against that pair rather than against a packet that did not exist, with the
recommendation left to the synthesis and the call to the operator. **The synthesis recommended
opening one and did, on 2026-09-06 ~16:17**: one structural adoption Notion has and we do not
(per-column freeze), five small additions, and thirteen already-parity behaviours of which five are
unasserted — enough that scattering the rows across `053` and `052` would have hidden the shape.
`053` and `052` still own the files; the packet owns the reading. The operator's to overturn.

**`067` and `068` sit outside that reservation and are real folders.** `067-sheet-family-remediation`
was opened from the sheet family's own synthesis, and `068-rename-to-obnotion` from a direct operator
instruction — neither is a Notion-refinement child, neither consumes a `059`-`066` slot, and `068`
took 68 precisely because `059`-`066` are spoken for. **`068` is also the one child that is sequenced
against all the others**: it renames every class name in the tree, so it runs after the whole queue,
including the eight reserved children, in a single rebase window (its ADR-006).
...
| **`007-gallery-view-deprecation`** — the **second sibling packet**, opened 2026-09-05 | [`../007-gallery-view-deprecation/goal.md`](../007-gallery-view-deprecation/goal.md) | No surface offering gallery and no path minting one, every gallery-configured view opening as a board with the same cover once and with a notice, `gallery-renderer.ts` gone, its bench and coverage pin and constructed scenario removed rather than skipped with `npm run gate` still exiting 0, the dead `db-gallery-*` CSS gone, and the operator opening a migrated vault and reporting it as migrated rather than broken. Its four children each carry their own goal; see [`../007-gallery-view-deprecation/roadmap.md`](../007-gallery-view-deprecation/roadmap.md). **Children `001` through `003` are done and shipped in release 0.0.28 (`d3433d81`); `004` closed the doc half — README, CHANGELOG and `030`'s own rows against this retirement — leaving only the packet's operator row open.** | 9/10 |

**The five family phases 051-055 were added to this table 2026-09-05**, opened together by one
operator instruction and landed together after review. **All five ran their T001 the same day**, in
worktrees `087`-`092`, and each is still `0/N` — which is the honest figure, not a placeholder: a
true-up designs against the captures, it does not tick a criterion. The five reads corrected each
other and their parent, and the four numbers on which two of them disagreed are reconciled in
`roadmap.md` §7.10, along with the one owner per shared primitive.
...
**What this entry does not claim.** No product code was written. No T001 had run in any of the five
at the time of this entry, so no design in any of them had been checked against the captures its own
rows name — which is why every one reads `0/N` in the DONE table above. *(Superseded the same day by
the entry below: all five T001s ran, and the §5.A state cells now read* T001 true-up done 2026-09-05,
implementation pending*.)* The parent's `completion_pct` is unchanged by this entry — it records
phases opened, not phase progress — and stays at **5 of 7 = 71**.

### 2026-09-05 (evening): all five T001s landed, and the reconciliation pass that read them together

**Five true-ups landed on main within hours of each other** — `8e0149af` (051), `a58bbcd5` (052),
`fbbddc13` (053), `621de37f` (054) and `ffcf434b`+`cd8030a8` (055) — each run in its own worktree
(`087`-`092`) against the same Anytype capture set. The record-open generalisation landed alongside
them at `ae46da94` and shipped as **0.0.26** (`8c7b65aa`), closing the in-repo half of `roadmap.md`
§4 row 52 and leaving only the operator's own look. This entry records the pass that read all five
together, which is a different job from running any one of them.

**A capture read is a gate, and a gate can itself be wrong.** `050`'s `design-trueup.md` was the
program's read of record and **four of its claims did not survive `053`'s T001**: C2's "no chip row
on any capture" (the rail is on eleven captures and is conditional — present on 5 of 5 List views
carrying a filter, absent on the five ruleless layouts), C7's "no per-view default in the product"
(it is one surface over, in the `New ⌄` menu's `Settings` section), REQ-013's "the filter panel is
one `+ New filter` row" (that is the panel's **empty state**; the populated form is captured across
twelve relation formats), and the flat 60-row page limit (it is per-layout — Gallery 60, Kanban 10,
and no limit row on Grid, List, Calendar or Graph). `052` added a fifth: the "no hover state was
captured" caveat five documents had inherited from the capture index is false — 37 of the 150 menus
were reached by hovering a parent row and each photographs that row hovered. All five are corrected
at their own sites in `050`'s documents and in `screenshots/anytype/README.md`.

**Three of the four wrong claims are the same error**, and naming it is the point of this entry:
**an absence asserted from a surface that was never examined.** C2 scanned the four toolbar icons and
never scanned the band beneath them. C7 searched the settings panel and concluded "absent from the
product". The page limit was read off one layout's panel and quoted as a product default. `050`
ADR-003 gains a second corollary — **absence in one captured surface is evidence about that surface
only** — beside the first, which already said absence of a capture is not evidence of absence.

**Where two reads measured the same element and disagreed, the disagreement is resolved rather than
left in two documents.** `roadmap.md` §7.10 carries all four: the secondary-text ratio (7.11:1, not
7.95:1 — the packet that stated its method wins), the `#232323` fill (1.14:1, not 1.20:1 — three
packets and a re-derivation against one), Anytype's motion exit (0.2s, not 0.1s — `055` read the
source file `047` had paraphrased), and the iOS sheet row pitch, which is **not** a disagreement:
five packets measured five different sheets and no single product figure exists to adopt. The same
section records **one owner per shared primitive**, two of which — the condition row (`053`) and the
inline editors (`054`) — had no written owner until this pass.

**What this entry does not claim.** No product code was written here either. No criterion was
