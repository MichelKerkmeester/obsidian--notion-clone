...
|-------|-----|---------------------|--------------|--------|--------|
| AC-001 | REQ-001 | **Given** the operator's ask, **When** the captures land, **Then** `screenshots/anytype/` and `screenshots/appflowy/` each carry board, table, calendar and timeline from **both** official product images and the locally installed app. **Failing value today: 0 of 16 rows** — neither root exists and `brew info --cask` reports both apps "Not installed". **2026-09-05: both casks installed (0.56.5, 0.14.1). Of the 16 product×surface×source rows: 8 captured (Anytype board/table/calendar-official + table-installed [Grid, 10 rows]; AppFlowy board/table/calendar-official + board-installed [To-dos Kanban, shipped data]), 4 N/A (neither product has a timeline/gantt view — confirmed against each product's own documentation), 4 not reachable (Anytype board/calendar-installed, AppFlowy table/calendar-installed — no simulated mouse click works in this environment: `System Events`'s `click at` is refused with "osascript is not allowed assistive access", and both apps' content areas expose no accessibility tree to click through instead). Beyond the matrix, Anytype also carries an official properties image plus 18 keyboard-reachable installed captures (search, slash menu, object creation, settings, type pages); AppFlowy carries an official properties image plus its search palette and About dialog. Still Unmet — the gaps are structural (an environment permission, and two products' real feature sets), not a skipped step** | The two directories and their contents, plus the uncaptured-row record for anything that could not be taken — see `screenshots/anytype/README.md` and `screenshots/appflowy/README.md`, "Views not captured, and why" | Unmet | - |
...
The capture leg (AC-001, AC-002, AC-008) ran 2026-09-05: AC-008 is Met, AC-001 and AC-002 are
partially satisfied and stay Unmet for structural reasons recorded against each row. The
