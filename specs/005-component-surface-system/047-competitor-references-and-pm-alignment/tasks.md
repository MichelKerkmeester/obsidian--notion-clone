...
- [ ] T013 Run the gantt comparison, the shape `037`'s AC-007 used (`scratch/gantt-comparison.md`). **Unchanged, and now this packet's only alignment leg** — the operator's clarification keeps the gantt on Project Manager, and there is no Anytype timeline layout to move it to: the capture sweep holds six set layouts (Grid, Gallery, List, Kanban, Calendar, Graph) and no gantt among them. §4 rows 37/38's *"align closer"* ruling now applies here alone
- [ ] T014 Close each measured **gantt** gap with a before and an after number; disposition the ones not closed with a reason (`src/views/calendar-timeline-renderer.ts`, `styles.css`). **Narrowed 2026-09-05 ~22:45**: `src/views/board-renderer.ts` is removed from this task's file list — the board is `056`'s and an edit here would collide with its rebuild. A gantt fix that has to touch `styles.css` takes the parent's serialized CSS lane like any other
