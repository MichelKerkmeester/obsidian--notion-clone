...
trigger_phrases:
  ...
  - "reference roots readme section"
...
<!-- ANCHOR:adr-006 -->
## ADR-006: Name both reference roots, and their lane scope, in the generated screenshots index
...
`screenshots/README.md` is generated by `tools/screenshots/capture.mjs` from the capture
manifest, grouped by `scenario.group`. `screenshots/project-manager/` reads as an ordinary group
in that walk and already appeared in the index. `screenshots/anytype/` carries no manifest
entries at all (ADR-004 above is why the css lane excludes it, for the same underlying reason:
no script in this repository produces it), so it was invisible from the generated index even
though the directory exists on disk. A reader opening only `screenshots/README.md` had no way to
learn `anytype/` exists, whether either root is one of this repo's own renders, or where
`anytype/`'s own index lives.

### Constraints

- Keep the README's own rule intact: generated, not hand-written, so it cannot fall behind the
  manifest.
- No timestamps or other non-deterministic content in the generated text — the file must stay
  byte-comparable between two runs over the same inputs.
- The lane-scope claim must not go stale on its own: `tools/lane/css-lane.json`'s
  `inScopeCaptureRoots` already changed once (ADR-004) and could again.
<!-- /ANCHOR:adr-006-context -->
...
**We chose**: add a generated "Reference roots" section listing `screenshots/project-manager/`
and `screenshots/anytype/`, naming for each whether it is our render or a photograph, its lane
scope, and where its own index lives.

**How it works**: `capture.mjs` now exports a pure `buildReadme(manifest, laneInScopeRoots)`
that assembles the whole README, including the new section. The lane-scope line is computed by
checking each root against the `inScopeCaptureRoots` array passed in at call time (read from
`tools/lane/css-lane.json` at capture time) rather than a literal "in scope"/"not in scope"
string, so a future lane-config change moves the README's claim on the next capture run instead
of leaving it to drift.
<!-- /ANCHOR:adr-006-decision -->
...
| Option | Pros | Cons | Score |
|--------|------|------|-------|
| **A. Generated section, lane scope looked up from `css-lane.json` at capture time** | Stays accurate across a lane-config change with no code edit; keeps the "generated, not hand-written" invariant | One more exported function to keep in sync with the manifest shape | 9/10 |
| B. Hand-written paragraph appended to the README template text | Smaller diff | Violates "do not edit by hand"; the file is fully rewritten every run, so a hand-written paragraph would need re-adding after every capture or would silently vanish | 2/10 |
| C. Hard-code "in scope" / "not in scope" per root as a literal string | No lookup needed | Goes stale the moment `inScopeCaptureRoots` changes again, the exact drift ADR-004 exists to prevent | 3/10 |

**Why this one**: Option A is the only one that keeps the section both generated and accurate
without a future manual edit.
<!-- /ANCHOR:adr-006-alternatives -->
...
**What improves**:
- A reader of `screenshots/README.md` alone can now find `anytype/` and `project-manager/`,
  tell a render from a photograph, and know which one the css lane's review obligation covers.
- The README-generation logic is now unit-tested without a Chrome capture run, closing a gap
  where no test previously exercised it at all.

**What it costs**:
- `buildReadme` is a small refactor extracted from `main()`'s inline block; behavior for the
  existing scenario groups is unchanged, verified by `capture.test.mjs`'s third case and by the
  full recapture producing an identical set of group sections.

**Risks**:

| Risk | Impact | Mitigation |
|------|--------|------------|
| A third reference root is added later without updating `REFERENCE_ROOTS` in `capture.mjs` | L | The list is four lines per root at one call site; `capture.test.mjs` pins the current two so a reviewer sees the list needs a third entry rather than the gap going unnoticed |
<!-- /ANCHOR:adr-006-consequences -->
...
| # | Check | Result | Evidence |
|---|-------|--------|----------|
| 1 | **Necessary?** | PASS | `anytype/` and its lane-scope status were undiscoverable from the generated index before this change |
| 2 | **Beyond Local Maxima?** | PASS | A hand-written paragraph and a hard-coded scope literal were both considered and scored lower for breaking the generated-file invariant or drifting from `css-lane.json` |
| 3 | **Sufficient?** | PASS | Exactly the two roots that exist today, looked up against the live lane config; no unrelated capture or manifest behavior changed |
| 4 | **Fits Goal?** | PASS | Directly closes the documentation gap D2's vendor roots (ADR-003, ADR-004, ADR-005) left in the generated index |
| 5 | **Open Horizons?** | PASS | A future in-scope or reference root is a `REFERENCE_ROOTS` entry plus a `css-lane.json` line, not a rewrite of the section's logic |
...
**What changes**: `tools/screenshots/capture.mjs` gains `REFERENCE_ROOTS` and an exported
`buildReadme()`; `main()` calls it instead of assembling the README inline. `tools/screenshots/capture.test.mjs`
is new, red-first, and asserts the section's content, its lane-scope lookup, and that the
existing scenario-group output is unchanged. `screenshots/README.md` is regenerated via
`npm run screenshots`.

**Verification**: `npx vitest run tools/screenshots/capture.test.mjs` (red before, 3/3 green
after); `node tools/screenshots/verify.mjs` (554/554 current); `npm run gate` (25/25 green).

**How to roll back**: revert `tools/screenshots/capture.mjs` and `tools/screenshots/capture.test.mjs`,
