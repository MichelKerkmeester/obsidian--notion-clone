...
| C2 | Month grid, day cell, event chip match (AC-002) | **Was 91 classes, 133 rules, none trued** (measured on `3407dab0`). **Now measured on the landed captures, and the row does not close.** What holds, read at DPR 2 off `screenshots/notion-clone/views/calendar-month-view-desktop-{light,dark}.png`: eight vertical rules at device x 112/496/882/1268/1656/2042/2428/2814, 2 device px wide, `#EBEBEB` light and `#292929` dark; horizontal rules at y 477/751/1025/1299/1573, a 137px pitch (136px row + 1px border); `#F7F7F7` light and `#1E1E1E` dark weekend columns 1 and 7; a today disc measuring exactly 52x48 device px of `#216DFA` (26x24 CSS) at 13 device px inside the right rule and 15 below the row rule; chips flat at a 20px pitch with first ink 72 device px (36 CSS) below the rule. What does **not** hold, from the same images: the month grid's border box runs device x 112..2815 while its container runs 80..2783, so the new 16px inset applies on the left only and the grid overruns 16px on the right; and the weekday row's column pitch is 377.3 device px against the grid's 386.2, so the labels' right insets read 31/40/48/60/69/78/87 device px where a constant 22 (11 CSS) was intended. Five values remain **pixel read owed** exactly as recorded. `tasks.md` T015 carries all seven residuals | Each of the three matched to a measured value or deviating on a named accessibility ground | [ ] |
