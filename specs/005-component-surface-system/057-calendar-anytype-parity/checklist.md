...
| C2 | Month grid, day cell, event chip match (AC-002) | **Was 91 classes, 133 rules, none trued** (measured on `3407dab0`). **Now matched, measured on the retargeted tree** (93 renderer classes, 135 stylesheet rules — the growth is the new title-select, nav-chevron and show-icon-toggle modifiers, not scope creep): month grid at 136px rows / #EBEBEB / #292929 rules / 16px inset; day number top-right at 16px/400 with the outside-month and weekend-fill deltas closed; event chips flat at a 20px pitch with no background, border or radius, 10px icon inset, 8px icon gap, #292929/#DDDDDD ink; today marker a 26x24 #216DFA disc with a white 16px numeral. Five values remain **pixel read owed** exactly as recorded (hover, six-week months, overflow, truncation, multi-day spans) — kept unchanged, not silently dropped | Each of the three matched to a measured value or deviating on a named accessibility ground | [x] |
