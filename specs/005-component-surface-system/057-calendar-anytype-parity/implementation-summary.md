...
_memory:
  continuity:
    ...
    recent_action: "landed T021: unscheduled band to header chip, multi-day flex-grow fix"
    ...
    answered_questions:
      ...
      - "T021: ADR-006 picks a header chip over three other integrations; the multi-day range's own detachment was a title flex-grow bug, not a text-align one"
---
...
| `src/views/calendar-renderer.ts` | Edited | T021: `renderUnscheduledBacklog` retired for `renderUnscheduledChip`/`openUnscheduledMenu`; the three title-select methods now return their `title` element |
| `styles.css` | Edited | T021: `.db-calendar-backlog*` removed; `.db-calendar-unscheduled-chip` and the `:has()`-scoped title `flex-grow: 0` added |
| `src/views/calendar-renderer.test.ts` | Edited | T021: the backlog tests rewritten for the chip; the completed-row test reworked to call `isRowCompleted` directly (the popover needs a real document this file's `MockElement` does not provide) |
| `src/views/calendar-pinned-values.test.ts` | Edited | T021: two new pins — no `.db-calendar-backlog` selector survives, and the multi-day title/range flex-grow fix |
| `tools/live/render-assertion-harness.ts` | Edited | T021: `calendarUnscheduled`/`calendarMultiDay` scenario options; `calendarAssertions` gains the band/chip/range-gap checks |
| `tools/screenshots/constructed-scenarios.mjs` | Edited | T021: the two new options added to `SPEC_OPTIONS`; `constructed-calendar-month-unscheduled` registered |
| `tools/screenshots/constructed-capture.test.mjs` | Edited | T021: the new scenario id added to the registry-coverage list |
| `tools/lane/css-lane.json` | Edited | T021: lane taken over from `052`, released at `95cab2d8a169` naming 8 reviewed captures |
| 8 `screenshots/notion-clone/views/{calendar-month-view,constructed-calendar-month-unscheduled}-*.png` | Recaptured | T021's two fixes, both pixelHash-moved and opened in both themes/devices |
<!-- /ANCHOR:what-built -->
