...
| AC-011 | REQ-006 (2026-09-06 ~10:33 amendment) | **Given** a calendar with unscheduled items, **When** it renders, **Then** no dedicated band sits above the grid and the items are reachable from a compact header-row affordance, drag-onto-a-day kept | **Today: red on the operator's own ~10:33 desktop capture** — an *"Unscheduled (1)"* band of roughly **80** CSS px above the grid holding one centred item. Green is **0** px of band with the items still reachable and still droppable, plus an ADR naming the alternatives considered. **The same capture's centred multi-day range text and per-column chip drift are not this row's** — G5 and G3 own them, measured on the same corpus | Unmet | - |
