...
**Priority:** C1 through C5 and C7 are P0. C6, C8 and C9 are P1. C10 is the operator's, and
`059-notion-board-refinement` T004 named the four device-only checks it decomposes into — C10.1
through C10.4 below. They are **additive**: AC-010 stays **Unmet**, and no agent ticks any of them.
...
| C10.1 | **OPERATOR** — page scroll with **0px** desktop scrollbar chrome at rest (AC-010) | not asked. `dc1d54a9` moved the scroll to `.note-database-container.db-kanban-view` and set `::-webkit-scrollbar { height: 0 }` at rest, `10px` on hover or `.is-scrolling` (`styles.css:9348-9356`, `:9386-9392`) — measurable in the repository, but whether it *reads* as chrome-free is the operator's | The operator scrolls a real board on desktop and reports whether any scrollbar chrome is visible at rest | [ ] |
| C10.2 | **OPERATOR** — the board in **dark theme** (AC-010) | not asked, and **no reference exists**: the Notion harvest contains no dark-theme board or kanban capture on either platform (`notion-screens-digest.md` § 6.1). Nothing in this repository can substitute for the operator's own read | The operator opens the board in dark theme and reports whether it holds | [ ] |
| C10.3 | **OPERATOR** — desktop **hover-reveal** of `···` / `+` (AC-010) | not asked, and **uncaptured on either reference**: no web capture in the Notion harvest catches a column mid-hover (`notion-screens-digest.md` § 6.3). Ours is `opacity: 0` until hover or `:focus-within` on `:not(.is-touch)` (`styles.css:9458-9464`) — the rule is readable, the feel is not | The operator hovers a column header on desktop and reports whether the pair reveals cleanly | [ ] |
| C10.4 | **OPERATOR** — the **phone board on a real handset** (AC-010) | not asked. `056` R5's phone capture is harness-synthetic — a 402px viewport with `matchMedia("(pointer: coarse)")` forced (`tools/screenshots/capture.mjs:123`, `:389-390`) — so no capture in this repository is evidence about a real device | The operator opens the board on an actual phone and reports whether the touch targets and the column strip hold | [ ] |
<!-- /ANCHOR:protocol -->
