...
**RED/GREEN**: the clauses went into `tools/live/sheet-grammar.mjs`’s panel-sheets section (per-row interactive-control counts, the property/direction-on-own-rows pair, the reorder-affordance census, the ×/warning tally, the bulk-action-beside-the-title placement, the longest body-text run, and a five-rule 90svH/keyboard-inset mount driven through the sheet’s own add control) and ran RED first: **7 failures — sort 5 controls on one 48px row, 2 rows carrying both pickers, 2 affordances, 2 ×/0 warning rows, 1 group heading with 0 bulk actions, prose 126 characters, heaviest group row unasserted**. After the producers moved: **GREEN, exit 0 — 6/6 sort rows 44–52px (48, 44, 44, 48, 44, 44), span 357px = 357px, heaviest rule row 3 controls, 1 affordance, 0 ×/2 warning rows, 3 group headings 16/16 inset with dividers 0/1/1, 2 bulk actions on their own line, heaviest group row 1, prose 74**; the 005 clauses (44–52px, 16px/16px, 1px divider, 0 native selects, one row span, extent == clientWidth) unchanged and green on both engines; the 5-rule stack 759.6px = the 90svH ceiling exactly with the body scrolling (the shorter 378px read under the old 1-row rule proves the taller rule is what exercises the cap). `sort-panel-renderer.test.ts` grew the stacked-rule contract (2 picker rows, 1 affordance, the labelled warning row, ≤4 controls, the click removes the rule): 3/3, red-then-green proven by reverting the producer — 1 failed | 2 passed → 3 passed.
...
## 071/013-sheet-input-and-action-order — the sheets' input and action order (2026-09-10, this leg)

The operator's ruling (§4 row 87) closed its four-ordinal finding, each surface RED→GREEN on the
lane and held there. The stacked confirm reads destructive-then-Cancel (RED: Cancel first;
GREEN: the destructive action the first child, Cancel the last; the side-by-side footer's order
unchanged and separately asserted; `061`'s geometry re-read landed: inset 275.3/35/35/275.3px,
radius 16px ×4, column, heights 50/44px — the pre-reorder pair printed 44/50). The date picker's
body reads calendar → Today/Tomorrow/Next week → the typed segments → Clear its own plain row
outside the group's role (RED: calendar last, Clear the fourth of four; the placement clause holds
the published `--obnotion-keyboard-inset` and the calendar inside the sheet's lifted box, post-reorder
bottom 1305.9px inside floor 1451.9px). The add-view sheet's create rows precede its four settings
(RED: `choicesBeforeForm: false`; the name input's placeholder still `""`, the diff's evidence and the
clause's). The record popover hosts 0 free-text inputs: its fifteen per-column presets answer behind
a chevron row that swaps to their own popover at the same trigger — landed destination-before-source,
the lane proving 15/15 answering there while 15 still sat on the record surface, then 0 there;
the goal's second open question is answered, behind a row (`013/decision-record.md` ADR-002). The
order facts, the placement clause and the confirm's two-variant pair live in
`tools/live/sheet-grammar.mjs`'s input-and-action-order section; every clause RED first (the first
run: 7 failures, exactly the packet's clauses, every landed row green). Unit-held:
`confirm-sheet.test.ts` (created) 2F/1P → 3/3, the mutation proof the task mandated. One verifier
amended, recorded: the storybook add-view group-distance clause read the two groups' order as signed
(-344/-412px, the recorded red) and now reads the separation either way. The packet's edits also
shifted `toolbar-renderer.ts`'s line anchors, so `001`'s inventory.md was regenerated by its own
tool (two lines, both line numbers) and the toolbar-surface contract clause that named the removed
inline Settings section now names the row (`obnotion-utilities-presets-row`).

**The numbers**: tsc 0; vitest 1606/1606 (159 files); build 0; sheet-grammar 0 (2360 clause lines);
render-assertions 0; verify-placement 0 (420 checks, 418 + 2 declared); screenshots ×2, both 480/480 —
24 deterministic movers kept (4 constructed-add-view, 8 constructed-date-picker(-datetime), 8
field-date-value-picker(-datetime), size-differing because the reordered popover's intrinsic height
changed, 4 constructed-confirm(-stacked)), 2 one-run 1px@Δ1 jitters restored (board-mobile-desktop 2px,
board-view-desktop 8px; run 2 reproduced the committed bytes, so the working manifest already matched);
the css-lane signed acquire/edit/release, baselineHash `aa57d141259c`; evidence 16/16 fresh after 13
stale writers were re-run (exit 0 each); gate 28/0 once, from the final state; scan-comments 0;
scan-failing-values 0; validate --strict `RESULT: PASSED` ×3 (this packet, the 071 parent's first
RESULT, the 005 track) after the scoped backfill of each.

**Open**: the visual retuning — the Clear row's paddings, the calendar divider's margin, the presets
popover's width — is provisional until the operator's own device read (D3, the audit's C-1..C-6),
which closes the packet (goal 7/8; AC-011 stays the operator's row). The record popover's
before/after is the lane's printed 15→0 plus the destination's 15, not an image — the constructed
...
## 071/015-sheet-design-fundamentals — the design review's two cross-sheet findings: the 24px calendar nav and the group sheet's missing capture (2026-09-10, this leg)

The `sk-design-fundamentals` review's two findings that no single child owned, both landed.
Finding (a): the shared `.obnotion-calendar-mini-nav` (the date picker's, the sort sheet's
calendar-empty state's, and the filter's date-value picker's month arrows) now clears the 28px
coarse-pointer floor — the raise sits inside the existing `@media (pointer: coarse)`
floor-raise block by `min-width`/`min-height: 28px`, so the desktop's declared 24px box keeps
its density and only the touch surface's metal grows. The enforcement is new, not the census's:
a named-28px entry in `touch-targets.mjs`'s RAISED mechanism, the harness's one unconditional
floor check, because the movement ratchet the three 28px siblings rely on counts totals and
never catches a control resting at a constant under-floor size. RED, exit 1 — eight misses,
each `measured 24x24, under its named 28px floor` (two fixture, two constructed, both themes) —
GREEN, exit 0, census 127→123 fixture and 655→651 constructed as the arrows left the ratchet
into the named floor.

Finding (b): the toolbar's Group-by sheet, redesigned by 012 and never photographed, registered
its first scenario — `id: "group"` in `tools/screenshots/scenarios/panels.mjs` beside its
sort/filter siblings, phone-only (`devices: ["mobile"]`, exactly the two captures the task
names), `src/views/toolbar-renderer.ts` first in `sources` (with `popover-position.ts`,
`mobile-bottom-sheet.ts` and `surface-shell.ts` for the shell chrome the markup mirrors). The
markup reproduces what `populateGroupPopover` draws in its reachable nobody-grouped-yet state:
grab bar, the shell header with its 44px close, the `Group by` section, the checked No-group
row, the `Shown` section with its `Hide all` bulk action and three property rows, the `Hidden`
section with `Show all` and one hidden property — both sides of the 012 partition and both bulk
actions in frame. The corpus grew 480→482: `group-mobile-light.png` 32068 bytes,
`group-mobile-dark.png` 31175, 804×1748 device-px. What they show was recorded from the decoded
numbers per this worktree's standing measurement ruling (no image opened): every one of the
1,405,392 pixels differs between the themes, the top 100 rows are uniform, the bottom 200
painted — the sheet, bottom-anchored where a phone presents it. The criterion-2 look was the
review-continuation's, as that criterion's own wording allows; the operator's device read (D3)
stays the packet's standing habit, not a criterion of this child.

Verification, from the final state: the eight date-pair mobile captures moved 248px@Δ112-132
each, identical counts in every judged pass, and the changed-pixel bands are the two arrow boxes
alone — the centred month label's region contributed zero changed pixels, so the
...
`board-view-desktop-dark` (8px@1, which the convergence pass returned to its committed pixels
exactly) are today's engine, named in the lane release for that reason, and
`board-mobile-desktop-dark` (2px@1, one judged run) was restored to its committed bytes. Screenshots
×3 482/482 exit 0 (the third a convergence pass, clearing the one stale committed blob the
freshness check caught — the gate's first run, 27/28, then 28/0). The eleven stamp writers the
freshness check named (ten styles.css-input lanes plus capture-device-parity) were re-run by
their own CLIs, evidence 16/16 fresh. vitest 1614/1614 — one test, the storybook inventory's
registry-identity check, needed `001/inventory.md` regenerated by its own
`tools/storybook/sheet-inventory.mjs`, the generated table's own contract, its group row now
carrying the scenario; tsc 0, build 0, sheet-grammar 0 (both engines, negative controls
included), render-assertions 0, placement 418/420 (2 declared, exit 0), naming scans 0/0. The
css-lane took the acquire/edit/release triplet: holder `015-sheet-design-fundamentals`,
baselineHash `5560c2030c05` (sha256 of the edited `styles.css`, first 12), all twelve judged
paths named in the release's `reviewed`; `node tools/lane/check-lane.mjs` exit 0.

Docs: all four AC rows Met with their evidence, the goal's three criteria ticked, `tasks.md`
T001-T009 ticked with completion notes, the packet's `spec.md` and the closure statement record
the landing, a landing note added to the `071` row in §5.A. Validated: `015`, the `071` parent
(first `RESULT`) and the `005` track, `RESULT: PASSED`, after `015`'s scoped graph-metadata
backfill. Not pushed at the time this entry was written — see the commit trailer for the landing
SHA.
