...
| AC-4 | REQ-005 | Whether every action is reachable, and whether an overflowing bar says so | reachable, and every action's right edge inside the bar's client box | ~~scrollWidth 558px against clientWidth 356px, `overflow-x: auto`, `scrollbar-width: thin`~~ **Superseded 2026-09-02: the bar wraps instead of scrolling; see `goal.md` D2 and log** | **maxActionRight 567px past a 373px port (red), 341px inside 373px (green)** | Met |
