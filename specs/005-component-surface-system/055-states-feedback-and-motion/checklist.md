...
| C5 | Distinct empty states | **12 reasons ship** (`empty-state-renderer.ts:24-36`); **0 of 12 is the deleted-relation state** — deleted group field → silent re-group (`database-view.ts:2678`, `:2890`, `:3378`). `050`'s "all conditions render the same state" was false (`design-trueup.md` REQ-009). **Superseded by ADR-009**: a deleted/missing *source* is a third condition of its own, not a `no-matching-data` reading, so the true today is 13 reasons shipped (`group-relation-deleted` landed at `f61b1dc9`) and 0 of them the source-missing flavour | the existing 12-to-3 mapping **asserted**, the deleted-relation state **built** and pointing at view settings, and a fourteenth reason for the missing/deleted source, its own copy naming a primary "Choose database" action | [x] Green, 2026-09-06: `EmptyStateReason` holds 14 members; `getEmptyStateReason` answers `"source-missing"` when the caller reports the view's configured source no longer resolves in the vault, and keeps `"no-matching-data"` for a zero-row source that is still there — the row count alone cannot tell those apart, and reading it as if it could would have shown a brand-new empty database a lost-source card. `group-relation-deleted` (`isBoardGroupFieldMissing`) and `no-database` keep their own meaning. Two permanent lane rows (`table-empty-source-missing/file-view`, `table-empty-no-matching-data/file-view` in `tools/live/render-assertions.mjs`) drive the real predicate over the mock-data catalogue at zero rows, one variable apart, and assert the rendered `data-empty-reason`; unit tests assert all three zero-row cases and that all three titles differ. Negative control: collapsing the predicate back to the row count turned exactly the `no-matching-data` row red, every other row staying green; restored, green again. See `acceptance-criteria.md` AC-005 |
...
| C15 | Single-row delete asking for confirm before it can offer Undo | **every** single delete confirmed first — `row-menu.ts:169-176` calls `confirmWithModal` unconditionally before `deleteRow`, on the tree T018 landed | a single delete with a readable snapshot skips the confirm and deletes immediately, offering Undo on the toast (T018's mechanism); the confirm stays for a delete whose snapshot fails, for bulk (`deleteSelectedRows`, which still records no entry), and for anything else not undoable | [x] Green, 2026-09-06 (ADR-010, operator ruling *"No confirm for single delete, Undo toast"*): the confirm call moved out of `row-menu.ts` into `deleteRow` itself, in both classes, gated on a new `canUndoDeletion(app, file)` predicate (`modals/confirm-modal.ts`) that reads the row's content and returns it, or `false` on a read failure. `false` still confirms, with no history entry pushed and no Undo offered; a successful read skips the confirm, pushes the entry and offers Undo exactly as T018 built. `deleteSelectedRows` is untouched — it still confirms and still records nothing. **Negative control, observed:** the test mocks only `confirmWithModal` and spreads `importOriginal` for everything else, so the shipped `canUndoDeletion` runs rather than a copy. Breaking it to never return `false` — the change that would trash an unreadable row with neither confirm nor Undo — fails 7 of 17 cases, both "cannot be read" ones among them. An earlier draft that reimplemented the predicate inside the mock stayed 15/15 green under that same break, which is why it does not. Six new cases: no-confirm-on-readable and confirm-and-no-Undo-on-unreadable for both classes, plus one per class pinning that the history entry is recorded before anything that can throw after the trash — moving the push after the toast fails exactly those two and nothing else. `npx tsc --noEmit` 0; `npx vitest run` **1435/1435** in 138 files (17/17 in `deletion-undo.test.ts`, 1429/1429 on the landing base); `npm run build` 0; `node tools/screenshots/verify.mjs` 550 entries current, **0 `pixelHash` moves** — the eight `chrome-selection-status-bar` / `chrome-table-load-more` entries update only the `embedded-database-renderer.ts` source hash they track; `node tools/lane/check-lane.mjs` 0; `npm run lint` unchanged at **269** problems (244 errors, 25 warnings), the same count the landing base reports. The isolated `npm run gate` is **25 green / 1 RED, exit 1** — `failing-values`, which fails identically on the landing base (`scan-failing-values.mjs` exit 1, 334/188/146 against a baseline of 145, on two rows in `038` and `053` that this leg does not own and does not touch) |
...
**Verification Date**: 2026-09-06 (T004/T005/T006/T007/T008/T010/T018 landed)
<!-- /ANCHOR:summary -->
