...
- [x] T005 [P0] **REQ-055-5 — three empty-state flavours, one of them new**
      ...
      **Follow-up leg landed, 2026-09-06.** `EmptyStateReason` gains `"source-missing"` as its
      ...
      `database` icon. Both renderer classes push a primary `"Choose database"` action for the
      reason (`emptyState.chooseDatabase`, all three locales), wired to the same view-settings
      popover `group-relation-deleted`'s "Open view settings" already opens
      (`database-view.ts`'s `openViewSettingsAfterMutation`, `embedded-database-renderer.ts`'s
      inline `toggleHeaderPopover(config, "view", anchor)`); both reach
      `viewConfigPanelRenderer.render` and the real source-rules editor
      (`view-config-panel-renderer.ts`'s `renderSourceRules`), not a stub.
      **The predicate is not `sourceCount === 0`, and the first build of this row got that wrong.**
      `sourceCount` is the row pipeline's own tally of the records it was handed
      (`row-pipeline.ts`, `const sourceCount = rows.length`), so it cannot see the vault: a deleted
      folder and a folder that exists holding no notes yet are the same number. Routing that number
      straight to the new reason made a brand-new empty database read *"This view's source is
      missing"* with a **Choose database** button instead of an invitation to add the first record —
      the ADR's own third row wearing its first row's copy. `getEmptyStateReason` therefore takes a
      second argument: the caller says whether the configured source still resolves, and the
      predicate answers `"source-missing"` only then, keeping `"no-matching-data"` for every
      zero-row source still in the vault. Both classes compute it the same way
      (`isSourceFolderMissing`: database-level source folder over view-level, normalised through
      each class's own `normalizeVaultFolder`, then `dataSource.fileExists`), and a whole-vault view
      has no folder to lose, so it can never report the state. A missing source still wins the whole
      precedence chain ahead of search, filter and limit.
      **Green value:** `EmptyStateReason` holds **14** members; `EMPTY_STATE_COPY` holds **14**
      entries with no two reasons sharing a title/body signature (`empty-state-renderer.test.ts`);
      `rg -n "chooseDatabase" src/` finds the i18n key and its two wiring sites.
      **Distinctness proven three ways.** Unit, on the three zero-row cases: the same diagnostics
      answer `"source-missing"` when the source is gone and `"no-matching-data"` when it is there,
      a positive `sourceCount` answers `"no-matching-data"`, and all three titles differ from each
      other and from `"no-database"`'s (`empty-state-renderer.test.ts`). Collapsing the predicate
      back to reading the row count alone turns three of those tests red.
      `group-relation-deleted` is a separate condition entirely (`isBoardGroupFieldMissing`,
      unaffected by this predicate) and was already asserted distinct at its own landing. Lane: two
      permanent rows in `tools/live/render-assertions.mjs` —
      `table-empty-source-missing/file-view` and `table-empty-no-matching-data/file-view`
      (`render-assertion-bundle.mjs`'s `STATE_SCENARIOS`, `emptyReason` field) — mount the real
      `TableRenderer` over the mock-data catalogue's own columns at zero rows and drive the real
      `getEmptyStateReason` over one `RowPipelineDiagnostics`, varying only the source signal, then
      assert the rendered `data-empty-reason`
      (`render-assertion-harness.ts`'s `buildEmptyReasonOptions`/`emptyReasonAssertion`).
      **Negative control, observed red then green**: collapsing the predicate turned exactly the
      `no-matching-data` row red — `data-empty-reason="source-missing" (want "no-matching-data")`,
      lane exit 1 — while every other row stayed green; restoring returned the lane to exit 0.
      **Capture:** a new hand fixture, `empty-state-source-missing`
      ...
      map (Lucide's own icon, standing in for the runtime `setIcon` injection). All four PNGs were
      opened and read: the card, its title, its message and the "Choose database" button render in
      both themes on both devices. A full `npm run screenshots` took the corpus to **566** entries
      (this fixture's four, the only growth); `npm run screenshots:verify` reports 566 fresh, none
      blank or identical across themes. Two full runs of the identical page disagreed with each
      other on three captures' bytes and on six toast `layoutHash` values, in both directions — the
      capture nondeterminism this repository already records — so every one is restored to its
      committed value with the manifest reconciled to the files on disk. No capture's `pixelHash`
      moved anywhere in the corpus.
      **The touch-targets constructed ratchet does not move, and that is deliberate.** The two lane
      scenarios each mount a table whose summary footer draws one `db-table-footer-trigger` per
      column regardless of row count — 28 per scenario, 56 in total, every one a second copy of a
      control already counted at that same table's own scenarios and already this file's largest
      tracked shortfall since 2026-09-04. Measured both ways on this tree: 17852 elements over 77
      scenarios and 1030 under the floor with the probes measured, 17626 over 75 and **974**
      without, `db-table-footer-trigger` 229 against 173, no other class moving by one instance.
      A ratchet that rises whenever a scenario is added stops meaning what it says, so
      `touch-targets.mjs` reads scenarios carrying `emptyReason` past the constructed sweep and the
      baseline is re-pinned **down** to 974 — 1304 was main's number, and the rebase retired
      `db-calendar-month-segment` from the constructed corpus (330 to 0, no other class moving), so
      leaving it would allow headroom the tree no longer has. Nothing is exempted: the
      control keeps its 173 recorded instances, and resizing it stays the operator-scale call the
      baseline's own `why` field reserves
      (`tools/live/touch-targets-constructed-baseline.json`'s `emptyReasonScenariosNotRaised`).
      **Green:** `npx tsc --noEmit` 0; `npx vitest run` 1444/1444; `npm run build` 0;
      `node tools/live/render-assertions.mjs` exit 0, both new rows passing;
      `node tools/screenshots/verify.mjs` exit 0; `node tools/lane/check-lane.mjs` exit 0;
      ...
      `tools/live/render-assertions.mjs`, `tools/live/touch-targets.mjs`,
