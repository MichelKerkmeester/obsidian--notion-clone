...
STATE (2026-09-05 16:30): 0.0.26 shipped (8c7b65aa) carrying 006's record-open docking for every anchorless caller (ae46da94, §4 row 52). No device row is confirmed. Reference set: Anytype only — 120 set, 600 menu, 118 iOS captures. ALL SIX TRUE-UPS ARE DONE — 050's plus 051-055's (8e0149af, a58bbcd5, fbbddc13, 621de37f, ffcf434b+cd8030a8) — and reconciled together. That pass found 050's own read wrong five times: the chip rail IS captured (eleven times, conditional), the per-view default IS in the New menu, REQ-013's "one + New filter row" is the EMPTY state against twelve captured formats, the 60 page limit is per-layout (Gallery 60, Kanban 10, absent elsewhere), and hover WAS captured (37 menus). Three are one error: an absence asserted from a surface never examined. roadmap.md §7.10 settles four cross-family number conflicts and names one owner per shared primitive. 053's implementation runs on codex (worktree 094); screenshot regrouping is in flight — on a conflict keep ITS paths, your content.

ORDER OF WORK: 1) implement by family — 053 first (it carries 050 items 1, 2, 4, 7, 10, 12; codex holds it), then 052, 054, 051, 055 by inventory rank, 048 stacking and 044 grammar as constraints, a build after each; 2) the operator checks phone and desktop on 0.0.26; 3) 046's device rows and 050's remaining legs; 4) 007 children 003-004; 5) 047 rows 37/38. Each packet's T002 measures its own reds before its first code leg. Each phase gate-green and shipped; operator rows never ticked.
