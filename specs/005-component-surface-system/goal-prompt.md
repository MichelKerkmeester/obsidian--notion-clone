...
DONE WHEN: every phase goal.md's completion criteria are met or operator-owned; validate.sh --strict on both parents reports Errors 0 on the first RESULT line; npm run gate is 25 green read from $?; npm run replay holds with reversed 0; and every operator report in roadmap.md §4 is confirmed on device or deferred with the deferral recorded.
