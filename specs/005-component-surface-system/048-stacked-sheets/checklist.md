...
| C2 | Parent sheet bounding-box delta while a child is open | **unmeasured, and no code path reads the parent on child mount** — `setSheetMount` (`mobile-bottom-sheet.ts:274`) appends the child and touches nothing else. **measured 336.00px** on all 31 registered pairs against the pre-fix tree: the parent rose by the keyboard inset its child should have held | \|Δ\| ≤ 1px per edge | [x] **0.00px** on all 31, `sheet-grammar` stacked rows |
| C3 | Scrim nodes while two sheets are open, and where the node sits | **1, behind both** — `setScrim` (`mobile-bottom-sheet.ts:478`) reuses one `.db-mobile-sheet-scrim` for however many sheets are open, by design. **Measured: count 1 and `scrim between the top two sheets` red on all 31 pairs** | 1, **between** them | [x] 1, between them, on all 31 |
| C4 | Stacked children carrying a header with a 44px close | **0** — `createSheetHeader` and its equivalent are called by 5 surfaces (`filter-panel-renderer.ts:259`, `sort-panel-renderer.ts:113`, `toolbar-renderer.ts:1386`, `create-linked-view-modal.ts:59`, `view-config-panel-renderer.ts:388`), none of them a stacked child. **Measured: header, close-target, inset and title all red on all 31 pairs** | every stacked child | [x] all four green on all 31; `createSheetHeader` reaches K1, K3 (19 subclasses), K4 (3 modals) |
| C5 | `--db-mobile-sheet-bottom` on each sheet of a pair under a 336px keyboard | **each sheet holds whatever its own last placement wrote** (`popover-position.ts:406`); `keepSheetPlaced`'s own comment records `844 → 844 → 844` beside `844 → 508 → 844` under one declared keyboard. **measured with both surfaces re-placed under a declared 336px keyboard: red on all 31, the parent holding 336px** | top 336px, beneath 0px | [x] child 336px, parent 0px on all 31 |
| C6 | Parent scroll offset and draft value after a child is dismissed | **unmeasured**; `filter-panel-renderer.ts:472-475` rebuilds the panel node on rerender and `originalMount` (`mobile-bottom-sheet.ts:255`) points a closing child at whichever node was its parent when it opened. **Measured red separately: a press inside a stacked child was read as an outside press and closed the parent under the thumb** | both unchanged, by close and by drag | [x] partial — parent stays within 1px through a child drag on all 31, and `overlay-stack.test.ts` pins the press rule; `scrollTop` is not read back |
| C7 | Scroll affordance on a child taller than the viewport | **none** — the 14-property picker is cut mid-row at the sheet's bottom edge with no fade or scrollbar, per `../scratch/device-2026-09-05/stacked-filter-property-picker.png`. **Measured 985 > 424 with no fade** | fade or scrollbar at the cut | [x] **932 > 344 with the mask present** |
| C8 | `sheet-grammar` registry rows that are stacked pairs | **0 of 8** — every registered surface is a first sheet (`tools/live/sheet-grammar.mjs:46-68`) | one per pair, ≥ 1 at depth 3 | [x] **31 pairs**, three of them depth 3 |
| C9 | `npm run gate` exit status, with the stacking negative control observed red | **the same lane against the pre-fix tree: 253 failing assertions** | exit 0, control red then green | [x] `$?` **0**, 25 lanes green; control read from rendered style — parent opacity 0.88 → 1 → 0.88 |
