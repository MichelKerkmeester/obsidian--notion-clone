...
_memory:
  continuity:
    ...
    recent_action: "T004-T006 landed: timeline scale + captureData; 19/26 rows ticked"
    next_safe_action: "T002, T009-T012, T016 remain open"
    blockers:
      - "T019 (second dispatch leg) was skipped; the landing went straight from leg a to leg c"
      - "table and chart constructed captures still show untyped/no per-row data: table's harness bag renders cells through a stub the captureData option does not reach, and chart shows no per-row field to type"
    key_files:
