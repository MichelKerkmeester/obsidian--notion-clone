...
- [x] T030 Construct the fixture half of `goal.md` DONE row 6's residual (done-audit-10's re-scope): give every hand-written fixture without a `fixtureOf` a constructed counterpart through the shared bundle's production paths, or record why one cannot exist. Inventory first (51 fixtures, read out of `tools/screenshots/scenarios/*.mjs` by id — the families and the entry each state reaches):

| fixture (51 without fixtureOf) | production module | public entry that renders the state | disposition |
|---|---|---|---|
| chrome-toolbar | toolbar-renderer.ts | `ToolbarRenderer.render` | constructed-toolbar |
| chrome-view-switcher | toolbar-renderer.ts | `ToolbarRenderer.render` (view tabs; the ⋯ overflow tab is ResizeObserver-measured, not in a static mount) | constructed-toolbar (partial: overflow tab stays device-measured) |
| chrome-toolbar-search | toolbar-renderer.ts | `ToolbarRenderer.render` with `DatabaseViewState.searchText` set | constructed-toolbar-search |
| chrome-utilities-popover | toolbar-renderer.ts | the More-tools button's own onclick (`renderUtilitiesOverflowButton`) | constructed-toolbar-utilities |
| chrome-active-view-controls | active-view-controls-renderer.ts | `ActiveViewControlsRenderer.render` over filter/sort rules | constructed-active-view-controls |
| chrome-active-rule-popover-filter | active-rule-popover-renderer.ts | `ActiveRulePopoverRenderer.toggleFilter` → `FilterPanelRenderer.renderSingleRuleEditor` | constructed-active-rule-filter |
| chrome-active-rule-popover-sort | active-rule-popover-renderer.ts | `ActiveRulePopoverRenderer.toggleSort` → `SortPanelRenderer.renderSingleRuleEditor` | constructed-active-rule-sort |
| chrome-table-footer | table-renderer.ts / table-footer-renderer.ts | `TableRenderer.renderTable` with `config.summaryRules` (the footer renders inside it) | constructed-table-footer |
| chrome-group-header-row | table-renderer.ts / group-label-renderer.ts / summary-renderer.ts | `TableRenderer.renderGroupedTable` with a two-level group tree | constructed-table-grouped |
| chrome-summary-row | summary-renderer.ts | `SummaryRenderer.render` with the draggable-items onChange hook | constructed-summary |
| chrome-owned-menu | owned-menu.ts / menu-row.ts | `createOwnedMenu` + handle `addRow`/`addSection`/`showAt` | constructed-owned-menu |
| chrome-owned-menu-sheet | owned-menu.ts / mobile-bottom-sheet.ts | the same `showAt`, whose phone branch applies the sheet chrome | constructed-owned-menu (mobile device pass) |
| chrome-group-selection-controls | list-renderer.ts / gallery-renderer.ts / board-renderer.ts | `ListRenderer.renderGrouped` + `GalleryRenderer.renderGrouped` + the extensions board's column-header checkbox (the fixture's board-subgroup box no longer exists on the shipped board — the port's swimlane lane header carries no box) | constructed-group-selection-controls |
| chrome-board-extensions-selection | board-renderer.ts | `BoardRenderer.render` with `boardExtensionsEnabled` | constructed-board-extensions |
| chrome-selection-status-bar | embedded-database-renderer.ts | none — the host extends `MarkdownRenderChild` (the shared obsidian stub refuses it) and the selection state exists only mid-gesture | cannot (stub out-of-scope host + mid-gesture state) |
| panel-filter-conditions | filter-panel-renderer.ts | `FilterPanelRenderer.render` over a flat AND group | constructed-filter-panel |
| panel-filter-nested-group | filter-panel-renderer.ts / view-filter-tree.ts | `FilterPanelRenderer.render` over a tree with a NOT node and an inner OR group | constructed-filter-panel-nested |
| panel-sort-rules | sort-panel-renderer.ts | `SortPanelRenderer.render` over two rules | constructed-sort-panel |
| panel-sort-calendar-empty | sort-panel-renderer.ts | `SortPanelRenderer.render` with `viewType: "calendar"` and no rules | constructed-sort-panel-calendar |
| panel-view-config | view-config-panel-renderer.ts | `ViewConfigPanelRenderer.render` with a one-view database | constructed-view-config |
| panel-column-manager | column-manager-renderer.ts | `ColumnManagerRenderer.render` with one hidden column | constructed-column-manager |
| panel-record-detail | record-detail-panel.ts / card-field-renderer.ts | `openRecordDetailPanel` (note body absent: mounting it requires the `readNoteBody` action, which constructs an obsidian Component the shared stub refuses) | constructed-record-detail (partial: body region stays fixture-only — real MarkdownRenderer has no standalone build) |
| panel-record-detail-sheet | record-detail-panel.ts / popover-position.ts / mobile-bottom-sheet.ts | the same `openRecordDetailPanel`, whose phone placement applies the sheet chrome | constructed-record-detail (mobile device pass, same partial) |
| panel-record-detail-sheet-body-editing | note-body-region.ts | `mountNoteBodyRegion` + the handle's `beginEdit` | constructed-record-detail-body-editing |
| panel-record-detail-sheet-body-empty | note-body-region.ts | `mountNoteBodyRegion` over an empty body | constructed-record-detail-body-empty |
| panel-record-peek | table-record-peek.ts / table-renderer.ts | `openTableRecordPeek` docked beside a real table | constructed-record-peek |
| panel-computed-cleanup-modal | views/modals/computed-frontmatter-cleanup-modal.ts | none — `DbModal` extends obsidian `Modal`, which the shared stub refuses (out-of-scope throw) | cannot (stub out-of-scope host) |
| panel-invalid-events-modal | views/modals/invalid-time-events-modal.ts | same | cannot (stub out-of-scope host) |
| panel-base-import-modal | views/modals/base-import-confirm-modal.ts | same | cannot (stub out-of-scope host) |
| field-cell-edit-text | cell-renderer.ts | `CellRenderer.renderCell` + `startEdit` (the entry `database-view.ts` wires into `editCell`) on a markdown text cell and a number cell | constructed-cell-editor-text |
| field-cell-edit-select | cell-renderer.ts / column-types.ts | the same `startEdit` on a select cell | constructed-cell-editor-select |
| field-date-value-picker | date-value-picker.ts | `renderDateValuePicker` + clicking its own trigger | constructed-date-picker |
| field-date-value-picker-datetime | date-value-picker.ts | the same with `includeTime` | constructed-date-picker-datetime |
| field-icon-picker | icon-picker-popover.ts | `openIconPickerPopover` with a lucide current token (opens the Icons tab) | constructed-icon-picker |
| field-option-color-picker | option-color-picker.ts / status-colors.ts | `openOptionColorPicker` | constructed-option-color-picker |
| field-relation-values | relation-value-renderer.ts | `renderRelationValue` with no App (the module's documented no-vault mode; the unresolved-target variant needs a live metadata cache) | constructed-relation-values (partial: unresolved variant stays fixture-only) |
| field-file-fields | file-field-renderer.ts | `renderSpecialFileFieldValue` for `file.tags` and a link-list key | constructed-file-fields |
| field-number-displays | number-display-renderer.ts | `renderRating` / `renderProgress` / `renderProgressRing` | constructed-number-displays |
| field-record-icon | record-icon-renderer.ts / table-renderer.ts | the table's icon gutter via `showRecordIcon` + a real `renderRecordIcon` bag member (lucide token variants need Obsidian's `getIconIds` and degrade to the default fallback) | constructed-record-icon (partial: lucide variants stay fixture-only) |
| field-status-colors | cell-renderer.ts / status-colors.ts | the table renderer over a sixteen-colour option palette | constructed-status-colors |
| add-view-popover | toolbar-renderer.ts | the view-tab plus button's own onclick (`showAddViewMenu`) | constructed-toolbar-add-view |
| dropdown-field | dropdown-field.ts | `openDropdownMenu` | constructed-dropdown |
| table-column-header | column-header-controller.ts | `ColumnHeaderController.setup` (the call `database-view.ts` wires into `setupColumnHeader`) | constructed-column-header |
| board-empty-column | board-renderer.ts / group-visibility.ts | `BoardRenderer.render` with `withEmptyOptionGroups` backfilling a zero-row configured option | constructed-board-empty-column |
| board-drop-language | board-renderer.ts | none — the drag classes exist only while a pointer gesture is live (`dragstart`/`dragover` handlers add them) | cannot (mid-gesture state) |
| empty-state | empty-state-renderer.ts | `EmptyStateRenderer.renderCard` (the fixture's hero/card composite is not a shape any single renderer emits) | constructed-empty-state |
| card-cover-states | board-renderer.ts / gallery-renderer.ts | both renderers' cover paths with an image field the rows resolve nothing for (empty row cache — the only no-vault cover state) | constructed-card-covers |
| timeline-view-day | calendar-timeline-renderer.ts | `CalendarTimelineRenderer.renderTimeline` at `scale: "day"` | constructed-timeline-day |
| timeline-view-month | calendar-timeline-renderer.ts | the same at `"month"` | constructed-timeline-month |
| timeline-view-quarter | calendar-timeline-renderer.ts | the same at `"quarter"` | constructed-timeline-quarter |
| timeline-view-year | calendar-timeline-renderer.ts | the same at `"year"` | constructed-timeline-year |

Per family: panels 11 constructed / 3 cannot; chrome 14 constructed / 1 cannot; fields 11 constructed / 0 cannot; the two popovers constructed; the nine leftovers 8 constructed / 1 cannot — 46 constructed (4 with a documented partial bound) and 5 cannot, each reason recorded in the table. RED FIRST: the new marker assertions (one per constructed state, all real production classes) run against the pre-change harness (`git stash` on `render-assertion-harness.ts` only, the new cases kept) — `constructed-state-assertions: FAIL — 42 failure(s)`, exit 1; 39 were the new cases' markers (every one false on the old harness, which routed unknown renderer values into the table branch), and 3 were the pre-existing `constructed-board-subtask` "on" failures present at HEAD on this tree (main's board port moved the subtask tree's markers to the extensions card path; the paired case now constructs `boardExtensions` for its "on" side, the board the current product actually draws the tree on — after that correction it passes, so the green run covers the new cases alone). Constructed: `render-assertion-harness.ts` gained 26 new `ScenarioSpec.renderer` values (toolbar, active-view-controls, active-rule-popover, filter-panel, sort-panel, view-config, column-manager, record-detail, record-detail-body, record-peek, summary, owned-menu, cell-editors, date-picker, icon-picker, color-picker, relation-values, file-fields, number-display, record-icon, dropdown, empty-state, column-header, group-selection-controls, card-covers) plus additive options on the existing board/gallery/table branches (boardExtensions, boardImageField, boardEmptyColumn, galleryImageField, tableGroups, tableFooter, fullStatusPalette, recordIconColumn, columnHeaderController, longHeaderLabel) and on the toolbar branch (toolbarPopover, searchText); every state reaches production through the classes' own public render/open/toggle entries against real anchors, the toolbar's own trigger-button onclick handlers, or the field editors' real `startEdit` — never a hand-applied class. `constructed-scenarios.mjs` registers 43 new capture scenarios (46 fixture supersessions, three of them shared ids for a phone-sheet twin), all with `fixtureOf` naming the superseded fixture; the anchored/fixed surfaces declare `capture: "viewport"` per the established pattern. `manifest-schema.mjs`'s `CONSTRUCTED_RENDERERS` and `CONSTRUCTED_SCALES` (quarter/year for the timeline) extended. The 43 new specs went into `STATE_SCENARIOS` so touch-targets/unstyled-links measure them; they stayed out of `SCENARIOS` exactly as T029 recorded — `render-assertions.mjs`'s `BAGS` has no key for the new renderer values (or for the reused renderer/bag pairs under state options), so a merged list would throw on `BAGS[key]` instead of failing a check. One shared-seam addition: the obsidian DOM shim now installs the global `createDiv` Obsidian declares (the markdown toolbar's one bare call site), additive and unused by the three existing consumers. GREEN (what the assertions prove): `node tools/live/constructed-state-assertions.mjs` — after the harness change, exit 0, all new single-mount cases plus the paired cases PASS (69 PASS lines; the 3 pre-existing board-subtask failures corrected as above). `node tools/live/render-assertions.mjs` — exit 0, 21 scenarios, coverage 7 of 22 (published 7), same shape numbers as HEAD. `npx tsc --noEmit -p .` exit 0; `npx vitest run` 98 files / 981 tests exit 0 (the registry and fixtureOf snapshot expectations in `constructed-capture.test.mjs` updated to the 62 constructed ids / 66 declarations); `npm run lint` reports the same 172 problems as HEAD (src untouched); `npm run lint:tools` exit 0; `node tools/naming/scan-comments.mjs` exit 0. Not ticked past what the assertions prove: the 5 cannot fixtures stay fixture-only by design (recorded above), and the touch-targets constructed baseline (422) needs the in-runtime leg's re-measurement and raise once the 43 new STATE_SCENARIOS entries join that lane's page. IN-RUNTIME LEG (capture, read, lanes, gate — run on this worktree, every number re-measured here rather than carried): the claims above re-verified first — `npx tsc --noEmit` exit 0, `npx vitest run` 98 files / 981 tests exit 0, `npm run lint` 172 problems (byte-identical to HEAD by construction: no `src/` file is touched), `npm run lint:tools` exit 0, `node tools/naming/scan-comments.mjs` exit 0, `constructed-state-assertions.mjs` exit 0 with 69 PASS / 0 FAIL, `render-assertions.mjs` exit 0 at the unchanged 21 scenarios. The five `cannot` verdicts re-checked against source and all hold: `obsidian-stub.mjs` throws out-of-scope for `Modal` (the three DbModal-hosted panels), `chrome-selection-status-bar`'s host is `EmbeddedDatabaseRenderer` extending `MarkdownRenderChild` with a mid-gesture selection state, and `board-drop-language`'s classes are added by live `dragstart`/`dragover` handlers. FIVE DEFECTS the assertions did not catch, four of them found by reading the pictures: (1) two new `STATE_SCENARIOS` entries (`board-covers`, `gallery-covers`) threw `TypeError: Cannot read properties of undefined (reading 'metadataCache')` in `resolveCoverImage` and took `touch-targets.mjs`/`unstyled-links.mjs` down with them — the card-covers branch set an empty row cache but the board/gallery cover options did not; both now go through one `applyEmptyMetadataCache` helper. (2) Seven table-rendering scenarios measured 80,130px tall containers, which no element-mode capture can photograph — the table branch now honours `captureData` for its row count the way list, board and gallery already did, and the four element-mode table captures became possible at all. (3) The constructed captures drew rows of `◆` placeholders: a runtime census of every scenario found 62 icon ids the toolbar, its popovers, the anchored panels and the field editors reach that the shim's 21-icon set (traced when only the view renderers mounted) did not carry; all 62 added, census re-run to zero placeholders. (4) `constructed-timeline-day` was pixel-identical to `constructed-timeline` — production's own `normalizeTimelineDayScale` rewrites the day scale to week whenever the timeline's date field is a plain `date` column, so the capture showed the week scale under a day name; the branch now gives the field the `datetime` type and the times the scale requires, and the capture reads `February 2 2026` with hour columns and the Day chip active. (5) `constructed-board-empty-column` showed five populated lanes and not the empty one (`withEmptyOptionGroups` appends, and a horizontally-scrolling board crops), and `constructed-table-footer` put its footer below the phone fold; both reframed and recaptured. Two harness defects fixed on the way: the anchored surfaces that portal to `document.body` were never torn down, so each scenario measured every earlier scenario's panel as well (ten stacked panels by the tenth) — which made the touch-target lane non-deterministic at 1457 / 1471 / 1558 / 1584 / 1601 / 1623 across six runs of one tree, and the pre-existing 31 scenarios alone at 419 / 429 / 429 against a recorded 422, i.e. the ratchet main already carries was decided by timing; the runner now sweeps every body child a mount added, and three passes over one page plus two independent lane runs all return the same total. The harness's own hidden popover anchors were `<button>`s counted as production controls; they are spans now and the three T028 wrote inline fold into the same helper. Two branches were rebuilt to stop hand-writing production markup: the cell editors now render through `TableRenderer` (the `db-cell db-editable-cell` classes are `CellRenderer.renderCell`'s decision, not the harness's), and the column header wires `ColumnHeaderController.setup` into the renderer's own `setupColumnHeader` action, which also restored the property-type icon the hand-built header omitted. LANES: touch-targets constructed 31 scenarios / 422 under floor -> 73 scenarios / 1278, baseline raised to 1278 with per-class attribution measured per `ScenarioSpec` in `touch-targets-constructed-baseline.json` (407 for the corrected pre-existing set + 871 new; the largest single population is 558 `db-multi-select-remove`, a real product finding recorded in that file's `productFindings` and not fixed here, alongside the 10x14 `db-file-tag-remove`, the column manager's 48 `clickable-icon` plus 32 unclassed 24x24 buttons, the 18x18 colour swatches and the view-config panel's 13x13 checkbox inputs); unstyled-links 31 scenarios / 72 links -> 73 / 1476, still 0 user-agent link colours. CAPTURES: 528 entries, 172 new (43 ids x 2 devices x 2 themes) and 17 pre-existing constructed captures moved by the icon set and the table row count; every one of the 172 opened and read beside its `fixtureOf` fixture, and `css-lane.json` carries a release naming all 189 (`styles.css` untouched, `baselineHash` unchanged, `check-lane` red before the entry and green after). Twenty-two further captures moved bytes without moving pixels and were restored to HEAD. `npm run gate` 25 green, 0 red, no exemption. RESIDUAL: the 5 `cannot` fixtures stand; `constructed-card-covers` and `constructed-group-selection-controls` frame only their first host (three real grouped renders do not fit one viewport and an element capture crops to it — the other hosts stay proven by the state assertions, and both scenario notes now say so); the icon picker's grid reads `No results` because the shim has no icon registry, so that capture shows the picker's chrome and colour strip but not a populated grid; and the rating styles cannot distinguish filled from outline through a single stroked stand-in glyph. Post-rebase reconciliation (onto main's gantt one-to-one port, board fidelity pass and board closing fixes): re-measured directly on the merged tree rather than carried forward — touch-targets constructed `under` dropped 1278 -> 1223 (stable across three independent runs), exactly the three local timeline-nav classes main's gantt port already retired from the default render (`db-timeline-create-button` 6 + `db-timeline-nav-button` 42 + `db-timeline-scale-menu` 7 = 55; 1278 - 55 = 1223), since the timeline's default render now mounts the reference gantt tree instead of the local markup this branch's own raise measured against; every other class held its exact prior count. The harness's own "the timeline drew its subtask tree" assertion (`subtaskTreeAssertion`, kind `"timeline"`) was updated in step to the reference gantt's own vocabulary (`.pm-collapse-toggle`, `.pm-gantt-label-progress`, indentation via `.pm-gantt-label-row[data-task-id]` padding-left) since it no longer renders the retired local `db-subtask-event-toggle`/`db-timeline-subtask-progress`/`data-subtask-depth` markers by default; `constructed-state-assertions.mjs` exit 0 after the fix (was exit 1, 3 failures, before it). unstyled-links constructed link total unchanged at 1476 (link colour is unaffected by which markup renders the timeline's controls). Manifest re-merged per entry by owner (main's board/timeline entries it changed since the branch's base, this branch's 189 reviewed constructed entries) then regenerated fresh via `npm run screenshots` on the merged tree — 0 content changes against the manual merge, confirming `styles.css` stayed untouched by this branch. Recorded in `touch-targets-constructed-baseline.json`'s new `ganttPortMergeReconciliation` entry. `npm run gate` 25 green, 0 red on the merged tree.
<!-- /ANCHOR:phase-2 -->
