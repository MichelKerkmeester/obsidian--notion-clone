...
| AC-011 | — | **Given** any desktop dropdown, **When** the operator clicks it, **Then** the trigger becomes an active text input and typing filters the list, regardless of option count | `tasks.md` T016; a fixture with ≤ 8 options asserting search still activates (today it would not, per the `> 8` gate). **Today: RED** — `dropdown-field.ts:193` gates search on `options.length > 8`, and the trigger never becomes an input; search, where it exists at all, is a separate field inside the popover | Unmet | - |
| AC-012 | — | **Given** the filter/sort condition row, **When** the Operator dropdown opens, **Then** its popover's left edge aligns with its trigger's left edge | `tasks.md` T017, a placement assertion. **Today: RED, measured on the operator's screenshot** — popover x 123-489, trigger x 290-480, left edges 167px apart | Unmet | - |
