...
_memory:
  continuity:
    ...
    last_updated_at: "2026-09-06T02:10:00Z"
    last_updated_by: "t010-t012-verify-and-land"
    recent_action: "Landed T010-T012 on the picker host after rebasing onto T007; census 63/37"
    ...
    blockers:
      ...
      - "T008-T009 (toolbar-renderer.ts, column-menu.ts) land in a sibling worktree, not this one"
    key_files:
    ...
    answered_questions:
      ...
      - "Can the relation editor's listbox rows move onto menu-row.ts: NO — role=option/aria-selected conflicts with the builder's menuitem/menuitemcheckbox grammar"
---
...
|-------|-----|---------------------|--------------|--------|--------|
| AC-001 | REQ-001 | **Given** a menu row with `submenu: true`, **When** it is activated by pointer click, `ArrowRight`/`Enter`, hover on a hover-capable pointer, or tapped on a phone sheet, **Then** a nested menu produced by the same factory opens, registered in `overlayStack` with the row's menu as `parentId`; **and When** Escape fires, **Then** only the innermost closes | **Landed 2026-09-05, verification partial.** `owned-menu.ts`'s `addRow` gained `buildSubmenu`; all four activation paths and innermost-only Escape are driven in `owned-menu.test.ts` against a mock DOM (green; stashing the change reproduces 6/7 red), and `sheet-grammar.mjs` stays green (564/0). Two clauses are **not yet met as written**: the proof is a `vitest` unit suite, not a `tools/live` lane row; and only the phone submenu registers with `overlayStack` (automatically, through existing sheet-chrome registration) — the desktop submenu is coordinated through the same closure-based dismissal every desktop `createOwnedMenu` already uses, not a separate `overlayStack` registration, to avoid a second dismissal system for one surface family. See `tasks.md` T004 | Unmet | - |
| AC-002 | REQ-002 | **Given** the family census, **When** the legs complete, **Then** zero hand-built `db-menu-item` constructions remain outside `menu-row.ts` **or** every remaining site is individually dispositioned in `componentization-plan.md` §1-§2 with a written reason | `grep -rn "db-menu-item" src/views/*.ts \| grep "cls"` against the baseline **70 outside `menu-row.ts`** — `toolbar-renderer.ts` **44**, `column-menu.ts` 19, `dropdown-field.ts` 4, `cell-renderer.ts` 3 — and **76** including `menu-row.ts`'s own 6; each survivor's plan row cited. Today: **63** — re-measured 2026-09-05 after T007 migrated `showViewTabMenu` onto `createOwnedMenu` (`toolbar-renderer.ts` 37, down from 39 — the census counts hand-built-row definition lines, and deleting `renderViewTabPopoverRow`/`renderViewTypeChangeRow` removed one shared `cls` line each even though nine rows moved to `addRow`), `column-menu.ts` 19, `dropdown-field.ts` 4, `cell-renderer.ts` 3; **69** including `menu-row.ts`'s own 6. The 70/76 and 65/71 pairs above are the pre-T007 counts. **The earlier 71/45 figures were wrong** (`design-trueup.md` C8, re-measured 2026-09-05); `checklist.md` C2 carries the current number | Unmet | - |
| AC-003 | REQ-003 | **Given** the picker family, **Then** exactly one active-picker registry, one phone sheet-header construction, one shared search, one geometric grid navigator exist in `src/views/`, and the five pickers consume them | `grep -c "activePickers = new WeakMap"` → 1 (baseline **3**: `date-value-picker.ts:71`, `option-color-picker.ts:29`, `icon-picker-popover.ts:50`, verified 2026-09-05); navigator count → 1 (baseline **2**: `getColorNavigationTarget` at `option-color-picker.ts:138`, `getIconNavigationTarget` at `icon-picker-popover.ts:284` — line numbers corrected from ADR-003's `:130`/`:281`); header sites → 1 (baseline **5 in the family**, of 14 tree-wide). Each picker's captures re-taken and read | **Progress 2026-09-05 (T012).** The registry and the navigator clauses are both **Met**: `grep -c "activePickers = new WeakMap" src/views/*.ts` → 1 (`popover-host.ts`, the date/colour/icon pickers all consuming it); `grep -c "function getColorNavigationTarget\|function getIconNavigationTarget" src/views/*.ts` → 0, replaced by one `getGridNavigationTarget`, oracle-tested against both legacy grid shapes in `popover-host.test.ts` before either picker migrated. The header clause is **narrowed, not closed**: `mountPickerSheetHeader` is the one construction the three pickers now share, but `owned-menu.ts` and `dropdown-field.ts` still build their own — 4 sites in the family, not 1. The shared-search clause is **unmet for two of the family's five surfaces** by design rather than omission: the relation editor's virtualised list and the icon picker's tab/category-scoped catalogue do not fit `filterPickerRows`'s toggle-visibility shape (`checklist.md` C6). "The five pickers consume them" therefore still reads Unmet as a whole — three of five (date, colour, icon) now share the registry, header and navigator; the dropdown and the relation editor do not, for stated reasons | Unmet | - |
...
| AC-011 | — | **Given** any desktop dropdown, **When** the operator clicks it, **Then** the trigger becomes an active text input and typing filters the list, regardless of option count | `dropdown-field.test.ts`'s twelve cases, four of them on the trigger swap and one on a five-item list filtering. **Met 2026-09-06** — the count gate is now the phone sheet's alone; on a desktop `createDropdownField` replaces its trigger with the query input in the trigger's own slot (focused, the current value as placeholder, the row's layout classes carried over) and `openDropdownMenu`, whose anchor the caller owns, renders its query field first in the panel for every list. Two negative controls observed: re-adding the count gate turns the five-item menu case red; removing the trigger swap turns four combobox cases red. Photographed in `constructed-dropdown-search-desktop-*` (the trigger mid-search) and `constructed-dropdown-desktop-*` (a three-item menu with its search row). **The panel-first half for menus is a named deviation** from the ruling's literal words, recorded in ADR-006 | Met | - |
| AC-012 | — | **Given** the filter/sort condition row, **When** the Operator dropdown opens, **Then** its popover's left edge aligns with its trigger's left edge | `dropdown-field.test.ts`'s two anchoring cases, driving the real `positionToolbarPopover` against a mock DOM. **Met 2026-09-06** — the call passes `align: "left"`; the popover's left edge measures 290px against the pre-fix 200px on the operator's own trigger geometry, and a trigger hard against the viewport's right edge still clamps inside it | Met | - |
