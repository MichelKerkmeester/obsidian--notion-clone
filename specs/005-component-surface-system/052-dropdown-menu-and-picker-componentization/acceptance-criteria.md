...
_memory:
  continuity:
    ...
    last_updated_by: "t003-t006-verify-and-land"
    recent_action: "Landed T003-T006 on main; census re-measured to 65 and the registry cut"
    next_safe_action: "Execute T007, the view-tab context menu migration, onto the primitive"
    blockers:
...
| AC-002 | REQ-002 | **Given** the family census, **When** the legs complete, **Then** zero hand-built `db-menu-item` constructions remain outside `menu-row.ts` **or** every remaining site is individually dispositioned in `componentization-plan.md` §1-§2 with a written reason | `grep -rn "db-menu-item" src/views/*.ts \| grep "cls"` against the baseline **70 outside `menu-row.ts`** — `toolbar-renderer.ts` **44**, `column-menu.ts` 19, `dropdown-field.ts` 4, `cell-renderer.ts` 3 — and **76** including `menu-row.ts`'s own 6; each survivor's plan row cited. Today: **65** — re-measured 2026-09-05 after this leg rebased onto `053`, which moved five `toolbar-renderer.ts` rows onto `toolbar-primitives.ts` (39, not 44); **71** including `menu-row.ts`'s own 6. The 70/76 pair below was the pre-rebase count. **The earlier 71/45 figures were wrong** (`design-trueup.md` C8, re-measured 2026-09-05); `checklist.md` C2 already carried the right number | Unmet | - |
