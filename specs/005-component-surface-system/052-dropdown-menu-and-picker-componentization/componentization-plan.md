...
| P2 | Cell option editor (select/status/tag values) | `cell-renderer.ts:1123` `editOptionPopover` (`db-cell-option-popover`) | Select picker + menu row builder | The option rows (`:1270-1420`, 3 hand-built `db-menu-item` sites), drag-reorder and the colour-dot slot keep their behaviour but render through the shared row/host; its **add-option row stays** (G11) and becomes the convention's reference implementation. **Landed 2026-09-05 (T010).** The row shell and its checked state now build through `menu-row.ts`'s `createMenuRow`; the drag handle, reorder buttons and colour dot are spliced in front of the label, unchanged. The `"✓"` text node is deleted and rebuilt as a `setIcon` glyph carrying `role="menuitemcheckbox"`/`aria-checked` on the row (G14) | G11, G14 — `menus/anytype-menu-cell-multiselect-dark.png` (chips + **2 × 3-dot drag handles at the 16px inset**, 32px pitch, 24px chip) and `menus/anytype-menu-object-featured-tag-dark.png` (search + the create-instruction empty state *"Type to create a new option"*) | Drag-reorder; the option-commit transaction flow; the inline-editor boundary (`003` §9) |
| P3 | Relation/object picker | `cell-renderer.ts:899` `editRelationPopover` (`db-relation-popover`, width 360/420/520 at `:1096`) | Picker host | Search + windowed list + footer move onto the host; its checkmark becomes the shared icon (G14); width mapped per REQ-007 (a `panel`-role picker, not a bespoke 520). **Landed 2026-09-05 (T011), one clause narrowed.** The width is now `popover-host.ts`'s `RELATION_PICKER_POPOVER` role. The checkmark was already an icon at the row's trailing edge before this leg — G14's leading/trailing finding named the option editor and the dropdown field, not this row, so nothing moved there. **The row itself stays hand-built**: this list is a `listbox` (`role="option"`/`aria-selected"`, and its own keyboard handler selects on `[role=option]`) where the row builder's rows are `menuitem`/`menuitemcheckbox` for a `menu` — a different, incompatible ARIA pattern. Search and the windowed list stay this surface's own for the same reason `checklist.md` C6 already gives: virtualisation re-renders a window rather than toggling a fixed row set's visibility, which is what the host's shared filter does | G10, G14 — `menus/anytype-menu-cell-object-dark.png` (300×300, per-row icon + right-aligned source label) and `menus/anytype-menu-object-more-add-link-to-object-dark.png` (≈360 child: search, `＋ Create Object` **first**, then the list). The tick moves **trailing** (G14), so this pair's `sheet-grammar` selectors update in the same leg | Windowed list; multi-select footer; target-database resolution; the row's own `option`/listbox semantics |
| P4 | Date/time picker | `date-value-picker.ts:141` `renderDateValuePicker` → `openDateValuePicker`; width 252 (`:420`) | Picker host | Onto the host: registry, phone header, width. **Presets stay** (Today/Tomorrow/Next week/Clear, `:164-166`) — they already match Anytype-style relative affordances; segmented Y/M/D + H:M inputs stay ours. **Landed 2026-09-05 (T012).** The `{anchor, close}` registry, the phone header and the `DATE_PICKER_POPOVER` width role are all `popover-host.ts`'s now; this picker was the one the shared registry's shape was written against, since it is the only one of the three that reads `anchor` back to decide whether a second click toggles or re-opens | G13 — `menus/anytype-menu-cell-date-dark.png` (288×271: `‹ May 2026 ›`, `Mo…Su` header, tinted weekends, `#4686FB` selected day, divider, footer `Today` `Tomorrow` left / `Clear` right). Our presets are that footer set plus `Next week`, in that placement | The segmented input; mini-calendar body |
| P5 | Colour picker (option colour) | `option-color-picker.ts:29` `openOptionColorPicker`; width 124 (`:119`) | Picker host | Onto the host: registry, header, geometric grid nav (one function with the icon picker's, `:130-173`). **Landed 2026-09-05 (T012).** Registry, header and `SWATCH_PICKER_POPOVER` width moved to the host; `getColorNavigationTarget` and `getIconNavigationTarget` collapsed into the host's `getGridNavigationTarget`. The selected swatch now carries a `setIcon` trailing tick (`mix-blend-mode: difference` so it reads against all sixteen fills); its `aria-label` already named the colour. The phone swatch grows from 18px to 44px (Fable review P1 #9, `checklist.md` C11) | G14 — `menus/anytype-menu-object-block-menu-color-dark.png`. **Anytype's colour picker is a 224px labelled list, not a grid**: ten named colours, leading dot, trailing tick. Adopted from it: the **trailing tick** and **named colours as accessible names**. Its shape is **declined** (ADR-005) | The 12-swatch grid; `048`'s registered `record option colour picker` pair |
