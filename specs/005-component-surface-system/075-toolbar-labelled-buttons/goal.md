...
- [x] Phone toolbar controls render as icon+label buttons matching the reference's measured size and spacing
  - Watched it red before green: reverting the `.is-phone` label rule to `display:none` reported every control bare, scrollWidth 398 / clientWidth 398, no horizontal overflow; restored, all six controls render icon+label.
