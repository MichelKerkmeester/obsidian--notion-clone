...
- [x] T015 Investigate why the shipped Title field / Title format rows were not found: affordance
      visibility, naming, or placement inside the view Settings ⚙ sheet rather than a more visible
      toolbar/menu location. Propose and land a fix (AC-012). Finding: the rows sat mid-sheet behind
      the generic Settings entry with no affordance naming them, and the Title format row's
      file-name-only visibility condition lived only in a docstring — the operator met a
      disappearing row. Fix landed, deliberately additive so it cannot collide with the parallel
      071/004 redesign of the same sheet: the board card's own overflow menu (`row-menu.ts`) gained
      a board-only `Card title` row — tap 1: the card menu, tap 2: the entry — whose jump
      (`DatabaseView.openCardTitleSettings`) opens the one canonical settings sheet
      (`toggleHeaderPopover("view")`, the settings button as anchor, exactly what the toolbar's own
      click would have stored) and scrolls the `data-config-row="title-field"` row under the thumb;
      `view-config-panel-renderer.ts`'s `renderSelect` gained an optional trailing `hint` (the
      conditional-colour summary's precedent) and the Title format row gained the marker plus a
      hint naming what it controls and the chosen-column-keeps-its-own-format condition;
      `i18n.ts` gained `menu.cardTitle` and `viewConfig.titleFormat.hint` in all three locales.
      Placement precedented, not referenced: `screenshots/notion/` still does not exist (goal D6),
      so the choice follows the packet's own evidence — the card context menu is the affordance a
      thumb finds (the Rename note precedent) and the marker seam is AC-004's (ADR-007). Red
      first: 3/17 failing across `row-menu.test.ts` (the two-tap path, source-shaped per that
      suite's precedent — OwnedMenu needs a document the suite does not have) and
      `view-config-panel-renderer.test.ts` (the hint + marker); green 17/17; reverted-line proof:
      the hint argument alone removed → 1/14 failed, restored. No `styles.css` change, so no
      css-lane triplet owed; the hinted surface's captures (constructed-board-card-properties,
      both themes both devices) moved as REAL and were reviewed by decoded pixel delta in both
      sampled runs (194,742@229, 187,600@209, 55,562@176, 55,566@196 — moved in both runs with
      byte-identical deltas, the lane's hash untouched). Battery from the final tree:
      `npx tsc --noEmit` 0, `npm run build` 0, `npx vitest run` 1745/1745, `sheet-grammar` /
      `render-assertions` / `touch-targets` 0, `verify-placement` 418/420 (2 declared),
      `evidence --check-all` 15/15 after re-running `capture-device-parity` and `sheet-rebuild`,
      `npm run gate` PASS 27 green 0 red (first run FAIL, 2 stale lanes regenerated by their own
      mechanics: operator-list — 071/004's reworded landing row — and css-lane — this leg's 2
      unnamed movers; second run PASS, exit 0 read from the file), `scan-comments` 0,
      `scan-failing-values` 0. Not pushed — a fresh verifier lands it
<!-- /ANCHOR:phase-5 -->
...
- [x] All tasks marked `[x]` — except T011 (the operator's own row, never ticked by an agent);
      T015 landed 2026-09-09 (AC-012)
- [x] No `[B]` blocked tasks remaining
- [x] Every acceptance criterion in `acceptance-criteria.md` is `Met`, `Waived` or `Superseded`
      — except the operator-only row (AC-008); AC-012 Met 2026-09-09
<!-- /ANCHOR:completion -->
