...
| AC-012 | REQ-006 | Given the shipped "Title field" and "Title format" rows in the view settings sheet, When the operator looks for "how to set a board card name + number format", Then they find it without asking again — a discoverability check, not a functional one, opened after the operator (2026-09-08) reported not finding a feature that AC-001..AC-011 already shipped | **Unmet.** The feature exists (AC-001..AC-011); the operator could not find it in 0.0.32. Candidate fixes: a more prominent affordance in the view's own toolbar/menu (not only inside Settings ⚙), an onboarding hint, or a naming change from "Title field"/"Title format" to language the operator would search for. Not yet investigated | Unmet | - |
