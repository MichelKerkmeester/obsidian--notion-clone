...
| AC-012 | REQ-006 | Given the shipped "Title field" and "Title format" rows in the view settings sheet, When the operator looks for "how to set a board card name + number format", Then they find it without asking again — a discoverability check, not a functional one, opened after the operator (2026-09-08) reported not finding a feature that AC-001..AC-011 already shipped | **Met 2026-09-09.** Red first: with the shipped sources untouched, the discoverability coverage failed 3/17 — `row-menu.test.ts`'s two-tap path (no `Card title` menu entry, no `openCardTitleSettings` implementation) and `view-config-panel-renderer.test.ts`'s hint row (no hint, no `data-config-row="title-format"` marker). Fix, deliberately additive for the parallel 071/004 redesign of this very sheet: the board card's own overflow menu carries a board-only **Card title** row (tap 1) whose jump opens the one canonical settings sheet — never a second picker (ADR-002) — and scrolls the `data-config-row="title-field"` row under the thumb (tap 2); the Title format row now carries a one-line hint naming what it controls AND its own visibility condition (a chosen column keeps its own format), which no control in the sheet otherwise announced. Green 17/17; a unit test fails against a reverted line: removing the hint argument alone failed 1/14, restored. Placement precedented, not referenced: `screenshots/notion/` still does not exist (goal D6), so the choice follows the packet's own evidence — the card context menu is the affordance a thumb finds (the Rename note precedent) and the marker seam is AC-004's. The hinted surface's captures (constructed-board-card-properties, both themes both devices) moved as REAL and were re-reviewed by decoded pixel delta across two sampled runs: 194,742/187,600/55,562/55,566 changed pixels at max channel deltas 229/209/176/196, every one moved in BOTH runs with byte-identical deltas, `styles.css` untouched (the lane's own hash agrees). Battery from the final tree: `npx tsc --noEmit` 0, `npm run build` 0, `npx vitest run` 1745/1745, `sheet-grammar`/`render-assertions`/`touch-targets` 0, `verify-placement` 418/420 (2 declared), `evidence --check-all` 15/15 after re-running `capture-device-parity` and `sheet-rebuild`, `npm run gate` 27 green 0 red (first run FAIL — the operator-list and css-lane lanes' regeneration mechanics, the 071/004 landing's reworded row and this leg's two unnamed movers; both re-run, second run PASS, exit 0 read), `scan-comments` 0, `scan-failing-values` 0. Whether the operator now finds it stays their device read | Met | - |
...
AC-008 is unchanged — the operator's alone. **Reopened again 2026-09-08**: the operator asked how
to set a board card name/number format on the same build that ships it — AC-012 records this as a
discoverability gap (the feature is shipped, but not found), tracked separately from the ten Met
rows above.

AC-012 closed 2026-09-09: the board card's own overflow menu gained a board-only **Card title**
entry whose second tap opens the one settings sheet scrolled to the Title field row (ADR-007), and
the Title format row now carries a hint naming both what it controls and the
chosen-column-keeps-its-own-format condition that hides it. Eleven of twelve rows now stand at Met;
