...
| AC-114 | — | **Given** a table with zero rows, **When** its footer renders, **Then** it does not render at all; **Given** a table with rows, **When** its footer renders, **Then** it is 44px | `tasks.md` T015. **Today: RED, measured** — 173 `+ Calculate` triggers at 26px on an empty phone table, all under the 44px floor | Unmet | - |
