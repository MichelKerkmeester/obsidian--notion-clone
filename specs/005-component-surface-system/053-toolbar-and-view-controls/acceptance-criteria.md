...
_memory:
  continuity:
    ...
    recent_action: "AC-113 Met: the gear is measured in place, labelled, and permanent at every width"
...
| AC-113 | — | **Given** the toolbar rail, **When** the operator looks for Settings, **Then** a gear icon opens it directly, positioned before the `···` overflow button | `tasks.md` T014. **Red, observed first**: `.db-toolbar-settings-btn` matched nothing and the collapse sweep's `settingsButtonVisible` reading was false at every width in the 250-900px sweep; Settings opened only through the `···` menu, one level deep. **Green, measured 2026-09-06** on the shipped toolbar in headless Chrome with the token sheets attached: the utilities cluster holds gear then `···` (x 832 vs 864 desktop, 144 vs 176 phone), the gear is a focusable `<button>` with `aria-label` "Settings" in all three locales, `aria-haspopup="dialog"` and `aria-controls="db-view-config-panel"`, 28x28 like every other rail control (this repository's enforced floor is 28px; neither touch-target ratchet moved), and the `···` menu lists no settings row | Met | - |
...
| AC-115 | REQ (2026-09-06 amendment) | **Given** a table view with the Wrap text switch off and no column wrap override, holding a long-text column with a multi-line value, **When** the table renders, **Then** every row's height equals `--db-row-height` for its density and the long-text cell is ellipsised to one line | **Today: red on the operator's ~10:25 desktop screen** — the Journal column lays out over **6** lines while its neighbours clip to one, so the row stands about six times the rhythm. Green is measured row height == `--db-row-height` across every row, with a negative control that goes red when the clip is removed | Unmet | - |
...
**AC-113 closed 2026-09-06 (the gear landing).** The rail gained one permanent
`db-toolbar-settings-btn` before `···`, measured on the shipped toolbar rather than on a fixture:
the utilities cluster holds exactly two children in that order on desktop and on a phone body, the
gear carries `aria-label` "Settings" in all three locales with `aria-haspopup="dialog"` and
`aria-controls="db-view-config-panel"`, and it is keyboard-reachable at `tabIndex` 0. Red first was
observed twice — the selector matched nothing, and `run-toolbar-collapse-sweep.mjs`'s new
`settingsButtonVisible` reading was false at every width before the control existed. **One thing
this row does not claim**: the gear is 28x28, the rail's own control geometry, so it clears this
repository's enforced 28px coarse-pointer floor and not WCAG 2.5.5's 44px, exactly like the `···`
button beside it. Neither touch-target baseline moved.
