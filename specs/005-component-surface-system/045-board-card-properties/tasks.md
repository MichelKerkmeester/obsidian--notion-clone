...
- [x] T013 Add a constructed scenario with a non-default properties list, so the feature has a photographed state rather than only a unit test (`tools/live/render-assertion-harness.ts`, `tools/screenshots/constructed-scenarios.mjs`) — done. A `viewConfigVariant: "board"` option on the harness's `view-config` case mounts the real `ViewConfigPanelRenderer` for a board view carrying `board-card-properties-panel.stories.ts`'s own `Editable` fixture verbatim (Hours/Due stored visible, Tags stored hidden, Status appended hidden), captured as `constructed-board-card-properties` (`tools/screenshots/scenarios/panels.mjs`'s new `panel-board-card-properties` is its hand-fixture pair, mirroring the row markup class-for-class minus the column manager's unused wrap/edit/delete tracks). One `render()` call forks on `isMobileBottomSheet`, so the same scenario photographs both the desktop popover and the phone sheet (`.db-mobile-reorder-controls` visible in place of the drag handle on `.is-phone`, confirmed in the mobile capture). A second option, `boardCardFieldsHidden` on the `board` renderer, seeds an explicit list that reproduces the derived default verbatim except for hiding the schema's first currency column, captured as `constructed-board-card-properties-hidden` — the card-level half of the pair, proving a stored list removes a field from a rendered card and not only from the panel that edits it. **Caught during the Critic pass, not before landing:** the first attempt at that second scenario omitted `boardExtensionsEnabled`, so it rendered through `renderReferenceCard`'s fixed five-slot map (REQ-007's own boundary) where `resolveBoardCardFields` is never called and a stored list changes nothing — the capture came back byte-identical to the plain `constructed-board` scenario, which is AC-004's proof, not this task's. Confirmed by a direct DOM count before recapturing (`stocks`: 18 occurrences across the extensions board without the option, 0 with it) and by reading both capture pairs side by side. Fixed by adding `boardExtensions: true`; both new scenarios' ids were added to `tools/screenshots/constructed-capture.test.mjs`'s two hardcoded arrays, and `tools/live/constructed-state-assertions.mjs` gained a paired case (`constructed-board-card-properties`, confirmed red against the pre-branch harness, green after) proving the panel lists exactly the four fields with Tags unchecked and Hours/Due checked. `sheet-grammar.mjs` measured the mounted board Properties sheet directly: `surface`/`handle`/`header`/`dropdown`/`keyboard` green, `rows`/`segmented` red for the same reason the already-registered-red settings sheet is red (sibling `.db-view-config-row`/`.db-toggle-switch` rows elsewhere in the same shared body — not this section's own `.db-column-manager-row`/`db-checkbox-field` markup, which is why `dropdown` is clean). `describeSheetGrammar` reads the whole mounted sheet with no way to scope to a section, and this section never mounts except inside that body, so it is documented in the lane's own comment rather than registered — registering would either fail the gate on a defect this section does not own, or require a harness-only "Properties as its own sheet" surface the operator never sees. AC-005 stays partially unmet for exactly that reason; see `acceptance-criteria.md`. **Residual closed this session:** with `050-settings-body-grammar` landed (`4f090d2e`, `07be64fe`), `board-card-properties` is now registered in `sheet-grammar.mjs`'s `REGISTERED_SURFACES` (`{ renderer: "view-config", bag: "file-view", captureData: true, viewConfigVariant: "board" }`) and measures 7/7 — `rows`/`segmented` moved from red to green with no change to this section's own markup, confirming the blocker was owned by the shared body as documented. `npm run gate` 25/25. AC-005 is now `Met`; see `acceptance-criteria.md`.
...
- [x] T015 Field names on the board card's meta grid, from the 0.0.36 operator report — *"Board cards should also show field name and not just value"* (`styles.css`, `tools/live/render-assertions.mjs`). The shared card renderer already emits each field's name span from the column's schema label; the kanban meta block hid it. Un-hidden: 12px in the muted token before the value, the value keeping `tabular-numeric` digits; the property grid two columns at a 360px-or-wider viewport and one below; the value clamp one line so the measured uniform 25px row pitch survives; chip rows span the full meta row so a wrapped badge row cannot stretch its grid neighbour (caught mid-implementation by the row-pitch pin, 25/32/48). RED: render-assertions exit 1 — 36 of 306 labels visible, 13px/`rgb(108,111,116)`, digits `normal`, 1 column at 1440px and 340px. GREEN: exit 0 — 306 of 306, 12px/`rgb(154,155,158)`, `tabular-nums`, 2 columns at 1440px / 1 at 340px, row pitch 17×25px. Screenshots ×2 (480 entries, exit 0): 60 board-card movers, both runs, identical counts, no jitter; css-lane triplet `045-board-card-properties`, baseline 4261be904bfb → 7c46f607d7f6, release names all 60. AC-008; see its evidence in `acceptance-criteria.md`.
<!-- /ANCHOR:phase-3 -->
...
- [x] All tasks marked `[x]` — T013 landed with a sheet-grammar residual tracked against AC-005; that residual closed this session (registered, 7/7)
