...
| AC-008 | REQ-009 | **Given** a board card whose Properties list makes property fields visible, **When** the card renders, **Then** every property row in the card's meta grid shows its field's display name beside the value — 12px in the muted token, before the value, on phone and desktop; the property grid is two columns at a 360px-or-wider viewport and one below; the measured uniform 25px row pitch, the title, description and type-name slots, and the value's tabular-numeric digits are unchanged. **RED:** 36 of 306 meta fields showed a name (the shared renderer's label span was hidden); the label computed 13px/`rgb(108,111,116)`; `font-variant-numeric` read `normal`; the meta grid was one column at 1440px and 340px. **GREEN:** 306 of 306; 12px/`rgb(154,155,158)`; `tabular-nums`; 2 columns at 1440px, 1 at 340px; row pitch 17×25px. | `tools/live/render-assertions.mjs` — the board-geometry pass's `field label size` pin plus the `board card field names` block (name count, label style, value digits, meta-grid columns at two viewports), run against the bundled production renderer with the token sheets attached | Met | - |
...
**AC-008 evidence (landed 2026-09-09 against 0.0.36's report):** RED, `node tools/live/render-assertions.mjs` exit 1 — `36 label(s) visible of 306 meta field(s)`; label style read `13px / rgb(108,111,116)` against the muted token `rgb(154,155,158)`; the value's `font-variant-numeric` read `"normal"`; the meta grid computed 1 column at both 1440px and 340px; the geometry pin `field label size` read `"13px"` against its expected `"12px"`. GREEN, same check exit 0 — `306 visible of 306`, `12px / rgb(154,155,158)`, `"tabular-nums"`, `2 column(s) at 1440px, 1 at 340px`, and the pre-existing uniform-25px row pitch pin re-reading `17×25px` (a wrapped badge row sharing a grid row with the next property stretched it to 48px mid-implementation; chip rows now span the full meta row and the probe measured all 17 rows at 25px before the fix was called done). The label text comes from the shared card renderer's `col.label` — the column's schema display name, never the raw key — so no renderer change was needed: the whole defect was the kanban meta block hiding what it already rendered. Capture leg: `npm run screenshots` ×2, exit 0, 480 entries each; 60 board-card captures moved in BOTH runs at identical decoded-pixel counts (12 of them also changed dimensions, the cards growing taller); no single-run movers, so no jitter restores. Unit suite 1585/1585, `npx tsc --noEmit` 0, `npm run build` 0, `sheet-grammar` 0, `verify-placement` 0, `evidence --check-all` 15/15 after 12 artefacts were re-measured by their own writers, `npm run gate` 27/27 exit 0. The operator's device read is AC-006's, which stays open.
