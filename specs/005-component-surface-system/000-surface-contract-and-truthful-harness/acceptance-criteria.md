...
| AC-016 | REQ-015 | **No harness pins what the runtime computes.** Scan every harness stylesheet and fixture for an assignment of a custom property that `src/` also assigns; the four known cases are the scan's first fixtures | 0 pinned properties; the scan flags all four on the pre-repair tree and zero after | 4 known violations, enumerated in AC-005; the general scan does not exist, so a fifth would be invisible. **Produced by:** the harness pinning scan, at Stage 1. **AMENDED 2026-09-01 — the threshold was wrong and the fear was right.** The scan exists and deliberately skipped this population, because all **41** of these declarations are the harness supplying what the plugin computes, which is its purpose — a rule demanding zero fails a correct harness. What is enforced instead is the row's own stated worry, that a fifth would be invisible: the 41 are listed and ratcheted, and a 42nd fails the scan. Observed red on an added stand-in. | N14 | Partially met — concern enforced, threshold amended | - |
