...
_memory:
  continuity:
    ...
    recent_action: "Authored the acceptance criteria for the Obnotion rename"
    next_safe_action: "Answer Q2 and Q3, then run the leg after 0.0.30"
    blockers:
      - "Runs after 0.0.30 and after every in-flight leg; one rebase window"
      - "ADR-003 is Proposed until the operator accepts obn-"
    key_files:
      - "spec.md"
      - "plan.md"
      - "tasks.md"
    ...
    completion_pct: 0
    open_questions:
      - "Does the GitHub repository rename"
...
| AC-003 | REQ-003 | Given the same scope, When `db-` tokens are grepped, Then zero remain, and `db_view` is untouched | `git grep -ho '\bdb-[a-zA-Z0-9_-]*' -- styles.css src tools .storybook \| sort -u \| wc -l` prints 0 (baseline 1,724 distinct, 17,099 occurrences); `rg -n '\bdb_view\b' src tools \| wc -l` still prints 57 | Unmet | - |
| AC-004 | REQ-003 | Given the 209 `--db-*` custom properties, When the sweep runs, Then all 209 read `--obn-*` and no rule references a property that no longer exists | `git grep -ho '\-\-db-[a-zA-Z0-9_-]*' -- styles.css src tools \| sort -u \| wc -l` prints 0; `npm run gate` css lanes green | Unmet | - |
...
Written at authoring time, before any work: fifteen rows open, one of them (AC-015) the operator's
