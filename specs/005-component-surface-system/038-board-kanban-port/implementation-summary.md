...
| T32 landed on main (`worktrees/046-board-line-height`, `74a26419` + `48c50cf3` + `d4127184`, reconciled `4df2720c`) | Rebased across the `verify.mjs` symlink-safe-ignore fix (`6328c9cb`/`40140e42`) and the list hide-and-migrate feature (`e0e1c568`/`e466696b`/`f49eda4c`); `tools/lane/css-lane.json` merged as main's history plus this leg's own acquire+release pair; `screenshots/manifest.json` merged per entry by owner (main's 554, this leg's 32 board-owned entries under `screenshots/notion-clone/`); re-fingerprinted `sourceHashes` for `tools/live/render-assertion-harness.ts`/`tools/screenshots/constructed-scenarios.mjs`/`src/i18n.ts` on the 32 board entries after confirming the list-migration landing's changes to those three files are additive-only and do not touch board rendering. A full `npm run screenshots` proved the harness completes cleanly post-rebase (554 entries, 0 failures); every difference it produced (72 manifest entries, 45 PNGs) traced to `board-renderer.ts`'s due-date badge and `timeline-render-bench.ts`'s `EVENT_START` both anchoring on the real wall clock, which advanced two days mid-session, so that run's PNG/manifest output was discarded rather than signed. `constructed-board-desktop-dark.png` read beside `reference-kanban-desktop-dark.png`: count pill and every card band align at matching y-positions. `npx tsc --noEmit` exit 0; `npx vitest run` 1110/1110 (107 files); `npm run lint` 172 (unchanged); `lint:tools` clean; `scan-comments` PASS; `npm run screenshots:verify` 554/554 fresh, 0 stale; `npm run gate` **25 green / 0 red** — the `tests` lane (`verify.test.mjs`, previously failing in symlinked worktrees) is now green with the upstream fix, closing the one red this leg owed. |
| Second reconciliation (onto `origin/main`, landing `worktrees/045-board-card-properties` underneath this leg, `b7dc7cf5`) | Re-rebased after `a79d7421`/`ff1dacec`/`56a34199` landed mid-session; re-fingerprinted `sourceHashes` for `src/views/board-renderer.ts`/`src/i18n.ts` on the same 32 board-owned entries after confirming the card-properties feature is additive and opt-in (`boardExtensionsEnabled` off leaves `renderCard`'s default path untouched, per `a79d7421`'s own 0-moved-pixelHash/layoutHash proof). A third `npm run screenshots` run again proved the harness clean (554 entries, 0 failures) and was again discarded — the same wall-clock-anchored drift, two days further along. `node tools/live/evidence.mjs --check-all` found 8 of 16 artefacts STALE against the merged `styles.css` hash (`e191ebb917cd`); re-ran each artefact's own tool (`cascade-audit`, `checkbox-appearance`, `checkbox-inventory`, `design-conformance`, `engine-parity`, `surface-census`, `token-census`, `view-census`) rather than hand-editing, 16/16 fresh after. `npx tsc --noEmit` exit 0; `npx vitest run` 1136/1136 (109 files); `npm run lint` 172 (unchanged); `lint:tools` clean; `scan-comments` PASS; `npm run screenshots:verify` 0 stale; `npm run gate` 25 green / 0 red. |
<!-- /ANCHOR:verification -->
