...
| AC-7 | `SURFACE_PHASE=038-board-kanban-port npm run gate` | exit 0 | **Refreshed 2026-09-04 (rebase-onto-main landing).** 24 green / 1 red, exit 1 — `screenshots-fresh` is RED; confirmed present on a stashed pristine `origin/main` tip (`a78000c`) before this session made any edit of its own, so it predates rather than regresses this criterion. Root cause: `tools/screenshots/theme.css` changed content at T29 (`d07f47e5`) but that commit's own recapture named only its 11 owned captures, leaving ~800 unrelated fixture entries' `sourceHashes` stale against the new file hash. Out of this landing's scope (fixing it means recapturing hundreds of files this leg does not own); tracked as an open follow-up rather than silently claimed green. First closed 2026-09-03 (T7); every fidelity pass through T30 reconfirmed 25/25 without re-testing `screenshots-fresh` against a clean tree, which is how the gap went unnoticed until this session's own pristine-HEAD test. |
