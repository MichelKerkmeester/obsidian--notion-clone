...
| The embedded codeblock host gets its own migration pass (§1.3, REQ-003) | 007-002's ADR-001 precedent, even though 0 fence-configured views of the three exist today — the host resolves the referenced view's config itself, and the transplant shape (`migrateListViewOnOpen`, `embedded-database-renderer.ts:742,776-800`) already ships |
