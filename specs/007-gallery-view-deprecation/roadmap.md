...
| 3 | `003-remove-renderer-and-harness` | `gallery-renderer.ts` and its whole measurement surface deleted together — bench, driver, coverage pins, constructed scenario, capture entries, placement checks, unit specs and 81 `db-gallery-*` selectors | Implemented, gate green — `003/implementation-summary.md`. `gallery` stays accepted-but-redirected on `DatabaseViewType` (ADR-001, same shape as `list`); `renderer-coverage.json` floor lowered to 5/20. The gate's 25 lanes are unchanged BY NAME — gallery never owned a dedicated lane the way list owned `list-window` — and 3 of the 4 board-shared capture ids moved pixels for a named, reviewed reason (their own gallery half removed, or a capture crop correction), not a board regression; all 12 opened and read by hand |
...
| The renderer is gone | `git log --diff-filter=D` names `src/views/gallery-renderer.ts`; the gate exits 0 read from `$?` | Done — 25/25 green |
