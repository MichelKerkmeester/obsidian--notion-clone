...
| 2 | `002-settings-redirect-and-migrate` | The settings-load sanitizer and the `.base` importer stop accepting `gallery`; the migration runs on open in **both** hosts, once, with a notice | Implemented, gate green — `002/implementation-summary.md`. The sanitizer is closed by routing a loaded gallery through the real migration rather than the bare unknown-type fallback (`decision-record.md` ADR-002, since a verbatim copy of `006`'s edit would strand the cover); the embedded host gained the migration call it never had (ADR-001); `galleryImageAspectRatio`/`galleryImageFit` now carry to their board equivalents. **Blocked on a release cut before `003` can start (parent D8)** |
| 3 | `003-remove-renderer-and-harness` | `gallery-renderer.ts` and its whole measurement surface deleted together — bench, driver, coverage pins, constructed scenario, capture entries, placement checks, unit specs and 81 `db-gallery-*` selectors | Implemented, gate green — `003/implementation-summary.md`. `gallery` stays accepted-but-redirected on `DatabaseViewType` (ADR-001, same shape as `list`); `renderer-coverage.json` floor lowered to 5/20. The gate's 26 lanes are unchanged BY NAME — gallery never owned a dedicated lane the way list owned `list-window` — and 3 of the 4 board-shared capture ids moved pixels for a named, reviewed reason (their own gallery half removed, or a capture crop correction), not a board regression; all 13 movers opened and read by hand. Landed on `main` after a rebase that re-derived the stylesheet sweep, the capture corpus and every evidence artefact against the moved base |
| 4 | `004-docs-and-release` | README, CHANGELOG, closing `030`'s open rows against this retirement, and the release that carries the removal | In progress — doc half done, `004/implementation-summary.md`. `README.md` and `package.json` no longer offer gallery as a current view; `CHANGELOG.md`'s `## 0.0.28` entry names every `001`-declared loss individually; `030/spec.md` reads Superseded. Release **0.0.28** (`d3433d81`) already carries `001`-`003`. Only the operator row is open |
...
`database-view.ts:11678` and from nowhere else. `embedded-database-renderer.ts` renders the gallery
...
| The gate | `npm run gate` is 26 lanes today. This packet removes gallery measurement from several of them and must leave `$?` at 0 with the lanes gone rather than disabled |
...
|---|---|---|
| The audit is real | `001/implementation-summary.md` names every accepting and minting surface and every declared loss individually | Done — `001/implementation-summary.md` |
| Nothing can be a gallery | The sanitizer routes through the real migration instead of exempting `gallery`; the `.base` importer is pinned by regression tests | Done — `002/decision-record.md` ADR-002, `gallery-hide-and-migrate.test.ts` |
| Nothing still is a gallery | The migration runs in both hosts, once, with a notice, and is **shipped in a release** | Done — `002/decision-record.md` ADR-001 (embedded host), shipped in **0.0.28** (`d3433d81`) |
| The renderer is gone | `git log --diff-filter=D` names `src/views/gallery-renderer.ts`; the gate exits 0 read from `$?` | Done — 26/26 green |
| The gate got smaller honestly | `renderer-coverage.json`'s new floor carries its reason beside the number | Done — `5/20`, `"was 6/21; gallery renderer retired"` |
| The user is told | README and CHANGELOG name every loss individually | Done — `004/implementation-summary.md`, `CHANGELOG.md` `## 0.0.28` |
