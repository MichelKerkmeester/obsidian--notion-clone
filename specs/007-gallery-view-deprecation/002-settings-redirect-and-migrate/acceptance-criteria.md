...
| AC-003 | REQ-002 | **Given** a vault view configured as a gallery, **When** it is opened in the standalone host, **Then** it renders as a board with the same cover, and the notice appears once. **Today: this already works** via `database-view.ts:11678`; the criterion exists so a regression here is caught, not because it is new | The existing migration path re-asserted, plus a two-open test proving the notice fires once | Unmet | - |
