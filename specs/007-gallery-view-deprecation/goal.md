...
| D6 | **The dead CSS comes out in this packet.** `006`'s `007-remove-renderer-and-harness` deferred its `db-list-*` sweep as T010 and it is still open. 84 gallery lines in `styles.css` are not deferred here. |
...
- [ ] No surface offers gallery as a choice **and no surface mints one**. The two pickers are
      already filtered by `030`; the settings-load sanitizer (`main.ts:144`, `:180`) and the `.base`
      importer (`main.ts:1548-1616`) still accept it and must not.
- [ ] A vault carrying a gallery-configured view opens it as a board with the same cover, once,
      ...
      `database-view.ts:11663` today; `embedded-database-renderer.ts` has no equivalent call.
...
| Inventory taken | Done | `spec.md` §4 — 41 `src/` files, 31 `tools/` files, 84 `styles.css` lines, 24 of 546 capture entries, a 787-line renderer. Counted against the tree at `2a7db8cf`, not estimated |
...
| Two minting surfaces survived `030`'s withdrawal | `main.ts:144`/`:180` and the `.base` importer at `:1548`. `006`'s audit found exactly this class for the list and recorded it as a gap in that packet's own REQ-002. Named here in advance instead of discovered again. |
