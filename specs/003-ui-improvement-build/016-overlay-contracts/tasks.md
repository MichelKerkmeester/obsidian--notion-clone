---
title: "Task Breakdown: Overlay Presentation, Placement and Layer Contracts"
description: "Tasks for 016-overlay-contracts, one per requirement, each closed only with read evidence."
trigger_phrases:
  - "016 overlay contracts tasks"
importance_tier: "high"
contextType: "planning"
---
# Task Breakdown: Overlay Presentation, Placement and Layer Contracts
...
- [ ] **T-001** Presentation is a caller-declared mode, never inferred from whether an anchor was passed
- [ ] **T-002** `.db-record-detail-panel` keeps its panel-tier z-index. Its field editors are siblings rather than descendants (`styles.css:8852-8867`), so raising the sheet to modal tier makes the panel paint over its own editors
- [ ] **T-003** `layer-scale-and-timeline-width.test.ts` passes unmodified
- [ ] **T-004** The literal `998` and the undeclared `--db-layer-sticky` are replaced by named tiers
- [ ] **T-005** Item 6 is closed by a device observation or explicitly deferred, never by speculative padding
