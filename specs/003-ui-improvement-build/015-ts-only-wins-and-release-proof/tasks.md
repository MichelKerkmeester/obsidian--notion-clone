---
title: "Task Breakdown: TS-Only Wins and Release-Loop Proof"
description: "Tasks for 015-ts-only-wins-and-release-proof, one per requirement, each closed only with read evidence."
trigger_phrases:
  - "015 ts only wins and release proof tasks"
importance_tier: "high"
contextType: "planning"
---
# Task Breakdown: TS-Only Wins and Release-Loop Proof
...
- [ ] **T-001** `t("calendar.unscheduled")` resolves; the raw key no longer renders to the user
- [ ] **T-002** A test asserts every `t()` argument resolves against `i18n.ts`. `t()` accepts any string, so tsc cannot catch this class of defect
- [ ] **T-003** On a phone the `⤢` affordance opens the editable detail panel, not the display-only side rail
- [ ] **T-004** An annotated tag with a non-empty body produces a GitHub release; `package.json`, `manifest.json` and `versions.json` bump together
- [ ] **T-005** The operator confirms the new build reaches the device and all three fixes are visible
