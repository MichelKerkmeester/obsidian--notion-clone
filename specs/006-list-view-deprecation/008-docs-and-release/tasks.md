...
- [ ] T011 Drop the retired **list** view from `package.json`'s `description` (`package.json:4`) and its `keywords` (`package.json:39`)
  Found 2026-09-06 by the fresh reviewer of `specs/007-gallery-view-deprecation/004-docs-and-release`, which cleared the *gallery* from `package.json` and `manifest.json` and left this one alone as another packet's. `package.json:4` still reads `"…table, board, list, chart, calendar, timeline…"` and `:39` still carries the `list` keyword, both retired at `0.0.23`. Recorded rather than fixed, per scope lock. `manifest.json`'s own description — the one Obsidian's community-plugin browser renders — never named the list, so nothing user-facing is offering it; this is npm-facing metadata only.
<!-- /ANCHOR:phase-3 -->
