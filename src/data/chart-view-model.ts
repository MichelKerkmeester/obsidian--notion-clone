...
// COMPONENT: chart titles/config normalization, plus click-to-filter drilldown
// ───────────────────────────────────────────────────────────────────
//
// The drilldown section (getDrilldownRows, rowMatchesChartGroup, getRowChartValue)
// re-derives a row's field value independently of chart-aggregation.ts's
// getChartFieldValue rather than reusing it — the two must be kept in sync
// by hand when a new field-value source is added, since a bucket clicked on
// the chart has to resolve back to the exact same rows that produced it.
// normalizeChartConfigForType clears config fields that don't apply to the
// selected chart type (e.g. axis range on a pie chart) so stale settings from
// a previous chart type can't leak into rendering or the options panel.
...
import { t } from "../i18n";
import { getDefaultChartValueField, isChartAggregationValueColumn, isChartCheckboxGroupColumn, isChartDateGroupColumn, isChartNumberGroupColumn, isChartSeriesColumn, isChartValueColumn, parseNumberBucketKey, requiresChartValueField, toChartNumber } from "./chart-aggregation";
import { isObsidianTagsKey, normalizeObsidianTagValue } from "./column-types";
import { isDateLikeColumnType } from "./date-time-format";
