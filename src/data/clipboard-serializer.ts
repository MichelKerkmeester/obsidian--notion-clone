// ───────────────────────────────────────────────────────────────────
// MODULE:    clipboard-serializer
// COMPONENT: cell-grid <-> clipboard text conversion (TSV/Markdown/CSV)
// ───────────────────────────────────────────────────────────────────
//
// parseClipboardTable sniffs format in a fixed order — Markdown table, then
// unquoted-tab-delimited, then quote-led CSV, else plain lines — because a
// pasted CSV of prose can contain commas that aren't a delimiter and a TSV
// paste must win over CSV whenever both look plausible; isExplicitCsv only
// fires when the text actually starts with a quote so ordinary comma-bearing
// text isn't misread as a CSV table. Reordering these checks changes what a
// given paste is interpreted as.
...
import { isObsidianTagsKey, toMultiSelectValuesForKey } from "./column-types";
