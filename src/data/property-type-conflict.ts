// ───────────────────────────────────────────────────────────────────
// MODULE:    property-type-conflict
// COMPONENT: cross-database detection and resolution of clashing property types
// ───────────────────────────────────────────────────────────────────
//
// Two databases can each own a column/computed field that writes the same
// frontmatter key with a different type (e.g. one "text", another "number").
// Obsidian's own property-type registry only tracks one type per key, so
// this collapses plugin column types into a coarser "observable" type space
// first, then diffs writers against that space to find real clashes and
// tracks in-flight draft type changes so a conflict can be resolved without
// forcing every writer to be edited atomically.
