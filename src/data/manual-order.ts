// ───────────────────────────────────────────────────────────────────
// MODULE:    manual-order
// COMPONENT: LexoRank-like fractional ranks for drag-to-reorder persistence
// ───────────────────────────────────────────────────────────────────
//
// Ranks are variable-length base62 strings compared with plain `<`/`>`
// (ASCII byte order matches CHARSET order), so a reorder only needs to
// rewrite the moved row's rank — never its neighbors — which keeps drag
// persistence to a single frontmatter write. `rankBetween` returns null when
// two ranks are adjacent with no room for a midpoint; callers must then call
// `rebalanceRanks` before retrying, or ranks silently collide.
