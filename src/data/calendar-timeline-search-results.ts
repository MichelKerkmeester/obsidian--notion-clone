// ───────────────────────────────────────────────────────────────────
// MODULE:    calendar-timeline-search-results
// COMPONENT: flattens calendar/timeline events into a sorted, searchable list
// ───────────────────────────────────────────────────────────────────
//
// The search panel (embedded-database-renderer.ts, database-view.ts) queries
// this over every row regardless of what's currently scrolled into view, then
// each result carries an `inCurrentRange` flag so the UI can dim/hide
// off-screen hits itself — this file intentionally never filters rows out by
// visibleRange, it only tags them.
...
import { addDateKeyDays, dateKeyDaysBetween, MINUTES_PER_DAY } from "./calendar-date-time";
import { formatDateTimeRangeDisplay } from "./date-time-format";
