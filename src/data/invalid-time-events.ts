// ───────────────────────────────────────────────────────────────────
// MODULE:    invalid-time-events
// COMPONENT: Detects and offers a quick fix for start >= end timeline/calendar events
// ───────────────────────────────────────────────────────────────────
//
// The scan reuses `isInvalidEventRange` as the single source of truth for
// "invalid" (shared with the timeline renderer) so this modal and the
// renderer never disagree about which events are broken. Scanning large row
// sets runs in idle-callback chunks (InvalidTimelineEventsScanner) with a
// setTimeout fallback, and caches by a key that includes the resolved
// column display types — not just field names — so a field that flips
// between date/datetime invalidates the cache correctly.

// ───────────────────────────────────────────────────────────────────
// 1. IMPORTS
// ───────────────────────────────────────────────────────────────────

import { extractTimelineEndpointMinutes, isInvalidEventRange } from "./calendar-timeline-model";
