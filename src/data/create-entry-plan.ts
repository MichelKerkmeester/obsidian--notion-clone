// ───────────────────────────────────────────────────────────────────
// MODULE:    create-entry-plan
// COMPONENT: Turns a database/view's required source rules into a concrete new-note plan
// ───────────────────────────────────────────────────────────────────
//
// Shared by every creation entry point (table/board/gallery/list/calendar/
// timeline/toolbar/database-file view) so "new note satisfies its own source
// rules" is computed once, not reimplemented per view. Only rules that every
// matching result must satisfy (a single leaf, an AND, or a single-child OR)
// are adopted as creation constraints; multi-branch OR, NOT and expression
// rules cannot be safely reduced to one write, so they only produce
// diagnostics and never block creation. Merge priority is fixed low-to-high:
// column defaults < view filter/status/group/calendar-entry defaults < source
// rules — a source rule silently overriding a context default is exactly the
// case that must be surfaced as a conflict diagnostic, since the user may
// not expect a newly created record to land outside the group/filter they
// clicked from. The filename is always sanitized and auto-suffixed by
// DataSource.createNote; an exact file.basename/file.name constraint that
// would be broken by that sanitization or suffixing is diagnosed here rather
// than silently failing later.

// ───────────────────────────────────────────────────────────────────
// 1. IMPORTS
// ───────────────────────────────────────────────────────────────────
import { isDateLikeColumnType, toDateTimestamp } from "./date-time-format";
import { ColumnDisplayType, getColumnDisplayType } from "./column-display";
import {
...
import { isFileFieldKey } from "./file-fields";
import {
...
import { nextUniqueId } from "./unique-id-stamp";
