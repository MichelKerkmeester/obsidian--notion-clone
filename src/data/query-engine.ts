// ───────────────────────────────────────────────────────────────────
// MODULE:    query-engine
// COMPONENT: shared sort/filter/group primitives consumed by every row-based view
// ───────────────────────────────────────────────────────────────────
//
// One engine backs table, board, and calendar rendering, so a rule encoded
// here changes every view at once: "select"/"status" columns compare by
// configured option order rather than string sort, checkbox "empty" means
// unchecked (boolean-aware, not a stringified-false check) so two
// equally-unchecked rows never split into different filter buckets, and
// date/datetime comparisons always go through toDateTimestamp so mixed
// date-only and date+time values sort on the same wall-clock epoch.

// ───────────────────────────────────────────────────────────────────
// 1. IMPORTS
// ───────────────────────────────────────────────────────────────────

import { getColumnOptionValues, hasObsidianTagValue, isObsidianAliasesKey, isObsidianTagsKey, normalizeObsidianTagValue, toBooleanValue, toMultiSelectValues, toObsidianTagValues } from "./column-types";
import { getColumnDisplayType } from "./column-display";
import { isDateLikeColumnType, parseDateTimeParts, toDateTimestamp } from "./date-time-format";
import { getDateGroupMode } from "./group-display";
import { getRowFileFieldValue, isBaseFileField } from "./file-fields";
import { compareMultiSelect } from "./multi-select";
import { stringifyValue } from "./stringify";
import { ColumnDef, DateGroupMode, RowData, FilterRule, SortRule, ViewConfig } from "./types";
import { t } from "../i18n";
import { evaluateViewFilterTree } from "./view-filter-tree";
