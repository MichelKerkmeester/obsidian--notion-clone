// ───────────────────────────────────────────────────────────────────
// MODULE:    view-filter-tree.test
// COMPONENT: Kleene-logic evaluation and dual-write coherence suite for the view filter tree
// ───────────────────────────────────────────────────────────────────
//
// The filter tree evaluates to a three-valued (true/false/null) result so an
// empty or all-pruned branch reads as "unknown" rather than silently
// widening an OR or narrowing an AND. This suite locks that Kleene logic
// down, and separately locks the property that state.filterTree and the
// legacy flat state.filters array — which the app still mutates side by
// side — end up describing the same set of leaves after every helper the
// call sites delegate to (prune/map/remove/append) is applied to both.
