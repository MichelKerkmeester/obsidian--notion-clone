// ───────────────────────────────────────────────────────────────────
// MODULE:    let-variables.test
// COMPONENT: source-rewrite regression suite for transformLetCalls
// ───────────────────────────────────────────────────────────────────
//
// transformLetCalls rewrites let/lets(...) calls into nested arrow-function
// calls before the safe evaluator ever sees them, so the rewrite itself has
// to get quoting, member-call disambiguation ("obj.let(...)" is not a
// binding), and reserved-name rejection right — a bug here corrupts the
// formula text silently rather than failing loudly at evaluation time.
...
import { transformLetCalls } from "../let-variables";
