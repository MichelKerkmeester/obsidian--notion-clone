// ───────────────────────────────────────────────────────────────────
// MODULE:    view-filter-tree
// COMPONENT: build/normalize/prune/evaluate/serialize the recursive view filter tree (SourceRuleNode)
// ───────────────────────────────────────────────────────────────────
//
// Leaf evaluation is three-valued (true/false/null via KleeneValue), not
// boolean, because a group whose every branch is "unknown" must stay
// unknown rather than silently resolving to true/false — see
// evaluateViewFilterTree and the pruning note below. View filters only
// support the flat FilterOperator subset of the full SourceRuleOperator
// set (VIEW_FILTER_OPERATORS); tree nodes for other operators or malformed
// input are rejected by normalizeNode with a console warning instead of
// throwing, since filter trees can arrive from imported/hand-edited config.

// ───────────────────────────────────────────────────────────────────
// 1. IMPORTS
// ───────────────────────────────────────────────────────────────────

import { isEffectiveFilterRule } from "./filter-rules";
