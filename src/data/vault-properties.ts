...
// COMPONENT: plugin-owned cache of vault-wide frontmatter property names/types for source-rule pickers
// ───────────────────────────────────────────────────────────────────
//
// Deliberately built by scanning Obsidian's public metadataCache/vault APIs
// rather than its internal metadataTypeManager registries, so the picker's
// property list survives Obsidian internal API changes. The module-level
// cache is refreshed explicitly (plugin load / metadata change) rather than
// on every read, since walking every markdown file's frontmatter on each
// picker open would be too slow.
...
import { inferColumnType } from "./frontmatter-scanner";
