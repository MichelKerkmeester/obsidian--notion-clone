// ───────────────────────────────────────────────────────────────────
// MODULE:    data-source
// COMPONENT: Vault-backed persistence and view-config parsing facade — the only class touching Obsidian's Vault/MetadataCache for database records
// ───────────────────────────────────────────────────────────────────
//
// Optimistic frontmatter/view-def overrides exist because metadataCache
// updates lag a write by one or more event ticks; callers need to read back
// what they just wrote before Obsidian confirms it, or renders flicker to
// stale values. Writes to the same file path are serialized through a queue
// (writeQueues) because concurrent processFrontMatter calls into the same
// file corrupt each other. "Owned path" credits distinguish a change that
// originated from this plugin's own write from a genuinely external edit, so
// notify() does not loop the plugin into reacting to its own writes.
// parseDatabaseConfig/parseViewConfig absorb several generations of on-disk
// schema (flat view props, typeFilter, legacy source-rule shapes) because
// live vaults still contain files written by earlier plugin versions, and
// this file is the single place those migrations happen.
...
import { hasObsidianTagValue, normalizeStatusPresets, toMultiSelectValues, toObsidianTagValues } from "./column-types";
import { normalizeComputedSyncMode } from "./computed-sync";
import { fileHasLink, getBaseFileFieldType, getFileFieldValue, isBaseFileField } from "./file-fields";
import { absorbTypeFilterIntoRules, getSourceRuleTree, matchesBaseSourceType, matchesSourceRuleTree, parseSourceRuleTree, sourceRuleContainsValue, sourceRuleValuesLooseEqual, sourceRuleValuesStrictEqual } from "./source-rules";
import { linkDatabaseSchema } from "./column-config";
import { inspectReportsConfig } from "./reports-inspector";
import type { ReportsInspectRecord, ReportsSalesMeaning } from "./reports-inspector";
import { applyReportsComputedConfig } from "./reports-computed-config";
import type { ReportsComputedConfigOptions, ReportsComputedConfigResult } from "./reports-computed-config";
...
// ───────────────────────────────────────────────────────────────────
// 3. DATA SOURCE - LIFECYCLE & EVENTS
// ───────────────────────────────────────────────────────────────────
export class DataSource {
  ...
  // ─────────────────────────────────────────────────────────────────
  // 10. INTERNAL STATE & SOURCE MATCHING
  // ─────────────────────────────────────────────────────────────────
...
// ───────────────────────────────────────────────────────────────────
// 11. SOURCE RULE VALUE COMPARISON
// ───────────────────────────────────────────────────────────────────
