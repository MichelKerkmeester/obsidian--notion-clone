// ───────────────────────────────────────────────────────────────────
// MODULE:    column-types
// COMPONENT: status presets, option/type classification, and Obsidian tag helpers
// ───────────────────────────────────────────────────────────────────
//
// STATUS_OPTION_PRESETS' labels/values are hardcoded Chinese text, not i18n
// keys — they are seed *data* written into option/status columns, not UI
// chrome, so translating them would silently change already-saved
// frontmatter values on existing notes. Tag parsing has two distinct entry
// families that must stay separate: the plain (`getObsidianTagInputEntries`)
// family only splits on commas because it reads already-array-shaped
// frontmatter, while the "input"/text family additionally splits on
// whitespace because it reads freeform text a user just typed.
