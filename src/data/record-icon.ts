// ───────────────────────────────────────────────────────────────────
// MODULE:    record-icon
// COMPONENT: parses/serializes the per-row icon token stored in a text column
// ───────────────────────────────────────────────────────────────────
//
// The icon lives in an ordinary text-typed column so it survives as plain
// frontmatter text with no schema migration, encoded as either a single
// emoji grapheme or a `lucide:<id>@<color>` token. A "single emoji" check
// must use a grapheme segmenter, not string length or codepoint count — a
// flag or ZWJ-joined emoji is one user-visible glyph but multiple UTF-16
// code units, and rejecting it would silently break valid icon values.
...
export const RECORD_ICON_COLORS = STATUS_COLORS;
