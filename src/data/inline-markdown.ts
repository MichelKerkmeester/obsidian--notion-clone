...
// COMPONENT: Lightweight inline-markdown parser for text cell/card values
// ───────────────────────────────────────────────────────────────────
//
// Pure logic — no DOM, no Obsidian, no external markdown library. Mirrors the
// pattern of `parseTextLink` (src/data/text-link.ts): opt-in, lenient, string
// in → token tree out.
//
...
//
// The renderer (src/views/inline-markdown-renderer.ts) builds DOM from the token
...
import { getLinkLabel, isExternalUrl, normalizeExplicitLinkTarget } from "./text-link";
