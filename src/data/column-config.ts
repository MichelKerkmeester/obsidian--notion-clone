...
// COMPONENT: schema linking, column ordering/visibility, and column-rename propagation
// ───────────────────────────────────────────────────────────────────
//
// updateColumnKeyReferences is the single place a renamed/deleted column key
// gets propagated everywhere it might be referenced — config fields, sort/
// filter rules, per-view-mode state, and computed formula text. Filters are
// stored in two representations at once (a legacy flat `filters` array and a
// nested `filterTree`); this file dual-writes both on every rename so they
// never drift apart, rather than treating one as derived from the other.
...
import { isOptionColumnType } from "./column-types";
...
import { updateSourceRuleTreeKeyReferences } from "./source-rules";
import { flattenLeaves, mapLeafAt } from "./view-filter-tree";
