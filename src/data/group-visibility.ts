...
// COMPONENT: show/hide empty groups and per-group row expansion limits
// ───────────────────────────────────────────────────────────────────
//
// Only option-shaped columns (status/select/multi-select) support an "empty
// groups" toggle at all — other column types don't have a fixed option set
// to backfill missing groups from, so isEmptyGroupVisibilityColumn gates
// every read/write in this module. setShowEmptyGroups additionally prunes
// the stored override back to `undefined` whenever it matches the type's
// default, keeping persisted view config from accumulating no-op entries.
...
import { getDefaultGroupOrder } from "./group-order";
