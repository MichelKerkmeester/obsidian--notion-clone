// ───────────────────────────────────────────────────────────────────
// MODULE:    calendar-title-formatter
// COMPONENT: formats calendar/timeline header titles and month/day axis bands
// ───────────────────────────────────────────────────────────────────
//
// All date math here is done in UTC (see calendar-date-time.ts) so that a
// date key like "2024-03-15" formats identically no matter the reader's
// timezone — mixing in local Date getters would shift band boundaries by a
// day near midnight. English and Chinese title formatting are kept as
// separate code paths (not a shared template) because Chinese month/day
// text has its own numeral rules (see chineseDay) that don't map onto a
// Western month-name substitution.
...
import { addUtcDays, daysBetweenUtc, makeUtcDate, parseDateKeyToUtc, startOfNextUtcMonth } from "./calendar-date-time";
