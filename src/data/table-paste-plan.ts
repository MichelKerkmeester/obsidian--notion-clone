// ───────────────────────────────────────────────────────────────────
// MODULE:    table-paste-plan
// COMPONENT: Computes the spreadsheet-style multi-cell paste rectangle without mutating rows/columns.
// ───────────────────────────────────────────────────────────────────
//
// A one-cell clipboard value fills the whole current selection; a matrix
// always anchors at the selection's top-left cell. Rows may extend past the
// current record grid (the caller can create new records for them), but
// columns stay bounded to the existing grid — a paste must never implicitly
// create schema (new columns).

import type { TableCellAddress } from "./table-keyboard-navigation";
