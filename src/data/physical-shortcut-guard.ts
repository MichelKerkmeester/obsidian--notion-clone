// ───────────────────────────────────────────────────────────────────
// MODULE:    physical-shortcut-guard
// COMPONENT: Rejects Mod+key shortcuts whose modifier flag wasn't backed by a real keydown
// ───────────────────────────────────────────────────────────────────
//
...
//
// Exists because a pointer gesture (e.g. a Cmd-click drag on macOS) can
// report `metaKey: true` on synthetic/trailing events without ever firing a
// real Meta keydown — without this guard those events would trigger Mod
// shortcuts the user never intended. `beginPointerGesture`/`endPointerGesture`
// bracket that window, and `pointerGestureSettling` extends the block one
// tick past pointerup so a shortcut key that lands right as the gesture ends
// still doesn't fire on the stale modifier state.
