// ───────────────────────────────────────────────────────────────────
// MODULE:    source-rules
// COMPONENT: Source-rule tree model — construction, legacy migration, matching, and value comparison.
// ───────────────────────────────────────────────────────────────────
//
// Source rules can live as either a flat legacy list (`sourceRules`) or a tree
// (`sourceRuleTree`), at both database and view level. Merging the two levels
// must normalize each side (tree wins over flat) BEFORE combining, or a
// same-side tree silently drops the other side's flat rules — see
// mergeDbAndViewSourceRuleTrees for the fix and why it's structured this way.
// Value comparisons also treat `[[wikilink]]`-shaped strings as file targets
// (path + optional heading stripped, `.md` normalized) rather than plain text,
// so relation/link fields compare correctly against typed rule values.
...
import { getBaseFileFieldType, isBaseFileField } from "./file-fields";
