// ───────────────────────────────────────────────────────────────────
// MODULE:    base-expression
// COMPONENT: Obsidian Bases-syntax expression evaluator (parser/normalizer + sandboxed eval)
// ───────────────────────────────────────────────────────────────────
//
// Base expressions use a dot-method surface (`.isType()`, `.contains()`,
// duration subtraction between dates, etc.) that isn't valid JS on its own,
// so normalizeBaseExpression rewrites the source into plain JS calls before
// safeEval ever runs it — the regex passes here ARE the parser, and their
// ordering matters (e.g. duration/date rewrites must run before the generic
// dot-method rewrites, or a `.days` accessor gets mangled first). Everything
// downstream still runs through safeEval's sandboxed scope with the
// DANGEROUS_TOKENS/RESERVED denylist as a second layer, since formula text
// ultimately comes from user-authored vault content, not trusted source.
...
import { hasObsidianTagValue, toObsidianTagValues } from "./column-types";
...
import type { MomentConstructor, MomentDurationLike } from "./moment-types";
