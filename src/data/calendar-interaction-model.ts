// ───────────────────────────────────────────────────────────────────
// MODULE:    calendar-interaction-model
// COMPONENT: pure drag/resize/create math for calendar and timeline event editing
// ───────────────────────────────────────────────────────────────────
//
// Every function here takes primitive inputs (minutes, date keys, unit
// deltas) and returns the resulting date-change payload — no DOM, no
// Obsidian API — so the drag/resize/move interaction contracts for the
// calendar and the timeline can be pinned down with plain unit tests.
// resolveTimedDragRange in particular encodes several non-obvious rules:
// move ignores endMaxMinutes so a long event's duration is never clipped by
// a same-day resize boundary, and resize/move deliberately use different
// clamping strategies to avoid corrupting a cross-day event's start/end.

// ───────────────────────────────────────────────────────────────────
// 1. IMPORTS
// ───────────────────────────────────────────────────────────────────

import { CALENDAR_TIME_SNAP_MINUTES, formatTimeRange, snapMinutes } from "./calendar-date-time";
