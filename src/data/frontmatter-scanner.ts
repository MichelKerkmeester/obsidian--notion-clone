// ───────────────────────────────────────────────────────────────────
// MODULE:    frontmatter-scanner
// COMPONENT: scan vault Markdown files against source rules for column setup
// ───────────────────────────────────────────────────────────────────
//
// Every scan re-applies the same source-rule matching used by live database
// queries (matchesRule/matchesRules mirror the query engine's operator set)
// so the keys, samples, and tags a user is offered while configuring a view
// always agree with what that view will actually show. Files carrying
// `db_view: true` are always skipped — a database's own view note must
// never be scanned as a data row of itself.
...
import { hasObsidianTagValue, isObsidianAliasesKey, isObsidianTagsKey, toMultiSelectValues, toObsidianTagValues } from "./column-types";
import { hasDateTimeValue } from "./date-time-format";
import { getSourceRuleTree, matchesBaseSourceType, matchesSourceRuleTree, sourceRuleContainsValue, sourceRuleValuesLooseEqual, sourceRuleValuesStrictEqual } from "./source-rules";
import { fileHasLink, getFileFieldFixedType, getFileFieldValue, isBaseFileField, isFileFieldKey } from "./file-fields";
...
// ───────────────────────────────────────────────────────────────────
// 5. COLLECT FRONTMATTER KEYS & SAMPLES
// ───────────────────────────────────────────────────────────────────
