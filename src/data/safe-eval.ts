// ───────────────────────────────────────────────────────────────────
// MODULE:    safe-eval
// COMPONENT: A safe recursive-descent parser and evaluator for JavaScript-like expressions.
// ───────────────────────────────────────────────────────────────────
//
// Replaces `new Function()` / `eval()` with a controlled interpreter that only
// supports a fixed, safe grammar. This matters because formulas here come from
// vault content the plugin doesn't control — a real `eval()`/`Function()` would
// let a formula run arbitrary JS with the plugin's full Obsidian API access
// (read/write any file in the vault). Scoping evaluation to this grammar means
// a formula can never escape into real JS execution semantics.
//
