...
// COMPONENT: debounces/coalesces external file-change events into one refresh
// ───────────────────────────────────────────────────────────────────
//
// Owns no view or Obsidian state, only timing: the first dirty path starts a
// bounded maxDelayMs timer so a steady trickle of edits can never starve a
// refresh indefinitely, while every later event only resets a short
// idleDelayMs timer so a burst collapses into a single pass. When a refresh
// is blocked (view hidden, mid-edit) or ineligible, dirty paths stay queued
// and a cheap retry timer keeps polling instead of dropping the batch — a
// missed focus/active-leaf event must not permanently strand pending data.
