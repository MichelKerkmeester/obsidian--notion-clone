// ───────────────────────────────────────────────────────────────────
// MODULE:    csv-markdown-zip-export
// COMPONENT: Bundles a database into one ZIP of per-note Markdown, per-view CSVs and a schema CSV
// ───────────────────────────────────────────────────────────────────
//
// The summary CSV's column set is the union of every view's visible columns
// (deduplicated by key), not just one view's selection, and its row set is
// deduplicated by file path across all views (allRowsByPath) — a row visible
// in two views must appear once in the summary, not once per view, while
// still exposing every column any view chose to show for it.
...
import { t } from "../i18n";
import { createStoredZip, ZipEntry } from "./zip-export";
import { saveZipWithPicker } from "./export-save-target";
