...
// COMPONENT: parse, format, and compare date/datetime cell values
// ───────────────────────────────────────────────────────────────────
//
// Two timezone conventions are intentionally mixed: numeric millisecond
// timestamps (e.g. file.ctime) are absolute instants read with UTC-safe
// local getters, while YAML/Obsidian Date objects and naive date strings
// are read via their UTC accessors so the same wall-clock date renders
// everywhere regardless of runtime timezone. Comparison and grouping paths
// must go through parseDateTimeParts/toDateTimestamp rather than a bare
// `new Date()`/`Date.parse()`/`toISOString()`, or that convention silently
// breaks.
