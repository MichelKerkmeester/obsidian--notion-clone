// ───────────────────────────────────────────────────────────────────
// MODULE:    calendar-layout-model
// COMPONENT: month-grid segment lanes and time-grid column layout for the calendar view
// ───────────────────────────────────────────────────────────────────
//
// Month-view spanning events and day-view timed events need different
// packing strategies, so this module keeps them side by side rather than
// sharing one algorithm: compareMonthSegments gives longer/multi-day spans
// the low lanes, so assignSegmentLanes can't push a spanning event to a
// high lane where it would later get folded away on a quiet day;
// assignTimedColumns clusters overlapping timed events and packs them into
// the fewest columns. getCalendarEventTiming is deliberately strict about
// requiring both endpoints to be genuine datetime columns before treating
// an event as "timed" — a mixed date/datetime pairing or a dirty value must
// fall back to the all-day lane rather than landing on the wrong hour.

// ───────────────────────────────────────────────────────────────────
// 1. IMPORTS
// ───────────────────────────────────────────────────────────────────

import { CalendarDayModel, CalendarTimelineEvent, extractTimelineEndpointMinutes, getDefaultEventDateField } from "./calendar-timeline-model";
import { formatCalendarTime, normalizeVisibleHourRange } from "./calendar-date-time";
