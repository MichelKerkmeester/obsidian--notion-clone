// ───────────────────────────────────────────────────────────────────
// MODULE:    chart-aggregation
// COMPONENT: turns raw rows into chart-ready grouped/bucketed/stacked points
// ───────────────────────────────────────────────────────────────────
//
// A multi-select/relation/tags value fans one row into multiple group keys
// (see toGroupKeys, then `for (const key of keys)` in aggregateChart) — a
// row with three tags deliberately contributes to all three tag buckets,
// this is not double-counting to fix. Group sort order falls back through
// explicit sortBy, then the column's own configured option order (so a
// status/select bar chart matches that field's Kanban order), then value,
// because losing the configured order regressed a shipped chart before.
...
import { t } from "../i18n";
import { getColumnOptionValues, isOptionColumnType, toBooleanValue } from "./column-types";
