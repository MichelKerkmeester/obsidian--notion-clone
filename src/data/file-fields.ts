// ───────────────────────────────────────────────────────────────────
// MODULE:    file-fields
// COMPONENT: virtual file.* field namespace — resolution, read/write rules
// ───────────────────────────────────────────────────────────────────
//
// Every `file.*` key is reserved, even ones the plugin does not support, so
// a real frontmatter property can never collide with virtual file metadata.
// Capability differs per field (editable vs read-only, link vs self-link vs
// plain value), which is why this module exposes a set of narrow predicates
// instead of one shared type map — callers ask "can I edit this" or "does
// this render as a link" rather than branching on the key themselves.
...
import { toValidObsidianTagValues } from "./column-types";
