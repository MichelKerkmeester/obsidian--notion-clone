...
// COMPONENT: Evaluates a row against a view's conditional format rules and paints the matching style/icon
// ───────────────────────────────────────────────────────────────────
//
// Rules are tried in order and the first match wins with no merging — a later
// rule's color/bold/icon never blends with an earlier match, so rule order is
// itself the tie-break the UI exposes. `valueSource: "today"` rules resolve
// their comparison date at match time via prepareTree (a fresh tree copy, the
// original conditionTree is never mutated) instead of at rule-save time, so a
// "due today" rule keeps matching on the day it's evaluated rather than the
// day it was authored. A tree leaf referencing a field the row/schema no
// longer has collapses to a literal `false` so a stale rule fails closed
// instead of throwing or matching everything.

// ───────────────────────────────────────────────────────────────────
// 1. IMPORTS
// ───────────────────────────────────────────────────────────────────
import { getLocalDateKey } from "./calendar-date-time";
import { getColumnDisplayType } from "./column-display";
import { getFileFieldFixedType } from "./file-fields";
import { QueryEngine } from "./query-engine";
import { normalizeViewFilterTree } from "./view-filter-tree";
import { getValidRecordIconIds, renderRecordIcon } from "../views/record-icon-renderer";
import { parseRecordIconToken } from "./record-icon";
...
// ───────────────────────────────────────────────────────────────────
// 5. GET CONDITIONAL FORMAT MATCH
// ───────────────────────────────────────────────────────────────────
