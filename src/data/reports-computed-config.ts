// ───────────────────────────────────────────────────────────────────
// MODULE:    reports-computed-config
// COMPONENT: writes the Reports template's Remaining/Saved computed fields into a config
// ───────────────────────────────────────────────────────────────────
//
// This is the only writer for the two managed keys ("remaining", "saved") —
// it always replaces both the column and computed-field definitions in
// place (upsert, not append) so re-running the command is idempotent, and
// removes Saved's column/field entirely when the caller opts out rather
// than leaving a stale duplicate definition behind. The config passed in is
// never mutated; every return value is a fresh object so a caller diffing
// before/after can trust the input was inert.
...
import { findInspectedRollup, lockReportsExpressions } from "./reports-inspector";
import type {
