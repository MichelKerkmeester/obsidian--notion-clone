...
// COMPONENT: Bulk frontmatter-key operations — rename/copy/convert/delete across many files
// ───────────────────────────────────────────────────────────────────
//
// Routes every mutation through `processFrontmatter`, which prefers the
// injected `mutateFrontmatter` (DataSource) over calling
// `app.fileManager.processFrontMatter` directly, so bulk property edits
// share the same queued writes and metadata-cache overlays as regular cell
// edits instead of racing them. `renameKey`'s conflict handling treats a
// generated/stale target value as safe to overwrite (`isStaleGeneratedValue`)
// but leaves a genuinely user-authored value in place and counts it as a
// skipped conflict — silently clobbering user data on rename would be worse
// than leaving the old key behind.
...
import { getDefaultCellValue, normalizeOptionValueForKey, toBooleanValue, toMultiSelectValuesForKey } from "./column-types";
import type { FrontmatterMutator } from "./data-source";
