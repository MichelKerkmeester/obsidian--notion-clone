// ───────────────────────────────────────────────────────────────────
// MODULE:    card-roving-tabindex
// COMPONENT: shared WAI-ARIA roving-tabindex controller for Board/Gallery/List cards
// ───────────────────────────────────────────────────────────────────
//
// Implements a two-level nested-widget pattern: Tab reaches exactly one
// card, and Enter/F2 descends into that card's fields for a second roving
// level, mirroring how a native grid widget nests row and cell focus. The
// board's 2D column-aware navigation and the gallery/list's 1D linear
// navigation share this controller by passing `columnIndices` only when a
// layout actually has columns, rather than forking the controller in two.
