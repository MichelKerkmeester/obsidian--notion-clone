...
export const Grammar: Story = {
  render: () => {
    // `obnotion-owned-menu` is not decoration: the row's flex layout is declared there, not on the row.
    // Building a bare `obnotion-menu` container instead lays every row out inline with its trailing value
    // jammed against the label — which is exactly what this story showed before it was corrected.
    const menu = document.createElement("div");
    menu.className = "obnotion-menu obnotion-owned-menu";
