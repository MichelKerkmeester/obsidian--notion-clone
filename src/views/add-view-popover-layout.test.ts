...
// COMPONENT: CSS regression suite for the "add view" card grid popover
// ───────────────────────────────────────────────────────────────────
//
// Reads the shipped stylesheet and toolbar source directly rather than
// mounting the popover, because the bugs this guards against are shared
// selectors bleeding in from elsewhere (a bare `input` sizing rule that
// also stretches the duplicate checkbox, or `db-menu-item` turning a
// card into a horizontal row) — the kind of regression a DOM snapshot
// would not catch but a raw selector/declaration check does.
