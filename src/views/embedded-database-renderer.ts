// ───────────────────────────────────────────────────────────────────
// MODULE:    embedded-database-renderer
// COMPONENT: read/write renderer for a database view embedded inside a note
// ───────────────────────────────────────────────────────────────────
//
// The same database and view can be embedded more than once across open
// panes at the same time; every local write is tagged with `instanceId` so
// peer embeds react to the change instead of reprocessing the echo of their
// own save (see handleDataChanged/handlePeerViewConfigChanged). Refresh is
// deferred while the embed is scrolled out of view (IntersectionObserver),
// and table/chart updates are patched incrementally when the change set is
// small, so a live note preview does not re-render the whole embed on every
// unrelated data change elsewhere in the vault.
...
import { ensureColumnOrder, getColumnsInOrder, getVisibleColumns } from "../data/column-config";
...
import { calendarSearchResultsPlacement, getVisiblePopoverBounds, positionToolbarPopover } from "./popover-position";
import { captureEmbeddedHostViewport, DatabaseViewportRequest, EmbeddedHostViewportSnapshot, restoreEmbeddedHostViewport } from "./database-viewport";
