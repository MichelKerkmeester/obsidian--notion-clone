...
// COMPONENT: Column CRUD mutations — rename, delete, insert, duplicate,
//            change type — with cascading reference updates.
// ───────────────────────────────────────────────────────────────────
//
// Column keys fan out into dozens of places — sort/filter/group
// bindings, rollup targets, chart/calendar/timeline field refs,
// conditional formats — so every mutation here must walk and patch
// all of them or a rename/delete leaves dangling references.
// Frontmatter changes are computed as explicit before/after pairs
// (not applied directly) because the app's undo system replays them;
// computed and rollup columns are virtual and must be excluded from
// that frontmatter path entirely.
...
} from "../data/record-icon";
import { createOptionsFromValues, isOptionColumnType } from "../data/column-types";
import { getDefaultChartDateBucket, getDefaultChartNumberBucket, isChartAggregationValueColumn, requiresChartValueField } from "../data/chart-aggregation";
import { normalizeTimelineDayScale } from "../data/calendar-timeline-model";
import { isDateLikeColumnType } from "../data/date-time-format";
import { getConditionalFormatConditionFromTree } from "../data/conditional-format-column-ops";
import { removeSourceRuleTreeReferences, updateSourceRuleTreeKeyReferences } from "../data/source-rules";
import { pruneViewFilterTree } from "../data/view-filter-tree";
...
import { getFileFieldFixedType, isFileFieldKey, isSupportedFileField } from "../data/file-fields";
import { ColumnRenameResult } from "./modals/column-rename-modal";
import { confirmWithModal } from "./modals/confirm-modal";
import { DatabaseViewState, ViewStateStore } from "./view-state-store";
import { ColumnPropertySync } from "./column-property-sync";
...
export class ColumnOperations {
  ...
  // ───────────────────────────────────────────────────────────────────
  // 5. COLUMN INSERTION & DUPLICATION
  // ───────────────────────────────────────────────────────────────────
  ...
  // ───────────────────────────────────────────────────────────────────
  // 8. FRONTMATTER CHANGE COMPUTATION
  // ───────────────────────────────────────────────────────────────────
