// ───────────────────────────────────────────────────────────────────
// MODULE:    calendar-mini-calendar-renderer
// COMPONENT: compact day/month/year picker popover shared by calendar views
// ───────────────────────────────────────────────────────────────────
//
// The "has events" dots come from a date/month/year key index built once
// per open rather than a per-cell scan, since a full month or year grid
// checking every row on every cell would be quadratic in row count. Grid
// keyboard navigation onto a cell outside the current month hands off via
// onNavigateDate instead of just focusing it, so paging months stays in
// sync with the visible grid.
...
import { addDateKeyDays, dateKeyDaysBetween } from "../data/calendar-date-time";
import { buildCalendarTimelineEvents, CalendarDayModel, CalendarTimelineEvent } from "../data/calendar-timeline-model";
