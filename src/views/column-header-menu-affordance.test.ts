...
// ───────────────────────────────────────────────────────────────────
// MODULE:    column-header-menu-affordance
// COMPONENT: Regression suite locking the column header's menu-trigger
//            button and drag cursor to their inline, in-flow layout.
// ───────────────────────────────────────────────────────────────────
//
// Asserts directly on the shipped `styles.css` (not computed styles),
// because the bug this guards against was a later, higher-specificity
// rule silently overriding an earlier one — a computed-style snapshot
// in a browser test would hide exactly that kind of regression.
...
const columnHeaderSource = readFileSync(resolve(__dirname, "./column-header-controller.ts"), "utf-8");
const boardSource = readFileSync(resolve(__dirname, "./board-renderer.ts"), "utf-8");
