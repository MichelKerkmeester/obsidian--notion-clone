// ───────────────────────────────────────────────────────────────────
// MODULE:    formula-modal
// COMPONENT: computed-field formula workbench — editor, live preview, and function/field help browser
// ───────────────────────────────────────────────────────────────────
//
// Syntax highlighting, bracket matching, and property-name autocomplete
// are all hand-rolled over a plain `<textarea>` (no editor library), so
// every one of those subsystems re-derives its state from `textarea.value`
// and cursor position on each keystroke rather than owning independent
// state that could drift from what's actually typed. `close()` is
// overridden (not just `onClose()`) to intercept the discard-changes
// confirmation before Obsidian's modal-close teardown runs.
...
import { FORMULA_FILE_FIELDS } from "../../data/formula-fields";
import { formulaIfsSwitchMathHelp } from "../../data/formula-ifs-switch-math";
import { getFileFieldValue } from "../../data/file-fields";
