// ───────────────────────────────────────────────────────────────────
// MODULE:    relation-rollup-config-modal
// COMPONENT: single dialog covering both relation-target and rollup-aggregation configuration
// ───────────────────────────────────────────────────────────────────
//
// Rollup's target-field list is filtered by aggregation (sum/avg/etc.
// need a numeric target, earliest/latest need a date-like one) because
// letting a non-numeric column into a sum silently produces NaN at
// compute time instead of failing here where the user can see it.
// Changing the relation's target database surfaces a records-cleared /
// rollups-invalidated warning before save, since that action is
// otherwise a silent, unrecoverable data loss.
