// ───────────────────────────────────────────────────────────────────
// MODULE:    add-database-modal
// COMPONENT: "new database" dialog that edits an in-memory DatabaseConfig via the shared globals renderer
// ───────────────────────────────────────────────────────────────────
//
// Reuses ViewConfigPanelRenderer (the settings-popover editor) against a
// throwaway `tempDb` so this creation form can never visually or
// behaviorally drift from editing an existing database. Structural edits
// re-render on the next animation frame, never inside the triggering
// click/focus handler, so a rebuild can't detach the element mid-click.
