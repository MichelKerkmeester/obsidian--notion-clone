// ───────────────────────────────────────────────────────────────────
// MODULE:    interaction-scope
// COMPONENT: named interaction-scope registry (ownership + pause) and a focus trap
// ───────────────────────────────────────────────────────────────────
//
// ownsElement falls back to `closest(selector)` for portalSelectors because
// portal content (menus, popovers) is appended to document.body outside the
// root's own DOM subtree, so a plain root.contains() check would miss it.
// setPaused saves and later restores focus so a blocking modal opened over an
// active scope does not lose the caller's keyboard position when it closes.
